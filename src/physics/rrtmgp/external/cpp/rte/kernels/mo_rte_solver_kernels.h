...
inline void sw_two_stream(int ncol, int nlay, int ngpt, Mu0T const &mu0, TauT const &tau,
  ...
  TIMED_KERNEL(FLATTEN_MD_KERNEL3(ncol, nlay, ngpt, icol, ilay, igpt,
    ...
    //   gives relative error with respect to conservative solution
    //   of < 0.1% in Rdif down to tau = 10^-9
    // Aaron: Use a precision-dependent floor to limit cancellation in FP32 Rdir/Tdir.
    // Aaron: 1e-5 balances thin-layer accuracy against thick-cloud bias; FP64 keeps 1e-12.
    // Aaron: See docs/developer-guides/rrtmgp-modifications.md for the comparison.
