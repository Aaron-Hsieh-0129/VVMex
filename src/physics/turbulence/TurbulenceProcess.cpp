...
void TurbulenceProcess::init_dh_coefficients(Core::State& state) {
    ...
    // 1. Initialize to 1.0
    Kokkos::parallel_for("Init_DH_One",
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({{0, 0, 0}}, {{nz, ny, nx}}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            dhuu1(k,j,i)=1.; dhuu2(k,j,i)=1.;
            dhuv1(k,j,i)=1.; dhuv2(k,j,i)=1.;
            dhuw1(k,j,i)=1.; dhuw2(k,j,i)=1.;
            dhvu1(k,j,i)=1.; dhvu2(k,j,i)=1.;
            dhvv1(k,j,i)=1.; dhvv2(k,j,i)=1.;
            dhvw1(k,j,i)=1.; dhvw2(k,j,i)=1.;
            dhwu1(k,j,i)=1.; dhwu2(k,j,i)=1.;
            dhwv1(k,j,i)=1.; dhwv2(k,j,i)=1.;
            dhww1(k,j,i)=1.; dhww2(k,j,i)=1.;
        }
    );

    // 2. Clear coefficients inside terrain
    // [CRITICAL FIX] Loop range must match Fortran "DO K=2, NN".
    // Fortran K=1 is Surface. C++ k=1 is Surface.
    // So we loop k from 2 to NN. This PRESERVES k=1 as 1.0.
    Kokkos::parallel_for("Init_DH_Topo_Inside",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({{0, 0}}, {{ny, nx}}),
        KOKKOS_LAMBDA(const int j, const int i) {
            int NN = static_cast<int>(hx(j, i)); // hx is 1-based height index
            if (NN > 1) { // Only run if topo goes above k=1
                for (int k = 2; k <= NN; ++k) { 
                    if (k >= nz) break;
    ...
    // 3. ITYPEW Logic (Terrain neighbors)
    int k_start = halo, k_end = std::min(max_topo + halo + 5, nz - halo);
    int j_start = halo, j_end = ny - halo;
    int i_start = halo, i_end = nx - halo;

    Kokkos::parallel_for("Init_DH_ITYPEW",
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({{k_start, j_start, i_start}}, {{k_end, j_end, i_end}}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            if (ITYPEW(k, j, i) != 1) { // Solid point
                // I-1 (U & Scalar)
                dhuu1(k, j, i-1) = 0.; 
                dhuv1(k, j, i-1) = 0.; 
                dhuw1(k, j, i-1) = 0.;
                ...
                // Other specific directions (Scalars only for I+1/J+1)
                // [FIX] Removed dhuu2(i+1) and dhuw2(i+1) to strictly match Fortran
                dhwu2(k, j, i+1) = 0.; // DHWU2(I+1)
                
                dhwv2(k, j+1, i) = 0.; // DHWV2(J+1)
                
                dhww2(k+1, j, i) = 0.; // DHWW2(K+1)
                dhww1(k-1, j, i) = 0.; // DHWW1(K-1)
            }
        }
    );
    ...
    Kokkos::parallel_for("Init_DH_ITYPEU",
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({{k_start, j_start, i_start}}, {{nz-halo, j_end, i_end}}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            if (ITYPEU(k, j, i) != 1) {
                dhuu2(k, j, i+1) = 0.;
                dhuu1(k, j, i-1) = 0.;
    ...
    // 6. Domain Boundary Logic (Physical Edges)
    Kokkos::parallel_for("Init_DH_Edge_Y",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({{0, 0}}, {{nz, nx}}),
        KOKKOS_LAMBDA(const int k, const int i) {
            // North Boundary
            if (ITYPEW(k, ny-halo, i) != 1) {
                dhvu1(k, ny-halo-1, i) = 0.; dhvv1(k, ny-halo-1, i) = 0.; dhvw1(k, ny-halo-1, i) = 0.;
                dhvu2(k, ny-halo-1, i) = 0.; dhvv2(k, ny-halo-1, i) = 0.; dhvw2(k, ny-halo-1, i) = 0.;
                dhwv1(k, ny-halo-1, i) = 0.;
            }
            // South Boundary (mjm)
            if (ITYPEW(k, halo-1, i) != 1) dhwv2(k, halo, i) = 0.;

            if (ITYPEU(k, ny-halo, i) != 1) dhuv1(k, ny-halo-1, i) = 0.;
            if (ITYPEU(k, halo-1, i) != 1) dhuv2(k, halo, i) = 0.;
            
            if (ITYPEV(k, ny-halo, i) != 1) dhvv1(k, ny-halo-1, i) = 0.;
            if (ITYPEV(k, halo-1, i) != 1) dhvv2(k, halo, i) = 0.;
        }
    );
    ...
    Kokkos::parallel_for("Init_DH_Edge_X",
        ...
        KOKKOS_LAMBDA(const int k, const int j) {
            // East Boundary
            if (ITYPEW(k, j, nx-halo) != 1) {
                dhuu1(k, j, nx-halo-1) = 0.; dhuv1(k, j, nx-halo-1) = 0.; dhuw1(k, j, nx-halo-1) = 0.;
                dhuu2(k, j, nx-halo-1) = 0.; dhuv2(k, j, nx-halo-1) = 0.; dhuw2(k, j, nx-halo-1) = 0.;
                dhwu1(k, j, nx-halo-1) = 0.;
            }
            // West Boundary (mim)
            if (ITYPEW(k, j, halo-1) != 1) dhwu2(k, j, halo) = 0.;

            if (ITYPEU(k, j, nx-halo) != 1) dhuu1(k, j, nx-halo-1) = 0.;
            if (ITYPEU(k, j, halo-1) != 1) dhuu2(k, j, halo) = 0.;

            if (ITYPEV(k, j, nx-halo) != 1) dhvu1(k, j, nx-halo-1) = 0.;
            if (ITYPEV(k, j, halo-1) != 1) dhvu2(k, j, halo) = 0.;
        }
    );
...
void TurbulenceProcess::init_boundary_masks(Core::State& state) {
    ...
    int nx = grid_.get_local_total_points_x();
    int halo = grid_.get_halo_cells();
    Kokkos::resize(mask_view_, nz, ny, nx);

    TopoMask all_open;
    all_open.set_all_open();
    Kokkos::deep_copy(mask_view_, all_open);

    int max_topo_idx = params_.max_topo_idx;

    int k_phys_start = halo;
    int k_phys_end   = std::min(max_topo_idx + 2 + halo, nz - halo); 
    
    int j_phys_start = halo;
    int j_phys_end   = ny - halo;
    
    int i_phys_start = halo;
    int i_phys_end   = nx - halo;

    auto mask_view = mask_view_;

    Kokkos::parallel_for("Init_Topo_Masks_With_Halo",
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>(
            {{k_phys_start, j_phys_start, i_phys_start}},
            {{k_phys_end,   j_phys_end,   i_phys_end}}
        ),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            
            TopoMask mask = mask_view(k, j, i);

            int topo_h_idx = halo + static_cast<int>(hx(j, i));
            if (k < topo_h_idx) {
                mask.set_all_closed();
                mask_view(k, j, i) = mask;
                return;
            }
            ...
            // U-Channel (X-direction)
            if (ITYPEU(k, j, i + 1) != 1) mask.close_u_right();
            if (ITYPEU(k, j, i - 1) != 1) mask.close_u_left();

            // U-Channel (Y-direction)
            if (ITYPEU(k, j + 1, i) != 1) mask.close_u_front();
            if (ITYPEU(k, j - 1, i) != 1) mask.close_u_back();

            // U-Channel (Z-direction)
            if (ITYPEU(k + 1, j, i) != 1) mask.close_u_top();
            if (ITYPEU(k - 1, j, i) != 1) mask.close_u_bot();


            // V-Channel
            if (ITYPEV(k, j, i + 1) != 1) mask.close_v_right();
            if (ITYPEV(k, j, i - 1) != 1) mask.close_v_left();
            if (ITYPEV(k, j + 1, i) != 1) mask.close_v_front();
            if (ITYPEV(k, j - 1, i) != 1) mask.close_v_back();
            if (ITYPEV(k + 1, j, i) != 1) mask.close_v_top();
            if (ITYPEV(k - 1, j, i) != 1) mask.close_v_bot();


            // W-Channel
            if (ITYPEW(k, j, i + 1) != 1) mask.close_w_right();
            if (ITYPEW(k, j, i - 1) != 1) mask.close_w_left();
            if (ITYPEW(k, j + 1, i) != 1) mask.close_w_front();
            if (ITYPEW(k, j - 1, i) != 1) mask.close_w_back();
            if (ITYPEW(k + 1, j, i) != 1) mask.close_w_top();
            if (ITYPEW(k - 1, j, i) != 1) mask.close_w_bot();
...
void TurbulenceProcess::compute_coefficients(Core::State& state, double dt) 
{
    ...
    const auto mask_view = mask_view_;
    Kokkos::parallel_for("ShuttsGray_Coeffs",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            double rkm_val = 0.0;
            double rkh_val = 0.0;
            double sqrt_TERM1 = std::sqrt(TERM1);
            ...
            if (TERM1 == 0.) {
                ...
                if (Ri < 0.0) { 
                    ...
                    rkh_val = sqrt_TERM1 * DDX * 1.4 * (1.0 - 1.2 * Ri) * Kokkos::pow(1.-4.*Ri, 4.); } else { 
                    rkm_val = 0.0;
                    rkh_val = 0.0;
                }
            }
    ...
    // 1. 使用 Kokkos::parallel_reduce 找出目前 Rank 的最大值
    // 只統計物理區域 (Physical Domain)
    ...
    // 2. 使用 MPI_Reduce 找出全域最大值 (Global Max)
    ...
    if (grid_.get_mpi_rank() == 0) {
        // 假設 state 有 get_step() 方法，沒有的話可以拿掉
        // 您也可以加上時間資訊 dt
                ...
                KOKKOS_LAMBDA(const int k, const int j, const int i) {
                    ...
                    if (k != nz-h-1) {
                        d2dz2 =  0.5 * flex_height_coef_mid(k)*(flex_height_coef_up(k)*rhobar_up(k)*(RKH(k+1,j,i)+RKH(k,j,i))
                                      ...
                                      -flex_height_coef_up(k-1)*rhobar_up(k-1)*(RKH(k,j,i)+RKH(k-1,j,i))
                                            *(var(k,j,i)-var(k-1,j,i))*DHWW2(k,j,i) ) / flex_height_coef_mid(k) * rdz2;
                    }
                    else {
                        d2dz2 = -0.5*flex_height_coef_mid(NK2)*(flex_height_coef_up(NK1)*rhobar_up(NK1)*(RKH(NK2,j,i)+RKH(NK1,j,i))
                                    *(var(NK2,j,i)-var(NK1,j,i))) / rhobar(NK2) * rdz2; 
