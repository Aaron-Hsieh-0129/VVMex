...
KOKKOS_FUNCTION
void Functions<S,D>::nc_conservation(const Spack& nc, const Spack& nc_selfcollect_tend, const Real& dt, Spack& nc_nucleat_tend, Spack& nc_collect_tend, 
...
const bool& use_hetfrz_classnuc, const Smask& context)
{
  ...
  const auto source_nc = nc + (nc_nucleat_tend+nc_selfcollect_tend) * dt;
