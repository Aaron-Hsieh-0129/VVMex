...
public:
    ...
    void
    exchange_halos(FieldT& field, int depth = -1) const {
    ...
    void
    exchange_halos_top_slice(Field<3>& field) const {
...
inline HaloExchanger::HaloExchanger(const Utils::ConfigurationManager& config,
    const Grid& grid,
    ncclComm_t nccl_comm,
    cudaStream_t stream)
    : grid_ref_(grid), cart_comm_(grid.get_cart_comm()), nccl_comm_(nccl_comm), stream_(stream),
      exec_space_(stream) {
    is_single_rank_ = (grid.get_mpi_size() == 1);

    if (config.has_key("optimization.cuda_graph_halo_exchange")) {
        auto vars =
            config.get_value<std::vector<std::string>>("optimization.cuda_graph_halo_exchange");
    ...
    if (grid_ref_.get_mpi_rank() == 0 && !enabled_graph_vars_.empty()) {
        ...
        for (const auto& var : enabled_graph_vars_) {
            std::cout << var << " ";
        }
        std::cout << std::endl;
    }
    ...
    if (cart_comm_ != MPI_COMM_NULL) {
        ...
        for (int i = 0; i < 4; ++i) {
            if (cart_ranks[i] != MPI_PROC_NULL) {
                MPI_Group_translate_ranks(cart_group,
                    1,
                    &cart_ranks[i],
                    world_group,
                    &world_ranks[i]);
    ...
    if (h > 0) {
        ...
        size_t max_x =
            2 *
            std::max(
                {buffer_size_x_2d_, buffer_size_x_3d_, buffer_size_x_4d_, buffer_size_slice_x_});
        size_t max_y =
            2 *
            std::max(
                {buffer_size_y_2d_, buffer_size_y_3d_, buffer_size_y_4d_, buffer_size_slice_y_});
...
inline void
HaloExchanger::exchange_multiple_halos(const std::vector<std::string>& field_names,
    State& state) const {
    batch_fields_.clear();
...
inline void
HaloExchanger::exchange_multiple_halos(const std::vector<Field<3>*>& fields) const {
    if (fields.empty()) {
        return;
    }
    const int h = grid_ref_.get_halo_cells();
    ...
    const bool same_remote_x = neighbor_left != MPI_PROC_NULL && neighbor_left == neighbor_right &&
                               neighbor_left != my_rank;
    const bool same_remote_y = neighbor_bottom != MPI_PROC_NULL &&
    ...
    if (send_x_left_.extent(0) < required_x_total) {
        Kokkos::resize(send_x_left_, required_x_total);
        Kokkos::resize(recv_x_left_, required_x_total);
        Kokkos::resize(send_x_right_, required_x_total);
        Kokkos::resize(recv_x_right_, required_x_total);
    }
    if (send_y_bottom_.extent(0) < required_y_total) {
        Kokkos::resize(send_y_bottom_, required_y_total);
        Kokkos::resize(recv_y_bottom_, required_y_total);
        Kokkos::resize(send_y_top_, required_y_total);
        Kokkos::resize(recv_y_top_, required_y_total);
    }
    ...
    if (grid_ref_.is_singleton_x()) {
        for (Field<3>* field : fields) {
            if (field) {
                Detail::fill_singleton_x_halo(exec_space_, *field, halo_start_offset, h);
            }
        }
    }
    if (grid_ref_.is_singleton_y()) {
        for (Field<3>* field : fields) {
            if (field) {
                Detail::fill_singleton_y_halo(exec_space_, *field, halo_start_offset, h);
            }
        }
    }
    ...
    if (count_x_total > 0 && !grid_ref_.is_singleton_x()) {
        for (size_t f = 0; f < num_fields; ++f) {
            ...
            auto send_l =
                Kokkos::subview(send_x_left_, std::make_pair(offset, offset + buffer_size_x_3d_));
            auto send_r =
                Kokkos::subview(send_x_right_, std::make_pair(offset, offset + buffer_size_x_3d_));

            Kokkos::parallel_for("pack_multi_x",
        ...
        if (is_single || (neighbor_left == my_rank && neighbor_right == my_rank)) {
            Kokkos::deep_copy(exec_space_,
                ...
                Kokkos::subview(send_x_right_, std::make_pair((size_t)0, count_x_total)));
            Kokkos::deep_copy(exec_space_,
            ...
            Kokkos::deep_copy(exec_space_,
                ...
                Kokkos::subview(send_x_left_, std::make_pair((size_t)0, count_x_total)));
            ncclGroupStart();
            ncclSend(send_x_right_.data(),
                2 * count_x_total,
                VVM_NCCL_REAL,
                neighbor_right,
                nccl_comm_,
                stream_);
            ncclRecv(recv_x_right_.data(),
                2 * count_x_total,
                VVM_NCCL_REAL,
                neighbor_left,
                nccl_comm_,
        ...
        for (size_t f = 0; f < num_fields; ++f) {
            ...
            size_t offset = f * buffer_size_x_3d_;
            auto recv_l =
                Kokkos::subview(recv_x_left_, std::make_pair(offset, offset + buffer_size_x_3d_));
            auto recv_r =
                Kokkos::subview(recv_x_right_, std::make_pair(offset, offset + buffer_size_x_3d_));

            Kokkos::parallel_for("unpack_multi_x",
                Kokkos::MDRangePolicy<Kokkos::Rank<3>, ExecSpace>(exec_space_,
                    {0, 0, 0},
                    {nz, ny, h}),
                KOKKOS_LAMBDA(int k, int j, int i_h) {
                    const size_t idx = k * (ny * h) + j * h + i_h;
                    if (neighbor_left != MPI_PROC_NULL || is_single) {
                        data(k, j, halo_start_offset - h + i_h) = recv_l(idx);
                    }
                    if (neighbor_right != MPI_PROC_NULL || is_single) {
                        data(k, j, halo_start_offset + nx_phys + i_h) = recv_r(idx);
                    }
                });
        }
    }
    ...
    if (count_y_total > 0 && !grid_ref_.is_singleton_y()) {
        for (size_t f = 0; f < num_fields; ++f) {
            ...
            auto send_b =
                Kokkos::subview(send_y_bottom_, std::make_pair(offset, offset + buffer_size_y_3d_));
            auto send_t =
                Kokkos::subview(send_y_top_, std::make_pair(offset, offset + buffer_size_y_3d_));

            Kokkos::parallel_for("pack_multi_y",
                Kokkos::MDRangePolicy<Kokkos::Rank<3>, ExecSpace>(exec_space_,
                    {0, 0, 0},
                    {nz, nx, h}),
        ...
        if (is_single || (neighbor_bottom == my_rank && neighbor_top == my_rank)) {
            Kokkos::deep_copy(exec_space_,
                ...
                Kokkos::subview(send_y_top_, std::make_pair((size_t)0, count_y_total)));
            Kokkos::deep_copy(exec_space_,
            ...
            Kokkos::deep_copy(exec_space_,
                ...
                Kokkos::subview(send_y_bottom_, std::make_pair((size_t)0, count_y_total)));
            ncclGroupStart();
            ncclSend(send_y_top_.data(),
                2 * count_y_total,
                VVM_NCCL_REAL,
                neighbor_top,
                nccl_comm_,
                stream_);
            ncclRecv(recv_y_top_.data(),
                2 * count_y_total,
                VVM_NCCL_REAL,
                neighbor_bottom,
                nccl_comm_,
        ...
        for (size_t f = 0; f < num_fields; ++f) {
            ...
            size_t offset = f * buffer_size_y_3d_;
            auto recv_b =
                Kokkos::subview(recv_y_bottom_, std::make_pair(offset, offset + buffer_size_y_3d_));
            auto recv_t =
                Kokkos::subview(recv_y_top_, std::make_pair(offset, offset + buffer_size_y_3d_));

            Kokkos::parallel_for("unpack_multi_y",
                Kokkos::MDRangePolicy<Kokkos::Rank<3>, ExecSpace>(exec_space_,
                    {0, 0, 0},
                    {nz, nx, h}),
                KOKKOS_LAMBDA(int k, int i, int j_h) {
                    const size_t idx = k * (h * nx) + j_h * nx + i;
                    if (neighbor_bottom != MPI_PROC_NULL || is_single) {
                        data(k, halo_start_offset - h + j_h, i) = recv_b(idx);
                    }
                    if (neighbor_top != MPI_PROC_NULL || is_single) {
...
inline void
HaloExchanger::exchange_multiple_halos(const std::vector<Field<2>*>& fields, int depth) const {
    if (fields.empty()) {
        return;
    }
    ...
    assert(
        count_x_total * 2 <= send_x_left_.extent(0) && "2-D batch exceeds pre-sized halo buffer");
    assert(
...
inline void
HaloExchanger::exchange_halos(State& state) {
    // Kokkos::fence();
...
HaloExchanger::exchange_halos_impl(FieldT& field, int depth) const {
    ...
    if (is_single_rank_ && (grid_ref_.is_singleton_x() || grid_ref_.is_singleton_y())) {
        Detail::fill_periodic_x_halo(exec_space_, field, halo_start_offset, nx_phys, h);
        Detail::fill_periodic_y_halo(exec_space_, field, halo_start_offset, ny_phys, h);
        return;
    }
    ...
    if (is_single_rank_) {
        ...
        if constexpr (Dim == 2) {
            ...
            Kokkos::parallel_for("local_copy_x_2d",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {ny, h}),
                KOKKOS_LAMBDA(int j, int i_h) {
            ...
            Kokkos::parallel_for("local_copy_x_3d",
                ...
                KOKKOS_LAMBDA(int k, int j, int i_h) {
                    ...
                    data(k, j, halo_start_offset - h + i_h) =
                        data(k, j, halo_start_offset + nx_phys - h + i_h);
                    // Right Halo gets Left Physical
                    data(k, j, halo_start_offset + nx_phys + i_h) =
                        data(k, j, halo_start_offset + i_h);
                });
        }
            ...
            Kokkos::parallel_for("local_copy_x_4d",
                Kokkos::MDRangePolicy<Kokkos::Rank<4>, ExecSpace>(exec_space_,
                    {0, 0, 0, 0},
                    {nw, nz, ny, h}),
                KOKKOS_LAMBDA(int w, int k, int j, int i_h) {
                    data(w, k, j, halo_start_offset - h + i_h) =
                        data(w, k, j, halo_start_offset + nx_phys - h + i_h);
                    data(w, k, j, halo_start_offset + nx_phys + i_h) =
                        data(w, k, j, halo_start_offset + i_h);
                });
        }
        ...
        if constexpr (Dim == 2) {
            ...
            Kokkos::parallel_for("local_copy_y_2d",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {nx, h}),
                KOKKOS_LAMBDA(int i, int j_h) {
            ...
            Kokkos::parallel_for("local_copy_y_4d",
                Kokkos::MDRangePolicy<Kokkos::Rank<4>, ExecSpace>(exec_space_,
                    {0, 0, 0, 0},
                    {nw, nz, nx, h}),
                KOKKOS_LAMBDA(int w, int k, int i, int j_h) {
                    data(w, k, halo_start_offset - h + j_h, i) =
                        data(w, k, halo_start_offset + ny_phys - h + j_h, i);
                    data(w, k, halo_start_offset + ny_phys + j_h, i) =
                        data(w, k, halo_start_offset + j_h, i);
                });
        }
    ...
    if (count_x > 0 && !grid_ref_.is_singleton_x()) {
        ...
        if (neighbor_left != MPI_PROC_NULL && neighbor_left == my_rank &&
            neighbor_right == my_rank) {
        ...
        if constexpr (Dim == 2) {
            const int ny = data.extent(0);
            Kokkos::parallel_for("unpack_x_2d",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {ny, h}),
                KOKKOS_LAMBDA(int j, int i_h) {
                    if (neighbor_left != MPI_PROC_NULL) {
                        data(j, halo_start_offset - h + i_h) = recv_l(j * h + i_h);
                    }
                    if (neighbor_right != MPI_PROC_NULL) {
                        data(j, halo_start_offset + nx_phys + i_h) = recv_r(j * h + i_h);
                    }
                });
        }
        else if constexpr (Dim == 3) {
            ...
            const int nz = data.extent(1);
            const int ny = data.extent(2);
            Kokkos::parallel_for("unpack_x_4d",
                Kokkos::MDRangePolicy<Kokkos::Rank<4>, ExecSpace>(exec_space_,
                    {0, 0, 0, 0},
                    {nw, nz, ny, h}),
                KOKKOS_LAMBDA(int w, int k, int j, int i_h) {
                    size_t idx = w * (nz * ny * h) + k * (ny * h) + j * h + i_h;
                    if (neighbor_left != MPI_PROC_NULL) {
                        data(w, k, j, halo_start_offset - h + i_h) = recv_l(idx);
                    }
                    if (neighbor_right != MPI_PROC_NULL) {
                        data(w, k, j, halo_start_offset + nx_phys + i_h) = recv_r(idx);
                    }
                });
        }
    }
    ...
    if (count_y > 0 && !grid_ref_.is_singleton_y()) {
        ...
        if (neighbor_bottom != MPI_PROC_NULL && neighbor_bottom == my_rank &&
            neighbor_top == my_rank) {
        ...
        if constexpr (Dim == 2) {
            const int nx = data.extent(1);
            Kokkos::parallel_for("unpack_y_2d",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {nx, h}),
                KOKKOS_LAMBDA(int i, int j_h) {
                    const size_t idx = j_h * nx + i;
                    if (neighbor_bottom != MPI_PROC_NULL) {
            ...
            const int nz = data.extent(1);
            const int nx = data.extent(3);
            Kokkos::parallel_for("unpack_y_4d",
                Kokkos::MDRangePolicy<Kokkos::Rank<4>, ExecSpace>(exec_space_,
                    {0, 0, 0, 0},
                    {nw, nz, nx, h}),
                KOKKOS_LAMBDA(int w, int k, int i, int j_h) {
                    size_t idx = w * (nz * h * nx) + k * (h * nx) + j_h * nx + i;
                    if (neighbor_bottom != MPI_PROC_NULL) {
                        data(w, k, halo_start_offset - h + j_h, i) = recv_b(idx);
                    }
                    if (neighbor_top != MPI_PROC_NULL) {
                        data(w, k, halo_start_offset + ny_phys + j_h, i) = recv_t(idx);
                    }
                });
        }
    }
...
inline void
HaloExchanger::exchange_halos_slice(Field<3>& field, int k_layer) const {
    const int halo_start_offset = grid_ref_.get_halo_cells();
    const int h = grid_ref_.get_halo_cells();
    if (h == 0) {
        return;
    ...
    if (is_single_rank_ && (grid_ref_.is_singleton_x() || grid_ref_.is_singleton_y())) {
        Detail::fill_periodic_x_slice(exec_space_, field, k_layer, h, nx_phys, h);
        Detail::fill_periodic_y_slice(exec_space_, field, k_layer, h, ny_phys, h);
        cudaStreamSynchronize(stream_);
        return;
    }
    if (grid_ref_.is_singleton_x()) {
        Detail::fill_singleton_x_slice(exec_space_, field, k_layer, h, h);
    }
    if (grid_ref_.is_singleton_y()) {
        Detail::fill_singleton_y_slice(exec_space_, field, k_layer, h, h);
    }
    ...
    if (!grid_ref_.is_singleton_y()) {
        ...
        if (count > 0) {
            ...
            Kokkos::parallel_for("pack_y_slice",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {nx, h}),
                KOKKOS_LAMBDA(int i, int j_h) {
                    const size_t idx = static_cast<size_t>(j_h) * nx + i;
            ...
            Kokkos::parallel_for("unpack_y_slice",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {nx, h}),
                KOKKOS_LAMBDA(int i, int j_h) {
                    const size_t idx = static_cast<size_t>(j_h) * nx + i;
                    if (neighbor_bottom != MPI_PROC_NULL) {
                        data(k_layer, halo_start_offset - h + j_h, i) = recv_b(idx);
                    }
                    if (neighbor_top != MPI_PROC_NULL) {
                        data(k_layer, halo_start_offset + ny_phys + j_h, i) = recv_t(idx);
                    }
                });
        }
    }
    ...
    if (!grid_ref_.is_singleton_x()) {
        ...
        if (count > 0) {
            ...
            Kokkos::parallel_for("pack_x_slice",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {ny, h}),
                KOKKOS_LAMBDA(int j, int i_h) {
                    const size_t idx = static_cast<size_t>(j) * h + i_h;
            ...
            Kokkos::parallel_for("unpack_x_slice",
                Kokkos::MDRangePolicy<Kokkos::Rank<2>, ExecSpace>(exec_space_, {0, 0}, {ny, h}),
                KOKKOS_LAMBDA(int j, int i_h) {
                    const size_t idx = static_cast<size_t>(j) * h + i_h;
                    if (neighbor_left != MPI_PROC_NULL) {
                        data(k_layer, j, halo_start_offset - h + i_h) = recv_l(idx);
                    }
                    if (neighbor_right != MPI_PROC_NULL) {
                        data(k_layer, j, halo_start_offset + nx_phys + i_h) = recv_r(idx);
                    }
                });
        }
    }
