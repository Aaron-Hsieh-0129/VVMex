...
inline HaloExchanger::HaloExchanger(const Grid& grid)
    ...
    if (cart_comm_ == MPI_COMM_NULL) {
        std::cerr << "Rank " << rank << ": HaloExchanger initialized with NULL communicator!"
                  << std::endl;
...
inline void
HaloExchanger::exchange_halos(State& state) const {
...
template <typename FieldT>
HaloExchangeRequests
HaloExchanger::post_exchange_halo_x(FieldT& field, int depth) const {
    ...
    if (grid_ref_.is_singleton_x()) {
        Detail::fill_singleton_x_halo(Kokkos::DefaultExecutionSpace{}, field, halo_start_offset, h);
        return {};
    }
    ...
    size_t count = 0;
    if constexpr (Dim == 2) {
        count = static_cast<size_t>(h) * data.extent(0);
    }
    else if constexpr (Dim == 3) {
        count = static_cast<size_t>(h) * data.extent(1) * data.extent(0);
    }
    else if constexpr (Dim == 4) {
        count = static_cast<size_t>(h) * data.extent(2) * data.extent(1) * data.extent(0);
    }
    ...
    if constexpr (Dim == 2) {
        ...
        Kokkos::parallel_for("pack_x_4d",
            Kokkos::MDRangePolicy<Kokkos::Rank<4>>({0, 0, 0, 0}, {nw, nz, ny, h}),
...
template <typename FieldT>
void
HaloExchanger::wait_exchange_halo_x(FieldT& field, HaloExchangeRequests& reqs, int depth) const {
    constexpr size_t Dim = FieldT::DimValue;
    if (reqs.count == 0) {
        return;
    }
...
template <typename FieldT>
HaloExchangeRequests
HaloExchanger::post_exchange_halo_y(FieldT& field, int depth) const {
    constexpr size_t Dim = FieldT::DimValue;
    const int halo_start_offset = grid_ref_.get_halo_cells();
    const int h = (depth == -1) ? grid_ref_.get_halo_cells() : depth;
    if (h == 0) {
    ...
    if (grid_ref_.is_singleton_y()) {
        Detail::fill_singleton_y_halo(Kokkos::DefaultExecutionSpace{}, field, halo_start_offset, h);
        return {};
    }
    ...
    size_t count = 0;
    if constexpr (Dim == 2) {
        count = static_cast<size_t>(h) * data.extent(1);
    }
    else if constexpr (Dim == 3) {
        count = static_cast<size_t>(h) * data.extent(2) * data.extent(0);
    }
    else if constexpr (Dim == 4) {
        count = static_cast<size_t>(h) * data.extent(3) * data.extent(1) * data.extent(0);
    }
    ...
    if constexpr (Dim == 2) {
        ...
        Kokkos::parallel_for("pack_y_3d",
            Kokkos::MDRangePolicy<Kokkos::Rank<3>>({0, 0, 0}, {nz, nx, h}),
        ...
        Kokkos::parallel_for("pack_y_4d",
            Kokkos::MDRangePolicy<Kokkos::Rank<4>>({0, 0, 0, 0}, {nw, nz, nx, h}),
...
template <typename FieldT>
void
HaloExchanger::wait_exchange_halo_y(FieldT& field, HaloExchangeRequests& reqs, int depth) const {
    constexpr size_t Dim = FieldT::DimValue;
    if (reqs.count == 0) {
        return;
    }
...
HaloExchanger::exchange_multiple_halos(const std::vector<Field<3>*>& fields) const {
    ...
    if (send_x_left_.extent(0) < count_x_total) {
        Kokkos::resize(send_x_left_, count_x_total);
        Kokkos::resize(recv_x_left_, count_x_total);
        Kokkos::resize(send_x_right_, count_x_total);
        Kokkos::resize(recv_x_right_, count_x_total);
    }
    if (send_y_bottom_.extent(0) < count_y_total) {
        Kokkos::resize(send_y_bottom_, count_y_total);
        Kokkos::resize(recv_y_bottom_, count_y_total);
        Kokkos::resize(send_y_top_, count_y_total);
        Kokkos::resize(recv_y_top_, count_y_total);
    }
    ...
    if (grid_ref_.is_singleton_y()) {
        for (Field<3>* field : fields) {
            if (field) {
                Detail::fill_singleton_y_halo(Kokkos::DefaultExecutionSpace{},
                    *field,
                    halo_start_offset,
                    h);
            }
    ...
    if (grid_ref_.is_singleton_x()) {
        for (Field<3>* field : fields) {
            if (field) {
                Detail::fill_singleton_x_halo(Kokkos::DefaultExecutionSpace{},
                    *field,
                    halo_start_offset,
                    h);
            }
        ...
        for (size_t f = 0; f < num_fields; ++f) {
            ...
            Kokkos::parallel_for("unpack_multi_x",
                ...
                KOKKOS_LAMBDA(int k, int j, int i_h) {
                    const size_t idx = k * (ny * h) + j * h + i_h;
                    if (neighbor_left != MPI_PROC_NULL) {
                        data(k, j, halo_start_offset - h + i_h) = recv_l_f(idx);
                    }
                    if (neighbor_right != MPI_PROC_NULL) {
                        data(k, j, halo_start_offset + nx_phys + i_h) = recv_r_f(idx);
                    }
                });
        }
    }
        ...
        for (size_t f = 0; f < num_fields; ++f) {
            ...
            Kokkos::parallel_for("unpack_multi_y",
                ...
                KOKKOS_LAMBDA(int k, int i, int j_h) {
                    const size_t idx = k * (h * nx) + j_h * nx + i;
                    if (neighbor_bottom != MPI_PROC_NULL) {
                        data(k, halo_start_offset - h + j_h, i) = recv_b_f(idx);
                    }
                    if (neighbor_top != MPI_PROC_NULL) {
                        data(k, halo_start_offset + ny_phys + j_h, i) = recv_t_f(idx);
                    }
                });
        }
    }
...
HaloExchanger::exchange_halos_slice(Field<3>& field, int k_layer) const {
    ...
    if (grid_ref_.is_singleton_y()) {
        Detail::fill_singleton_y_slice(Kokkos::DefaultExecutionSpace{}, field, k_layer, h, h);
    }
    if (grid_ref_.is_singleton_x()) {
        Detail::fill_singleton_x_slice(Kokkos::DefaultExecutionSpace{}, field, k_layer, h, h);
    }
        ...
        if (count > 0) {
            ...
            if (neighbor_bottom_ != MPI_PROC_NULL) {
                MPI_Irecv(recv_b.data(),
                    ...
                    VVM_MPI_REAL,
                    neighbor_bottom_,
                    static_cast<int>(HaloExchangeTags::SLICE_SEND_TO_TOP),
                    cart_comm_,
                    &reqs[req_count++]);
            }
            if (neighbor_top_ != MPI_PROC_NULL) {
                MPI_Irecv(recv_t.data(),
                    ...
                    VVM_MPI_REAL,
                    neighbor_top_,
                    static_cast<int>(HaloExchangeTags::SLICE_SEND_TO_BOTTOM),
                    cart_comm_,
                    &reqs[req_count++]);
            }
            if (neighbor_top_ != MPI_PROC_NULL) {
            ...
            Kokkos::parallel_for("unpack_y_slice",
                ...
                KOKKOS_LAMBDA(int i, int j_h) {
                    const size_t idx = static_cast<size_t>(j_h) * nx + i;
                    if (neighbor_bottom != MPI_PROC_NULL) {
                        data(k_layer, j_h, i) = recv_b(idx);
                    }
                    if (neighbor_top != MPI_PROC_NULL) {
                        data(k_layer, h + ny_phys + j_h, i) = recv_t(idx);
                    }
                });
            // Kokkos::fence();
        }
        ...
        if (count > 0) {
            ...
            if (neighbor_right_ != MPI_PROC_NULL) {
                MPI_Irecv(recv_r.data(),
                    ...
                    VVM_MPI_REAL,
                    neighbor_right_,
                    static_cast<int>(HaloExchangeTags::SLICE_SEND_TO_LEFT),
                    cart_comm_,
                    &reqs[req_count++]);
            }
            if (neighbor_left_ != MPI_PROC_NULL) {
                MPI_Irecv(recv_l.data(),
                    ...
                    VVM_MPI_REAL,
                    neighbor_left_,
                    static_cast<int>(HaloExchangeTags::SLICE_SEND_TO_RIGHT),
                    cart_comm_,
                    &reqs[req_count++]);
            }
            if (neighbor_left_ != MPI_PROC_NULL) {
            ...
            Kokkos::parallel_for("unpack_x_slice",
                ...
                KOKKOS_LAMBDA(int j, int i_h) {
                    const size_t idx = static_cast<size_t>(j) * h + i_h;
                    if (neighbor_left != MPI_PROC_NULL) {
                        data(k_layer, j, i_h) = recv_l(idx);
                    }
                    if (neighbor_right != MPI_PROC_NULL) {
                        data(k_layer, j, h + nx_phys + i_h) = recv_r(idx);
                    }
                });
            // Kokkos::fence();
        }
