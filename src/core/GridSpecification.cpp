...
constexpr const char* default_rcemip_grid_data_path =
...
void
require_positive_integer(const int value, const char* key) {
    if (value <= 0) {
...
void
require_valid_halo_width(const int value, const char* key) {
    if (value < 2) {
        configuration_error(
            std::string("'") + key + "' must be at least 2 for the Takacs stencil.");
    }
}

void
require_finite_positive(const VVM::Real value, const char* key) {
    if (!std::isfinite(value) || value <= VVM::real(0.0)) {
        configuration_error(std::string("'") + key + "' must be finite and greater than zero.");
    }
}

HorizontalEdgeTopology
parse_structured_topology(const std::string& value, const char* key) {
    if (value == "periodic") {
...
HorizontalEdgeTopology
parse_legacy_boundary(const std::string& value, const char* key) {
    if (value == "periodic") {
        return HorizontalEdgeTopology::Periodic;
    }
    ...
    configuration_error(
        std::string("'") + key + "' must be 'periodic', 'zero_gradient', or 'bounded'.");

    return HorizontalEdgeTopology::Bounded;
}

VerticalCoordinateType
parse_vertical_coordinate_type(const std::string& value, const char* key) {
    if (value == "default") {
...
parse_geometry_kind(const std::string& value, const char* key) {
    ...
    configuration_error(
        std::string("'") + key + "' must be 'cartesian', 'regular_latlon', or 'cubed_sphere'.");

    return Geometry::GeometryKind::Cartesian;
}

std::vector<double>
read_bounds(const Utils::ConfigurationManager& config, const char* key) {
    const std::vector<double> bounds = config.get_value<std::vector<double>>(key);

    if (bounds.size() != 2) {
        configuration_error(std::string("'") + key + "' must contain exactly two values.");
    }

    if (!std::isfinite(bounds[0]) || !std::isfinite(bounds[1])) {
        configuration_error(std::string("'") + key + "' must contain finite values.");
    }
...
void
configure_cartesian_geometry(HorizontalDomainSpec& horizontal,
...
void
configure_regular_lat_lon_geometry(HorizontalDomainSpec& horizontal,
    const Utils::ConfigurationManager& config) {
    const VVM::Real radius = config.get_value<VVM::Real>("grid.horizontal.geometry.earth_radius_m");
    const std::vector<double> longitude_bounds =
        read_bounds(config, "grid.horizontal.geometry.longitude_bounds_deg");
    const std::vector<double> latitude_bounds =
        read_bounds(config, "grid.horizontal.geometry.latitude_bounds_deg");
    ...
    if (longitude_east <= longitude_west) {
        configuration_error(
            "'grid.horizontal.geometry.longitude_bounds_deg' must be ordered west to east.");
    }
    ...
    if (longitude_span > 360.0) {
        configuration_error(
            "'grid.horizontal.geometry.longitude_bounds_deg' must not span more than 360 degrees.");
    }

    if (latitude_north <= latitude_south) {
        configuration_error(
            "'grid.horizontal.geometry.latitude_bounds_deg' must be ordered south to north.");
    }

    if (latitude_south <= -90.0 || latitude_north >= 90.0) {
        configuration_error("'grid.horizontal.geometry.latitude_bounds_deg' must remain strictly "
                            "between the poles.");
    }

    const double dlongitude_degrees = longitude_span / static_cast<double>(horizontal.nx);
    const double dlatitude_degrees =
        (latitude_north - latitude_south) / static_cast<double>(horizontal.ny);

    const double minimum_halo_latitude =
        latitude_south + (0.5 - static_cast<double>(horizontal.n_halo_cells)) * dlatitude_degrees;
    const double maximum_halo_latitude =
        latitude_north + static_cast<double>(horizontal.n_halo_cells) * dlatitude_degrees;
    ...
    if (horizontal.fix_lonlat) {
        configuration_error("'grid.horizontal.geometry.fix_lonlat' must be false for regular "
    ...
    horizontal.geometry.regular_lat_lon.longitude_west_edge =
        static_cast<VVM::Real>(longitude_west * degrees_to_radians);
...
HorizontalDomainSpec
parse_structured_horizontal(const Utils::ConfigurationManager& config) {
    ...
    horizontal.topology.q1 =
        parse_structured_topology(config.get_value<std::string>("grid.horizontal.topology.q1"),
            "grid.horizontal.topology.q1");
    horizontal.topology.q2 =
        parse_structured_topology(config.get_value<std::string>("grid.horizontal.topology.q2"),
            "grid.horizontal.topology.q2");

    const Geometry::GeometryKind kind =
        parse_geometry_kind(config.get_value<std::string>("grid.horizontal.geometry.kind"),
            "grid.horizontal.geometry.kind");
...
HorizontalDomainSpec
parse_legacy_horizontal(const Utils::ConfigurationManager& config) {
    HorizontalDomainSpec horizontal;
    ...
    horizontal.topology.q1 = parse_legacy_boundary(
        config.get_value<std::string>("grid.boundary_condition.x", "periodic"),
        "grid.boundary_condition.x");
    horizontal.topology.q2 = parse_legacy_boundary(
        config.get_value<std::string>("grid.boundary_condition.y", "periodic"),
        "grid.boundary_condition.y");
...
void
validate_vertical_spec(
    const VerticalGridSpec& vertical, const char* nz_key, const char* dz_key, const char* dz1_key) {
    ...
    if (vertical.type != VerticalCoordinateType::RCEMIP && vertical.dz1 > vertical.dz) {
        configuration_error(std::string("'") + dz1_key + "' must not exceed '" + dz_key +
...
VerticalGridSpec
parse_structured_vertical(const Utils::ConfigurationManager& config) {
    ...
    vertical.type = parse_vertical_coordinate_type(
        config.get_value<std::string>("grid.vertical.type", "default"),
        "grid.vertical.type");
    vertical.rcemip_grid_data_path =
        config.get_value<std::string>("grid.vertical.rcemip_grid_data_path", "");
...
VerticalGridSpec
parse_legacy_vertical(const Utils::ConfigurationManager& config) {
    VerticalGridSpec vertical;
    ...
    vertical.type = parse_vertical_coordinate_type(
        ...
        "grid.vertical_coordinate_type");
    vertical.rcemip_grid_data_path =
        config.get_value<std::string>("grid.rcemip_grid_data_path", default_rcemip_grid_data_path);
...
const char*
horizontal_edge_topology_to_string(const HorizontalEdgeTopology topology) noexcept {
...
const char*
vertical_coordinate_type_to_string(const VerticalCoordinateType type) noexcept {
...
GridSpecification
GridSpecification::from_config(const Utils::ConfigurationManager& config) {
