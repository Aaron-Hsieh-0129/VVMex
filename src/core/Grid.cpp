...
Grid::Grid(const VVM::Utils::ConfigurationManager& config, MPI_Comm comm)
    : dims_device_view_("GridDimensions", 3), // Initialize Kokkos::View for 3 dimensions (Z, Y, X)
      ...
      comm_(comm), cart_comm_(MPI_COMM_NULL)       // Initialize MPI_Comm to NULL for safety
{
...
void
Grid::calculate_local_grid_distribution() {
    ...
    int periods[2] = {horizontal.ny > 1 && topology.q2 == HorizontalEdgeTopology::Periodic ? 1 : 0,
    ...
    if (dims_host_mirror_(1).global_size == 1 && dims_host_mirror_(2).global_size > 1) {
        ...
        const int status = MPI_Dims_create(mpi_size_,
            2,
            process_dimensions); // Let MPI find the best 2D distribution
        if (status != MPI_SUCCESS) {
            throw std::runtime_error(
                "MPI_Dims_create failed to construct the horizontal process topology.");
        }
        ndims_cart = 2;
        if (mpi_rank_ == 0) {
        ...
        if (mpi_rank_ == 0) {
            ...
            if (mpi_size_ > 1) {
                std::cout << "WARNING: With nx=1 and ny=1, all MPI ranks will have full copy of "
                             "horizontal domain."
                          << std::endl;
            }
        }
    ...
    if (mpi_rank_ == 0) {
        std::cout << "MPI 2D Decomposition: Px=" << process_dimensions[1]
                  << ", Py=" << process_dimensions[0] << std::endl;

        if (periods[1] == 1) {
            std::cout << "Boundary Conditions: X-Periodic, ";
        }
        else {
            std::cout << "Boundary Conditions: X-Non Periodic, ";
        }
        ...
        if (mpi_rank_ == 0) {
            std::cout << "WARNING: " << msg.str() << "\n"
                      << "  Tolerated because radiation is off; it would abort with "
                         "physics.rrtmgp.enable_rrtmgp=true."
                      << std::endl;
        }
    }
    ...
    const int cart_status =
        MPI_Cart_create(comm_, ndims_cart, process_dimensions, periods, reorder, &cart_comm_);
    if (cart_status != MPI_SUCCESS || cart_comm_ == MPI_COMM_NULL) {
        throw std::runtime_error(
            "MPI_Cart_create failed; the process-grid dimensions do not match the communicator.");
    }
    ...
    if (process_dimensions[0] == 1) { // Only one process in Y direction, so no decomposition
        ...
        dims_host_mirror_(1).local_physical_start_idx =
            coords[0] * base_local_N_y + std::min(coords[0], remainder_y);
        dims_host_mirror_(1).local_physical_size =
            base_local_N_y + (coords[0] < remainder_y ? 1 : 0);
        dims_host_mirror_(1).local_physical_end_idx =
            dims_host_mirror_(1).local_physical_start_idx +
            dims_host_mirror_(1).local_physical_size - 1;
    }
    ...
    if (dims_host_mirror_(2).global_size == 1) {
        ...
        dims_host_mirror_(2).local_physical_start_idx =
            coords[1] * base_local_N_x + std::min(coords[1], remainder_x);
        dims_host_mirror_(2).local_physical_size =
            base_local_N_x + (coords[1] < remainder_x ? 1 : 0);
        dims_host_mirror_(2).local_physical_end_idx =
            dims_host_mirror_(2).local_physical_start_idx +
            dims_host_mirror_(2).local_physical_size - 1;
    }
...
Grid::print_info() const {
    ...
    if (cart_comm_ != MPI_COMM_NULL) {
        ...
        coords[0] = -1;
        coords[1] = -1; // Indicate invalid coords
        p_dims_retrieved[0] = 0;
        p_dims_retrieved[1] = 0;
        periods_retrieved[0] = 0;
        periods_retrieved[1] = 0;
    }

    std::cout << "MPI Rank " << mpi_rank_ << " (Coords: Y=" << coords[0] << ", X=" << coords[1]
              << ") Local Grid Info:" << std::endl;
    std::cout << "  Periodic (Y,X): (" << (periods_retrieved[0] ? "Yes" : "No") << ", "
              << (periods_retrieved[1] ? "Yes" : "No") << ")" << std::endl;
    std::cout << "  Z: Global Start=" << dims_host_mirror_(0).local_physical_start_idx
              ...
              << ", Total Size (incl. Halo)="
              << (dims_host_mirror_(0).local_physical_size +
                     2 * dims_host_mirror_(0).num_halo_cells)
              << std::endl;
    std::cout << "  Y: Global Start=" << dims_host_mirror_(1).local_physical_start_idx
              ...
              << ", Total Size (incl. Halo)="
              << (dims_host_mirror_(1).local_physical_size +
                     2 * dims_host_mirror_(1).num_halo_cells)
              << std::endl;
    std::cout << "  X: Global Start=" << dims_host_mirror_(2).local_physical_start_idx
              ...
              << ", Total Size (incl. Halo)="
              << (dims_host_mirror_(2).local_physical_size +
                     2 * dims_host_mirror_(2).num_halo_cells)
              << std::endl;
    std::cout << "------------------------------------" << std::endl;
}

void
Grid::initialize_horizontal_geometry() {
    ...
    geometry_ = Geometry::HorizontalGeometryFactory::create(grid_specification_.horizontal.geometry,
        layout);
