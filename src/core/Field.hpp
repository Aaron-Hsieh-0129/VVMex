...
struct FieldMetadata {
    ...
    FieldMetadata(GridStaggering grid_staggering_in,
        std::string units_in,
        std::string long_name_in,
        std::string standard_name_in = {},
        std::string comment_in = {})
        : grid_staggering(grid_staggering_in), units(std::move(units_in)),
          long_name(std::move(long_name_in)), standard_name(std::move(standard_name_in)),
...
struct ViewTypeHelper<0, ScalarType> {
    using type = Kokkos::View<ScalarType>;
};
template <typename ScalarType>
struct ViewTypeHelper<1, ScalarType> {
    using type = Kokkos::View<ScalarType*>;
};
template <typename ScalarType>
struct ViewTypeHelper<2, ScalarType> {
    using type = Kokkos::View<ScalarType**>;
};
template <typename ScalarType>
struct ViewTypeHelper<3, ScalarType> {
    using type = Kokkos::View<ScalarType***>;
};
template <typename ScalarType>
struct ViewTypeHelper<4, ScalarType> {
    using type = Kokkos::View<ScalarType****>;
};
...
public:
    ...
    using ViewType = std::conditional_t<std::is_void_v<Layout>,
        DefaultViewType,
    ...
    explicit Field(const std::string& field_name,
        ...
        if constexpr (Dim == 0) {
            data_ = ViewType(name_);
        }
        else if constexpr (Dim == 1) {
            data_ = ViewType(name_, dims[0]);
        }
        else if constexpr (Dim == 2) {
            data_ = ViewType(name_, dims[0], dims[1]);
        }
        else if constexpr (Dim == 3) {
            data_ = ViewType(name_, dims[0], dims[1], dims[2]);
        }
...
inline void
Field<Dim, Layout>::print_slice_z_at_k(
    const Grid& grid, int N_idx, int k_local_idx, int halo) const {
    int rank;
    ...
    if constexpr (Dim == 4) {
        if (N_idx < 0 || N_idx >= host_data.extent(0) || k_local_idx < 0 ||
            k_local_idx >= host_data.extent(1)) {
        ...
        if (k_local_idx < 0 || k_local_idx >= host_data.extent(0)) {
            std::cerr << "Warning: Z-slice index " << k_local_idx << " out of bounds for field '"
                      << name_ << "'." << std::endl;
            return;
        }
...
template <size_t Dim, typename Layout>
inline void
Field<Dim, Layout>::print_profile(
    const Grid& grid, int N_idx, int j_local_idx, int i_local_idx) const {
    int rank;
    MPI_Comm_rank(grid.get_comm(), &rank);
    ...
    // Use if constexpr to handle different dimensions at compile time
    if constexpr (Dim == 4) {
        std::cout << "  Slice at N=" << N_idx << ", j=" << j_local_idx << ", i=" << i_local_idx
                  << std::endl;
        ...
        for (int k = 0; k < host_data.extent(0); ++k) {
            std::cout << std::scientific << std::setprecision(6)
                      << host_data(k, j_local_idx, i_local_idx) << "\t";
        }
        std::cout << std::endl;
    }
    else if constexpr (Dim == 1) {
...
template <size_t Dim, typename Layout>
inline void
Field<Dim, Layout>::print_xz_cross_at_j(
    const Grid& grid, int N_idx, int j_local_idx, int halo) const {
    int rank;
    MPI_Comm_rank(grid.get_comm(), &rank);
    ...
    // Use if constexpr to handle different dimensions at compile time
    if constexpr (Dim == 4) {
        if (N_idx < 0 || N_idx >= host_data.extent(0) || j_local_idx < 0 ||
            j_local_idx >= host_data.extent(1)) {
        ...
        if (j_local_idx < 0 || j_local_idx >= host_data.extent(1)) {
            std::cerr << "Warning: Y-slice index " << j_local_idx << " out of bounds for field '"
                      << name_ << "'." << std::endl;
            return;
        }
