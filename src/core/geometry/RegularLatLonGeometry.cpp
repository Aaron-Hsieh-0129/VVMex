...
struct InitializeRegularLatLonMetrics {
    ...
    InitializeRegularLatLonMetrics(const Kokkos::View<const VVM::Real*>& latitude_in,
        ...
        : latitude(latitude_in), sqrt_g(sqrt_g_in), inv_sqrt_g(inv_sqrt_g_in),
          g_cov_11(g_cov_11_in), sqrt_g_g_contra_11(sqrt_g_g_contra_11_in),
          sqrt_g_g_contra_22(sqrt_g_g_contra_22_in),
          physical_to_contravariant_11(physical_to_contravariant_11_in),
          contravariant_to_physical_11(contravariant_to_physical_11_in), radius(radius_in) {}
...
bool
RegularLatLonGeometry::is_q1_staggered(const HorizontalLocation location) {
...
bool
RegularLatLonGeometry::is_q2_staggered(const HorizontalLocation location) {
...
const Kokkos::View<VVM::Real*>&
RegularLatLonGeometry::q1_for(const HorizontalLocation location) const {
    return is_q1_staggered(location) ? q1_staggered_ : q1_centered_;
}

const Kokkos::View<VVM::Real*>&
RegularLatLonGeometry::q2_for(const HorizontalLocation location) const {
    return is_q2_staggered(location) ? q2_staggered_ : q2_centered_;
}

const RegularLatLonGeometry::MetricStorage&
RegularLatLonGeometry::metrics_for(const HorizontalLocation location) const {
    return is_q2_staggered(location) ? staggered_q2_metrics_ : centered_q2_metrics_;
}

void
RegularLatLonGeometry::allocate_metric_storage(
    MetricStorage& storage, const std::string& label, const int size) {
    ...
    storage.physical_to_contravariant_11 =
        Kokkos::View<VVM::Real*>(label + "_physical_to_contravariant_11", size);
    storage.contravariant_to_physical_11 =
        Kokkos::View<VVM::Real*>(label + "_contravariant_to_physical_11", size);
}

void
RegularLatLonGeometry::validate() const {
    ...
    if (!std::isfinite(dlongitude_) || dlongitude_ <= VVM::real(0.0) ||
        ...
        throw std::invalid_argument(
            "RegularLatLonGeometry requires finite positive angular spacing.");
    }
    ...
    const VVM::Real minimum_halo_latitude =
        latitude_south_edge_ + (VVM::real(0.5) - static_cast<VVM::Real>(layout_.halo)) * dlatitude_;
    const VVM::Real maximum_halo_latitude =
        latitude_south_edge_ +
        static_cast<VVM::Real>(layout_.global_ny + layout_.halo) * dlatitude_;
...
void
RegularLatLonGeometry::initialize_coordinates() {
...
void
RegularLatLonGeometry::initialize_metrics() {
    const int ny = layout_.local_total_ny();
    ...
    Kokkos::parallel_for("InitializeRegularLatLonCenteredQ2Metrics",
        Kokkos::RangePolicy<>(0, ny),
        InitializeRegularLatLonMetrics(q2_centered_,
            centered_q2_metrics_.sqrt_g,
            centered_q2_metrics_.inv_sqrt_g,
            centered_q2_metrics_.g_cov_11,
            centered_q2_metrics_.sqrt_g_g_contra_11,
            centered_q2_metrics_.sqrt_g_g_contra_22,
            centered_q2_metrics_.physical_to_contravariant_11,
            centered_q2_metrics_.contravariant_to_physical_11,
            radius_));

    Kokkos::parallel_for("InitializeRegularLatLonStaggeredQ2Metrics",
        Kokkos::RangePolicy<>(0, ny),
        InitializeRegularLatLonMetrics(q2_staggered_,
            staggered_q2_metrics_.sqrt_g,
            staggered_q2_metrics_.inv_sqrt_g,
            staggered_q2_metrics_.g_cov_11,
            staggered_q2_metrics_.sqrt_g_g_contra_11,
            staggered_q2_metrics_.sqrt_g_g_contra_22,
            staggered_q2_metrics_.physical_to_contravariant_11,
            staggered_q2_metrics_.contravariant_to_physical_11,
            radius_));
}
...
RegularLatLonGeometry::device_view_impl(const HorizontalLocation location) const {
    ...
    result.physical_to_contravariant.a11 =
        GeometryField2D::varying_j(metrics.physical_to_contravariant_11);
    ...
    result.contravariant_to_physical.a11 =
        GeometryField2D::varying_j(metrics.contravariant_to_physical_11);
