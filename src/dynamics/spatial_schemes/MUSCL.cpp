...
MUSCL::Options
MUSCL::parse_options(const std::string& variable_name, const nlohmann::json& advection_config) {
    Options result;
    nlohmann::json options = nlohmann::json::object();
    if (advection_config.contains("scheme_options")) {
        options = advection_config.at("scheme_options");
        if (!options.is_object()) {
            throw std::runtime_error("Advected variable '" + variable_name +
                                     "': MUSCL scheme_options must be an object.");
        }
    }

    if (options.contains("limiter") && !options.at("limiter").is_string()) {
        throw std::runtime_error(
            "Advected variable '" + variable_name + "': MUSCL option 'limiter' must be a string.");
    }
    const std::string limiter = options.value("limiter", "vanLeer");
    if (limiter != "vanLeer") {
        throw std::runtime_error("Advected variable '" + variable_name +
                                 "': unknown MUSCL limiter '" + limiter +
                                 "'; supported limiter: vanLeer.");
    }
    ...
    auto read_finite_number = [&](const char* key, VVM::Real default_value) {
        ...
        if (!options.at(key).is_number()) {
            throw std::runtime_error("Advected variable '" + variable_name + "': MUSCL option '" +
    ...
    result.lower_bound = read_finite_number("lower_bound", VVM::real(0.0));
    result.max_cfl = read_finite_number("max_cfl", VVM::real(0.9));

    if (result.lower_bound < VVM::real(0.0)) {
        throw std::runtime_error(
            "Advected variable '" + variable_name + "': MUSCL lower_bound must be nonnegative.");
    }
    if (result.max_cfl <= VVM::real(0.0)) {
        throw std::runtime_error(
            "Advected variable '" + variable_name + "': MUSCL max_cfl must be greater than zero.");
    }
    return result;
}

MUSCL::Options
MUSCL::validate_configuration(const std::string& variable_name,
    ...
    if (spatial_scheme != "MUSCL" || temporal_scheme != "SSPRK2") {
        throw std::runtime_error("Advected variable '" + variable_name +
                                 "' requested spatial scheme '" + spatial_scheme +
                                 "' and temporal scheme '" + temporal_scheme +
                                 "'; supported pairing: spatial scheme "
    ...
    if (configured_halo_width < required_halo_width) {
        ...
        message << "Advected variable '" << variable_name
                ...
                << configured_halo_width << ", required " << required_halo_width << ".";
...
MUSCL::MUSCL(
    ...
    : variable_name_(std::move(variable_name)),
      options_(parse_options(variable_name_, advection_config)), grid_(grid),
    ...
    if (grid.get_halo_cells() < required_halo_width) {
        ...
        message << "Advected variable '" << variable_name_
                << "': MUSCL scalar halo width is insufficient; configured "
                << grid.get_halo_cells() << ", required " << required_halo_width << ".";
        throw std::runtime_error(message.str());
    }
}

void
MUSCL::validate_initial_field(const Core::State& state, const Core::Field<3>& scalar) const {
    if (initial_field_validated_) {
        return;
    }
    ...
    using DeviceMemorySpace = Core::Field<0>::ViewType::memory_space;
    auto local_reduction = reduction_local_.get_mutable_device_data();
    auto global_reduction = reduction_global_.get_mutable_device_data();

    Kokkos::parallel_reduce("MUSCL_initial_min_" + variable_name_,
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({h, h, h}, {nz - h, ny - h, nx - h}),
        KOKKOS_LAMBDA(const int k, const int j, const int i, VVM::Real& update) {
            if (fluid(k, j, i) == VVM::real(1.0) && q(k, j, i) < update) {
                update = q(k, j, i);
            }
        },
    ...
    const ncclResult_t min_result = ncclAllReduce(local_reduction.data(),
        global_reduction.data(),
        1,
        VVM_NCCL_REAL,
        ncclMin,
        state.get_nccl_comm(),
    ...
    if (min_result != ncclSuccess) {
        throw std::runtime_error("MUSCL NCCL minimum reduction failed for '" + variable_name_ +
                                 "': " + ncclGetErrorString(min_result));
    }
    const cudaError_t min_sync = cudaStreamSynchronize(state.get_cuda_stream());
    if (min_sync != cudaSuccess) {
        throw std::runtime_error("MUSCL NCCL minimum synchronization failed for '" +
    ...
    MPI_Allreduce(&local_min, &global_min, 1, VVM_MPI_REAL, MPI_MIN, grid_.get_comm());
#endif

    const VVM::Real largest = std::numeric_limits<VVM::Real>::max();
#if defined(ENABLE_NCCL)
    Kokkos::parallel_reduce("MUSCL_initial_finite_" + variable_name_,
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({h, h, h}, {nz - h, ny - h, nx - h}),
        KOKKOS_LAMBDA(const int k, const int j, const int i, VVM::Real& update) {
            const VVM::Real flag =
                fluid(k, j, i) == VVM::real(1.0) &&
                        (q(k, j, i) != q(k, j, i) || Kokkos::abs(q(k, j, i)) > largest)
                    ? VVM::real(1.0)
                    : VVM::real(0.0);
            if (flag > update) {
                update = flag;
    ...
    const ncclResult_t finite_result = ncclAllReduce(local_reduction.data(),
        global_reduction.data(),
        1,
        VVM_NCCL_REAL,
        ncclMax,
    ...
    if (finite_result != ncclSuccess) {
        throw std::runtime_error("MUSCL NCCL finite-value reduction failed for '" + variable_name_ +
                                 "': " + ncclGetErrorString(finite_result));
    }
    const cudaError_t finite_sync = cudaStreamSynchronize(state.get_cuda_stream());
    if (finite_sync != cudaSuccess) {
        throw std::runtime_error("MUSCL NCCL finite-value synchronization failed for '" +
                                 variable_name_ + "': " + cudaGetErrorString(finite_sync));
    }
    ...
    Kokkos::parallel_reduce("MUSCL_initial_finite_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i, int& update) {
            const int flag = fluid(k, j, i) == VVM::real(1.0) &&
                                     (q(k, j, i) != q(k, j, i) || Kokkos::abs(q(k, j, i)) > largest)
                                 ? 1
                                 : 0;
            if (flag > update) {
    ...
    MPI_Allreduce(&local_nonfinite, &global_nonfinite, 1, MPI_INT, MPI_MAX, grid_.get_comm());
#endif
    ...
    if (violates_lower_bound(global_min, options_.lower_bound)) {
        std::ostringstream message;
        message << "Advected variable '" << variable_name_ << "' has initial/restart minimum "
                << global_min << " below configured MUSCL lower_bound " << options_.lower_bound
...
void
MUSCL::validate_cfl(const Core::State& state,
    ...
    Kokkos::parallel_reduce("MUSCL_outgoing_cfl_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i, VVM::Real& update) {
            ...
            if (face_x(k, j, i) == VVM::real(1.0) && fluid(k, j, i + 1) == VVM::real(1.0) &&
                mx(k, j, i) > VVM::real(0.0)) {
                outgoing += mx(k, j, i) * rdx();
            }
            if (face_x(k, j, i - 1) == VVM::real(1.0) && fluid(k, j, i - 1) == VVM::real(1.0) &&
                mx(k, j, i - 1) < VVM::real(0.0)) {
                outgoing -= mx(k, j, i - 1) * rdx();
            }
            if (face_y(k, j, i) == VVM::real(1.0) && fluid(k, j + 1, i) == VVM::real(1.0) &&
                my(k, j, i) > VVM::real(0.0)) {
                outgoing += my(k, j, i) * rdy();
            }
            if (face_y(k, j - 1, i) == VVM::real(1.0) && fluid(k, j - 1, i) == VVM::real(1.0) &&
                my(k, j - 1, i) < VVM::real(0.0)) {
                outgoing -= my(k, j - 1, i) * rdy();
            }
            if (k < nz - h - 1 && fluid(k + 1, j, i) == VVM::real(1.0) &&
                mz(k, j, i) > VVM::real(0.0)) {
                outgoing += mz(k, j, i) * rdz() * flex(k);
            }
            if (k > h && fluid(k - 1, j, i) == VVM::real(1.0) && mz(k - 1, j, i) < VVM::real(0.0)) {
    ...
    const ncclResult_t cfl_result = ncclAllReduce(local_reduction.data(),
        global_reduction.data(),
        1,
        VVM_NCCL_REAL,
        ncclMax,
    ...
    if (cfl_result != ncclSuccess) {
        throw std::runtime_error("MUSCL NCCL CFL reduction failed for '" + variable_name_ +
                                 "': " + ncclGetErrorString(cfl_result));
    }
    const cudaError_t cfl_sync = cudaStreamSynchronize(state.get_cuda_stream());
    if (cfl_sync != cudaSuccess) {
        throw std::runtime_error("MUSCL NCCL CFL synchronization failed for '" + variable_name_ +
                                 "': " + cudaGetErrorString(cfl_sync));
    }
    ...
    MPI_Allreduce(&local_max, &global_max, 1, VVM_MPI_REAL, MPI_MAX, grid_.get_comm());
#endif

    if (global_max > options_.max_cfl) {
        std::ostringstream message;
        message << "Advected variable '" << variable_name_ << "' MUSCL outgoing CFL " << global_max
                << " exceeds configured max_cfl " << options_.max_cfl << " for model timestep "
                << stage_dt << ".";
        throw std::runtime_error(message.str());
    }
}
...
void
MUSCL::calculate_advection_tendency(const Core::State& state,
    ...
    if (var_name != variable_name_) {
        throw std::runtime_error("MUSCL instance for advected variable '" + variable_name_ +
    ...
    Kokkos::parallel_for("MUSCL_reconstruct_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            if (fluid(k, j, i - 1) == VVM::real(1.0) && fluid(k, j, i + 1) == VVM::real(1.0)) {
                slope_x = van_leer_slope(q(k, j, i) - q(k, j, i - 1), q(k, j, i + 1) - q(k, j, i));
            }
            if (fluid(k, j - 1, i) == VVM::real(1.0) && fluid(k, j + 1, i) == VVM::real(1.0)) {
                slope_y = van_leer_slope(q(k, j, i) - q(k, j - 1, i), q(k, j + 1, i) - q(k, j, i));
            }
            if (fluid(k - 1, j, i) == VVM::real(1.0) && fluid(k + 1, j, i) == VVM::real(1.0)) {
                const VVM::Real backward_spacing = z_mid(k) - z_mid(k - 1);
                const VVM::Real forward_spacing = z_mid(k + 1) - z_mid(k);
                if (backward_spacing > VVM::real(0.0) && forward_spacing > VVM::real(0.0)) {
                    slope_z = van_leer_slope((q(k, j, i) - q(k - 1, j, i)) / backward_spacing,
                        (q(k + 1, j, i) - q(k, j, i)) / forward_spacing);
                }
            }

            const VVM::Real lower_z_deviation = slope_z * (z_up(k - 1) - z_mid(k));
            const VVM::Real upper_z_deviation = slope_z * (z_up(k) - z_mid(k));
            const VVM::Real half = VVM::real(0.5);
            const VVM::Real theta = reconstructed_state_factor_from_deviations(q(k, j, i),
                lower_bound,
                -half * slope_x,
                half * slope_x,
                -half * slope_y,
    ...
    Kokkos::parallel_for("MUSCL_donor_factors_" + variable_name_,
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({h, h - 1, h - 1}, {nz - h, ny - h + 1, nx - h + 1}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            if (fluid(k, j, i) != VVM::real(1.0)) {
            ...
            if (k < nz - h - 1 && fluid(k + 1, j, i) == VVM::real(1.0) &&
                mz(k, j, i) > VVM::real(0.0)) {
                outgoing_rate += mz(k, j, i) * rzp * rdz() * flex(k);
            }
            if (k > h && fluid(k - 1, j, i) == VVM::real(1.0) && mz(k - 1, j, i) < VVM::real(0.0)) {
                outgoing_rate -= mz(k - 1, j, i) * rzm * rdz() * flex(k);
            }

            alpha(k, j, i) = donor_outflow_factor(excess, stage_dt * outgoing_rate / rho(k));
        });
    ...
    Kokkos::parallel_for("MUSCL_final_flux_x_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            const bool open = face_x(k, j, i) == VVM::real(1.0) &&
                              fluid(k, j, i) == VVM::real(1.0) &&
            ...
            const VVM::Real sign = mass >= VVM::real(0.0) ? VVM::real(0.5) : VVM::real(-0.5);
            const VVM::Real reconstructed_excess =
                q(k, j, donor_i) - lower_bound + sign * sx(k, j, donor_i);
            fx(k, j, i) = mass * (lower_bound + alpha(k, j, donor_i) * reconstructed_excess);
        });

    Kokkos::parallel_for("MUSCL_final_flux_y_" + variable_name_,
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({h, h - 1, h}, {nz - h, ny - h, nx - h}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            const bool open = face_y(k, j, i) == VVM::real(1.0) &&
                              fluid(k, j, i) == VVM::real(1.0) &&
            ...
            const VVM::Real sign = mass >= VVM::real(0.0) ? VVM::real(0.5) : VVM::real(-0.5);
            const VVM::Real reconstructed_excess =
                q(k, donor_j, i) - lower_bound + sign * sy(k, donor_j, i);
            fy(k, j, i) = mass * (lower_bound + alpha(k, donor_j, i) * reconstructed_excess);
        });
    ...
    Kokkos::parallel_for("MUSCL_final_flux_z_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            const bool open =
                fluid(k, j, i) == VVM::real(1.0) && fluid(k + 1, j, i) == VVM::real(1.0);
            if (!open) {
            ...
            const VVM::Real reconstructed_excess =
                q(donor_k, j, i) - lower_bound + sz(donor_k, j, i) * (z_up(k) - z_mid(donor_k));
            fz(k, j, i) = mass * (lower_bound + alpha(donor_k, j, i) * reconstructed_excess);
        });
    ...
    Kokkos::parallel_for("MUSCL_flux_divergence_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            tendency(k, j, i) += -(fx(k, j, i) - fx(k, j, i - 1)) * rdx() -
                                 (fy(k, j, i) - fy(k, j - 1, i)) * rdy() -
                                 (fz(k, j, i) - fz(k - 1, j, i)) * rdz() * flex(k);
        });
}
