...
WENO5::Options
WENO5::parse_options(const std::string& variable_name, const nlohmann::json& advection_config) {
    Options result;
    ...
    if (!options.is_object()) {
        throw std::runtime_error(
            "Tracer '" + variable_name + "': weno5 scheme_options must be an object.");
    }
    for (const auto& item : options.items()) {
        if (item.key() != "epsilon") {
            throw std::runtime_error("Tracer '" + variable_name + "': unknown weno5 option '" +
    ...
    if (!options.at("epsilon").is_number()) {
        throw std::runtime_error(
            "Tracer '" + variable_name + "': weno5 option 'epsilon' must be numeric.");
    }
    result.epsilon = options.at("epsilon").get<VVM::Real>();
    if (!std::isfinite(result.epsilon) || result.epsilon <= VVM::real(0.0)) {
        throw std::runtime_error(
            "Tracer '" + variable_name + "': weno5 epsilon must be finite and greater than zero.");
    }
    return result;
}

WENO5::Options
WENO5::validate_configuration(const std::string& variable_name,
    ...
    if (spatial_scheme != "weno5" || temporal_scheme != "SSPRK2") {
        throw std::runtime_error("Tracer '" + variable_name + "' requested spatial scheme '" +
    ...
    if (configured_halo_width < required_halo_width) {
        ...
        message << "Tracer '" << variable_name << "': weno5 halo width is insufficient; configured "
...
WENO5::WENO5(std::string variable_name,
    const nlohmann::json& advection_config,
    ...
    if (grid.get_halo_cells() < required_halo_width) {
        ...
        message << "Tracer '" << variable_name_
                << "': weno5 halo width is insufficient; configured " << grid.get_halo_cells()
                << ", required " << required_halo_width << ".";
        throw std::runtime_error(message.str());
    }
    if (grid.get_global_points_x() > 1 &&
        grid.get_local_physical_points_x() < required_halo_width) {
        throw std::runtime_error("Tracer '" + variable_name_ +
...
void
WENO5::calculate_advection_tendency(const Core::State& state,
    ...
    if (var_name != variable_name_) {
        throw std::runtime_error("weno5 instance for tracer '" + variable_name_ +
    ...
    Kokkos::parallel_for("WENO5_flux_divergence_x_" + variable_name_,
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            auto face_flux = [&](const int f) {
                const bool open = face_x(k, j, f) == VVM::real(1.0) &&
                ...
                if (mass >= VVM::real(0.0)) {
                    const bool complete = fluid(k, j, f - 2) == VVM::real(1.0) &&
                                          fluid(k, j, f - 1) == VVM::real(1.0) &&
                    ...
                    return mass * WENO5::reconstruct_left(q_im2, q_im1, q_i, q_ip1, q_ip2, epsilon);
                }

                const bool complete = fluid(k, j, f - 1) == VVM::real(1.0) &&
                                      fluid(k, j, f + 2) == VVM::real(1.0) &&
                                      fluid(k, j, f + 3) == VVM::real(1.0);
                if (!complete) {
                    return mass * q(k, j, f + 1);
                }
                ...
                return mass * WENO5::reconstruct_right(q_im1, q_i, q_ip1, q_ip2, q_ip3, epsilon);
            };

            const VVM::Real flux_right = face_flux(i);
            const VVM::Real flux_left = face_flux(i - 1);
            tendency(k, j, i) += -(flux_right - flux_left) * rdx();
        });

    // v(k,j,i) is located between centered tracer cells j and j+1.
    Kokkos::parallel_for("WENO5_flux_divergence_y_" + variable_name_,
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({h, h, h}, {nz - h, ny - h, nx - h}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            if (fluid(k, j, i) != VVM::real(1.0)) {
                return;
            ...
            auto face_flux = [&](const int f) {
                const bool open = face_y(k, f, i) == VVM::real(1.0) &&
                ...
                if (mass >= VVM::real(0.0)) {
                    const bool complete = fluid(k, f - 2, i) == VVM::real(1.0) &&
                                          fluid(k, f - 1, i) == VVM::real(1.0) &&
                    ...
                    return mass * WENO5::reconstruct_left(q_jm2, q_jm1, q_j, q_jp1, q_jp2, epsilon);
                }

                const bool complete = fluid(k, f - 1, i) == VVM::real(1.0) &&
                                      fluid(k, f + 2, i) == VVM::real(1.0) &&
                                      fluid(k, f + 3, i) == VVM::real(1.0);
                if (!complete) {
                    return mass * q(k, f + 1, i);
                }
                ...
                return mass * WENO5::reconstruct_right(q_jm1, q_j, q_jp1, q_jp2, q_jp3, epsilon);
            };

            const VVM::Real flux_top = face_flux(j);
            const VVM::Real flux_bottom = face_flux(j - 1);
            tendency(k, j, i) += -(flux_top - flux_bottom) * rdy();
        });
