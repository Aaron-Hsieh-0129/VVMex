...
Takacs::Takacs(const Utils::ConfigurationManager& config,
...
void
Takacs::calculate_flux_convergence_x(const Core::Field<3>& scalar,
    const Core::Field<3>& u_field,
    const Core::Grid& grid,
    ...
    Kokkos::parallel_for("Takacs_flux_convergence_x_fused",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, k_start, k_end), [&](const int k) {
                ...
                VVM::Real flux_i =
                    u(k, j, i) * (q(k, j, i + 1) + q(k, j, i)) +
                    -real(1.) / real(3.) *
                        (up_i * (q(k, j, i + 1) - q(k, j, i)) -
                            Kokkos::sqrt(up_i) * Kokkos::sqrt(up_im1) *
                                (q(k, j, i) - q(k, j, i - 1)) -
                            um_i * (q(k, j, i + 1) - q(k, j, i)) -
                            Kokkos::sqrt(Kokkos::abs(um_i)) * Kokkos::sqrt(Kokkos::abs(um_ip1)) *
                                (q(k, j, i + 2) - q(k, j, i + 1)));
                ...
                VVM::Real flux_im1 =
                    u(k, j, i - 1) * (q(k, j, i) + q(k, j, i - 1)) +
                    -real(1.) / real(3.) *
                        (up_im1 * (q(k, j, i) - q(k, j, i - 1)) -
                            Kokkos::sqrt(up_im1) * Kokkos::sqrt(up_im2) *
                                (q(k, j, i - 1) - q(k, j, i - 2)) -
                            um_im1 * (q(k, j, i) - q(k, j, i - 1)) -
                            Kokkos::sqrt(Kokkos::abs(um_im1)) * Kokkos::sqrt(Kokkos::abs(um_i)) *
                                (q(k, j, i + 1) - q(k, j, i)));
...
void
Takacs::calculate_flux_convergence_y(const Core::Field<3>& scalar,
    const Core::Field<3>& v_field,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
    ...
    Kokkos::parallel_for("Takacs_flux_convergence_y_fused",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, k_start, k_end), [&](const int k) {
                ...
                VVM::Real flux_j =
                    v(k, j, i) * (q(k, j + 1, i) + q(k, j, i)) +
                    -real(1.) / real(3.) *
                        (vp_j * (q(k, j + 1, i) - q(k, j, i)) -
                            Kokkos::sqrt(vp_j) * Kokkos::sqrt(vp_jm1) *
                                (q(k, j, i) - q(k, j - 1, i)) -
                            vm_j * (q(k, j + 1, i) - q(k, j, i)) -
                            Kokkos::sqrt(Kokkos::abs(vm_j)) * Kokkos::sqrt(Kokkos::abs(vm_jp1)) *
                                (q(k, j + 2, i) - q(k, j + 1, i)));
                ...
                VVM::Real flux_jm1 =
                    v(k, j - 1, i) * (q(k, j, i) + q(k, j - 1, i)) +
                    -real(1.) / real(3.) *
                        (vp_jm1 * (q(k, j, i) - q(k, j - 1, i)) -
                            Kokkos::sqrt(vp_jm1) * Kokkos::sqrt(vp_jm2) *
                                (q(k, j - 1, i) - q(k, j - 2, i)) -
                            vm_jm1 * (q(k, j, i) - q(k, j - 1, i)) -
                            Kokkos::sqrt(Kokkos::abs(vm_jm1)) * Kokkos::sqrt(Kokkos::abs(vm_j)) *
                                (q(k, j + 1, i) - q(k, j, i)));
...
void
Takacs::calculate_flux_convergence_z(const Core::Field<3>& scalar,
    const Core::Field<3>& w_field,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
    ...
    const bool is_idealized =
        (config_.get_value<std::string>("simulation.idealized_test", "none") == "advection_w");
    ...
    Kokkos::parallel_for("Takacs_flux_convergence_z_fused",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            auto get_wplus = [&](int idx_k) {
                return real(0.5) * (w(idx_k, j, i) + Kokkos::abs(w(idx_k, j, i)));
            };
            auto get_wminus = [&](int idx_k) {
                return real(0.5) * (w(idx_k, j, i) - Kokkos::abs(w(idx_k, j, i)));
            };
            ...
            auto compute_raw_f = [&](int k_raw) -> VVM::Real {
                ...
                if (k_raw == h) {
                    if (w(k_raw, j, i) < 0.) {
                        ...
                        raw_f += -real(1.) / real(3.) *
                                 (-wm_k * (q(k_raw + 1, j, i) - q(k_raw, j, i)) -
                                     Kokkos::sqrt(Kokkos::abs(wm_k)) *
                                         Kokkos::sqrt(Kokkos::abs(wm_kp1)) *
                                         (q(k_raw + 2, j, i) - q(k_raw + 1, j, i)));
                    }
                }
                    ...
                    if (w(k_raw, j, i) >= 0.) {
                        ...
                        raw_f += -real(1.) / real(3.) *
                                 (wp_k * (q(k_raw + 1, j, i) - q(k_raw, j, i)) -
                                     Kokkos::sqrt(wp_k) * Kokkos::sqrt(wp_km1) *
                                         (q(k_raw, j, i) - q(k_raw - 1, j, i)));
                    }
                }
                else {
                    VVM::Real wp_k = get_wplus(k_raw);
                    ...
                    raw_f +=
                        ...
                        (wp_k * (q(k_raw + 1, j, i) - q(k_raw, j, i)) -
                            Kokkos::sqrt(wp_k) * Kokkos::sqrt(wp_km1) *
                                (q(k_raw, j, i) - q(k_raw - 1, j, i)) -
                            wm_k * (q(k_raw + 1, j, i) - q(k_raw, j, i)) -
                            Kokkos::sqrt(Kokkos::abs(wm_k)) * Kokkos::sqrt(Kokkos::abs(wm_kp1)) *
                                (q(k_raw + 2, j, i) - q(k_raw + 1, j, i)));
                }
                return raw_f;
            };
                ...
                if (is_zeta) {
                    if (k_idx == nz - h - 2) {
                        ...
                        if (w(k_idx, j, i) >= real(0.)) {
                            ...
                            f += -real(1.) / real(3.) *
                                 (wp_k * (q(k_idx + 1, j, i) - q(k_idx, j, i)) -
                                     Kokkos::sqrt(wp_k) * Kokkos::sqrt(wp_km1) *
                                         (q(k_idx, j, i) - q(k_idx - 1, j, i)));
                        }
                        return f;
                    }
                    return real(0.);
                    ...
                    if (k_idx >= h - 1 && k_idx <= nz - h - 2) {
                        ...
                        if (k_idx == h - 1) {
                            if (w(k_idx, j, i) < real(0.)) {
                                ...
                                f += -real(1.) / real(3.) *
                                     (-wm_k * (q(k_idx + 1, j, i) - q(k_idx, j, i)) -
                                         Kokkos::sqrt(Kokkos::abs(wm_k)) *
                                             Kokkos::sqrt(Kokkos::abs(wm_kp1)) *
                                             (q(k_idx + 2, j, i) - q(k_idx + 1, j, i)));
                            }
                        }
                    ...
                    if (k_idx == h - 1) {
                        return is_idealized ? compute_raw_f(nz - h - 2) : real(0.);
                    }
                    if (k_idx == nz - h - 1) {
                        return is_idealized ? compute_raw_f(h) : real(0.);
                    }
                    if (k_idx >= h && k_idx <= nz - h - 2) {
                        return compute_raw_f(k_idx);
                    }
                    return real(0.);
                }
            };
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, tend_k_min, tend_k_max + 1),
                ...
                if (is_zeta) {
                    ...
                    tendency(k, j, i) +=
                        -0.5 * (flux_k - flux_km1) * rdz_view() * flex_height_coef_up(k);
                }
                else {
                    tendency(k, j, i) +=
                        -0.5 * (flux_k - flux_km1) * rdz_view() * flex_height_coef_mid(k);
                }
            });
        });
    return;
...
void
Takacs::calculate_stretching_tendency_x(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
    ...
    Kokkos::parallel_for("stretching_term_xi_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, h, nz - h - 1), [&](const int k) {
                const VVM::Real term_at_j_plus_1 =
                    (xi(k, j, i) + xi(k, j + 1, i)) *
                    (fact1_xi_eta(k) * rhobar(k + 1) *
                            (u(k + 1, j + 1, i) - u(k + 1, j + 1, i - 1)) +
...
void
Takacs::calculate_stretching_tendency_y(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
...
void
Takacs::calculate_stretching_tendency_z(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
    ...
    Kokkos::parallel_for("stretching_term_zeta_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, nz - h - 1, nz - h),
                [&](const int k) {
                const VVM::Real term_at_i =
                    (zeta(k, j, i - 1) + zeta(k, j, i)) * (w(k - 1, j, i) + w(k - 1, j + 1, i));
                const VVM::Real term_at_i_plus_1 = (zeta(k, j, i) + zeta(k, j, i + 1)) *
                                                   (w(k - 1, j, i + 1) + w(k - 1, j + 1, i + 1));

                tendency(k, j, i) += -real(0.125) * flex_height_coef_mid(k) * rdz() * rhobar(k) *
                                     (term_at_i + term_at_i_plus_1);
            });
        });
    return;
}
...
void
Takacs::calculate_R_xi(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    ...
    Kokkos::parallel_for("compute_R_xi_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, 0, nz - h), [&](const int k) {
...
void
Takacs::calculate_R_eta(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
...
void
Takacs::calculate_R_zeta(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    ...
    Kokkos::parallel_for("compute_R_zeta_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, 0, nz), [&](const int k) {
                R_zeta(k, j, i) =
                    (v(k, j, i + 1) - v(k, j, i)) * rdx() + (u(k, j + 1, i) - u(k, j, i)) * rdy();
            });
        });
...
void
Takacs::calculate_twisting_tendency_x(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, h, nz - h), [&](const int k) {
                ...
                const VVM::Real term_etaRzeta =
                    ((eta(k, j + 1, i) + eta(k, j, i)) *
                            (fact2_xi_eta(k) * rhobar(k) * R_zeta(k, j, i) +
                                fact1_xi_eta(k) * rhobar(k + 1) * R_zeta(k + 1, j, i)) +
                        (eta(k, j + 1, i - 1) + eta(k, j, i - 1)) *
                            (fact2_xi_eta(k) * rhobar(k) * R_zeta(k, j, i - 1) +
                                fact1_xi_eta(k) * rhobar(k + 1) * R_zeta(k + 1, j, i - 1)));

                const VVM::Real term_zetaReta =
                    (rhobar_up(k) * (zeta(k, j, i) + zeta(k + 1, j, i)) *
                            (R_eta(k, j + 1, i) + R_eta(k, j, i)) +
                        rhobar_up(k) * (zeta(k, j, i - 1) + zeta(k + 1, j, i - 1)) *
                            (R_eta(k, j + 1, i - 1) + R_eta(k, j, i - 1)));
...
void
Takacs::calculate_twisting_tendency_y(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, h, nz - h), [&](const int k) {
                const VVM::Real term_xiRzeta =
                    ((xi(k, j, i + 1) + xi(k, j, i)) *
                            (fact2_xi_eta(k) * rhobar(k) * R_zeta(k, j, i) +
                                fact1_xi_eta(k) * rhobar(k + 1) * R_zeta(k + 1, j, i)) +
                        (xi(k, j - 1, i + 1) + xi(k, j - 1, i)) *
                            (fact2_xi_eta(k) * rhobar(k) * R_zeta(k, j - 1, i) +
                                fact1_xi_eta(k) * rhobar(k + 1) * R_zeta(k + 1, j - 1, i)));

                const VVM::Real term_zetaRxi =
                    (rhobar_up(k) * (zeta(k, j, i) + zeta(k + 1, j, i)) *
                            (R_xi(k, j, i) + R_xi(k, j, i + 1)) +
                        rhobar_up(k) * (zeta(k, j - 1, i) + zeta(k + 1, j - 1, i)) *
                            (R_xi(k, j - 1, i) + R_xi(k, j - 1, i + 1)));
...
void
Takacs::calculate_twisting_tendency_z(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency,
    const std::string& var_name) const {
    ...
    Kokkos::parallel_for("twisting_term_zeta_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, nz - h - 1, nz - h),
                [&](const int k) {
                // WARNING: The documentation has rho weighted but the VVM code doesn't have. This code follows the VVM code.
                const VVM::Real term_xiReta = (fact1() * (xi(k, j, i + 1) + xi(k, j, i)) *
                                                   (R_eta(k, j + 1, i) + R_eta(k, j, i)) +
                                               fact2() * (xi(k - 1, j, i) + xi(k - 1, j, i + 1)) *
                                                   (R_eta(k - 1, j + 1, i) + R_eta(k - 1, j, i)));

                const VVM::Real term_etaRxi = (fact1() * (eta(k, j + 1, i) + eta(k, j, i)) *
                                                   (R_xi(k, j, i) + R_xi(k, j, i + 1)) +
                                               fact2() * (eta(k - 1, j + 1, i) + eta(k - 1, j, i)) *
                                                   (R_xi(k - 1, j, i) + R_xi(k - 1, j, i + 1)));
...
void
Takacs::calculate_vorticity_divergence(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
...
void
Takacs::calculate_buoyancy_tendency_x(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    ...
    Kokkos::parallel_for("buoyancy_tendency_x_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, h, nz - h - 1), [&](const int k) {
                const VVM::Real dB_dy =
                    (th(k, j + 1, i) - th(k, j, i)) / thbar(k) +
                    (th(k + 1, j + 1, i) - th(k + 1, j, i)) / thbar(k + 1) +
                    real(0.608) *
                        (qv(k, j + 1, i) - qv(k, j, i) + qv(k + 1, j + 1, i) - qv(k + 1, j, i)) -
                    qp_coeff *
...
void
Takacs::calculate_buoyancy_tendency_y(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency) const {
    ...
    Kokkos::parallel_for("buoyancy_tendency_y_team",
        ...
        KOKKOS_LAMBDA(const MemberType& team) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team, h, nz - h - 1), [&](const int k) {
                const VVM::Real dB_dx =
                    (th(k, j, i + 1) - th(k, j, i)) / thbar(k) +
                    (th(k + 1, j, i + 1) - th(k + 1, j, i)) / thbar(k + 1) +
                    (real(0.608) *
                        (qv(k, j, i + 1) - qv(k, j, i) + qv(k + 1, j, i + 1) - qv(k + 1, j, i))) -
                    qp_coeff *
...
void
Takacs::calculate_coriolis_tendency_x(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency) const {
    ...
    Kokkos::parallel_for("coriolis_tendency_x",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            tendency(k, j, i) += real(0.25) * f_2d(j, i) * flex_height_coef_up(k) * rdz() *
                                 (u(k + 1, j, i - 1) - u(k, j, i - 1) + u(k + 1, j, i) -
                                     u(k, j, i) + u(k + 1, j + 1, i) - u(k, j + 1, i) +
...
void
Takacs::calculate_coriolis_tendency_y(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency) const {
    ...
    Kokkos::parallel_for("coriolis_tendency_y",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            tendency(k, j, i) += -real(0.25) * f_2d(j, i) * flex_height_coef_up(k) * rdz() *
                                 (v(k + 1, j, i + 1) - v(k, j, i + 1) + v(k + 1, j, i) -
                                     v(k, j, i) + v(k + 1, j - 1, i) - v(k, j - 1, i) +
...
void
Takacs::calculate_coriolis_tendency_z(const Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency) const {
    ...
    Kokkos::parallel_for("coriolis_tendency_z",
        ...
        KOKKOS_LAMBDA(const int j, const int i) {
            tendency(NK2, j, i) += -real(0.25) * f_2d(j, i) *
                                       (u(NK2, j, i + 1) - u(NK2, j, i - 1) + u(NK2, j + 1, i + 1) -
                                           u(NK2, j + 1, i - 1)) *
                                       rdx() -
                                   real(0.25) * f_2d(j, i) *
                                       (v(NK2, j + 1, i) - v(NK2, j - 1, i) + v(NK2, j + 1, i + 1) -
                                           v(NK2, j - 1, i + 1)) *
                                       rdy();
        });
    return;
...
void
Takacs::calculate_advection_tendency(const Core::State& state,
                                       ...
                                       -real(1.) / real(3.) *
                                           (up_i * (q(k, j, i + 1) - q(k, j, i)) -
                                               Kokkos::sqrt(up_i) * Kokkos::sqrt(up_im1) *
                                                   (q(k, j, i) - q(k, j, i - 1)) -
                                               um_i * (q(k, j, i + 1) - q(k, j, i)) -
                                               Kokkos::sqrt(Kokkos::abs(um_i)) *
                                                   Kokkos::sqrt(Kokkos::abs(um_ip1)) *
                                                   (q(k, j, i + 2) - q(k, j, i + 1)));
                                       ...
                                       -real(1.) / real(3.) *
                                           (vp_j * (q(k, j + 1, i) - q(k, j, i)) -
                                               Kokkos::sqrt(vp_j) * Kokkos::sqrt(vp_jm1) *
                                                   (q(k, j, i) - q(k, j - 1, i)) -
                                               vm_j * (q(k, j + 1, i) - q(k, j, i)) -
                                               Kokkos::sqrt(Kokkos::abs(vm_j)) *
                                                   Kokkos::sqrt(Kokkos::abs(vm_jp1)) *
                                                   (q(k, j + 2, i) - q(k, j + 1, i)));
