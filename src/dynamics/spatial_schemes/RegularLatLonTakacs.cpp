...
void RegularLatLonTakacs::validate_dry_buoyancy(
    ...
    if (!enable_dry_buoyancy_) {
        throw std::runtime_error(
            "RegularLatLonTakacs dry buoyancy was not explicitly enabled.");
    }
...
void RegularLatLonTakacs::calculate_buoyancy_tendency_y(
    const Core::State& state, const Core::Grid& grid,
    const Core::Parameters& params,
    Core::Field<3>& out_tendency) const {
    ...
    dry_buoyancy_.add_eta_tendency(
        state.get_field<3>("th"),
        state.get_field<1>("thbar"),
        params.gravity,
        out_tendency,
