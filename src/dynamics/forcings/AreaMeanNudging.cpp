...
AreaMeanNudging::AreaMeanNudging(const Utils::ConfigurationManager& config,
    ...
    if (enable_) {
        ...
        const std::string target_source =
            config_.get_value<std::string>("dynamics.forcings.areamn.target_source", "initial");
        if (target_source == "netcdf") {
            use_netcdf_target_ = true;
            forcing_directory_ =
                config_.get_value<std::string>("dynamics.forcings.areamn.forcing_data.directory");
            forcing_file_prefix_ =
                config_.get_value<std::string>("dynamics.forcings.areamn.forcing_data.file_prefix",
                    "ls_forcing_");
            forcing_interval_s_ = config_.get_value<VVM::Real>(
                "dynamics.forcings.areamn.forcing_data.update_interval_s",
                3600.0);
            if (forcing_interval_s_ <= real(0.0)) {
                throw std::runtime_error(
                    "AreaMeanNudging forcing update interval must be positive.");
            }
            if (!forcing_directory_.empty() && forcing_directory_.back() != '/') {
                forcing_directory_ += '/';
            }

            constant_upper_wind_ =
                config_.get_value<bool>("initial_conditions.constant_upper_wind.enable", false);
            constant_upper_wind_threshold_Pa_ = config_.get_value<VVM::Real>(
                "initial_conditions.constant_upper_wind.pressure_threshold_Pa",
                3000.0);
        }
        else if (target_source != "initial") {
            throw std::runtime_error("Unsupported dynamics.forcings.areamn.target_source '" +
                                     target_source + "'. Expected 'initial' or 'netcdf'.");
        }

        VVM::Real total_pts =
            static_cast<VVM::Real>(grid_.get_global_points_x() * grid_.get_global_points_y());
        inv_total_xy_pts_ = 1.0 / total_pts;

        if (grid_.get_mpi_rank() == 0) {
            std::cout << "--- Initializing Area Mean Nudging (AREAMN) ---" << std::endl;
            std::cout << "  * UVTAU: " << uvtau_ << " s, Nudge Limit: " << nudgelim_ << " m"
                      << std::endl;
...
void
AreaMeanNudging::check_nc_error(int status, const std::string& message) const {
    if (status == NC_NOERR) {
        return;
    }

    if (grid_.get_mpi_rank() == 0) {
        std::cerr << "NetCDF error in AreaMeanNudging: " << message << ": " << nc_strerror(status)
                  << std::endl;
    }
...
std::string
AreaMeanNudging::forcing_filename(VVM::Real time) const {
    std::ostringstream filename;
    filename << forcing_directory_ << forcing_file_prefix_ << std::setfill('0') << std::setw(6)
             << static_cast<long long>(std::llround(time)) << ".nc";
    return filename.str();
}

void
AreaMeanNudging::load_wind_profiles(const std::string& filename,
    ...
    if (grid_.get_mpi_rank() == 0) {
        ...
        size_t file_nz = 0;
        check_nc_error(nc_inq_dimlen(ncid, nz_dimid, &file_nz),
            "Failed to read nz dimension in " + filename);
        ...
        check_nc_error(nc_get_att_longlong(ncid, NC_GLOBAL, "time_seconds", &file_time),
            "Missing time_seconds in " + filename);
        const long long expected_seconds = static_cast<long long>(std::llround(expected_time));
        if (file_time != expected_seconds) {
            nc_close(ncid);
            check_nc_error(NC_EINVAL,
                "time_seconds in " + filename + " does not match its requested forcing time");
        }
        ...
        check_nc_error(nc_get_var_double(ncid, u_varid, u_profile.data()),
            "Failed to read U from " + filename);
        check_nc_error(nc_get_var_double(ncid, v_varid, v_profile.data()),
            "Failed to read V from " + filename);
        check_nc_error(nc_get_var_double(ncid, pbar_varid, pbar_profile.data()),
            "Failed to read pbar from " + filename);
    ...
    ncclResult_t result =
        ncclBroadcast(u_target.data(), u_target.data(), nz, VVM_NCCL_REAL, 0, nccl_comm_, stream_);
    if (result != ncclSuccess) {
        throw std::runtime_error("AreaMeanNudging NCCL U-profile broadcast failed: " +
                                 std::string(ncclGetErrorString(result)));
    }

    result =
        ncclBroadcast(v_target.data(), v_target.data(), nz, VVM_NCCL_REAL, 0, nccl_comm_, stream_);
    if (result != ncclSuccess) {
        throw std::runtime_error("AreaMeanNudging NCCL V-profile broadcast failed: " +
                                 std::string(ncclGetErrorString(result)));
    }

    const cudaError_t sync_result = cudaStreamSynchronize(stream_);
    if (sync_result != cudaSuccess) {
        throw std::runtime_error("AreaMeanNudging NCCL broadcast synchronization failed: " +
...
void
AreaMeanNudging::deterministic_global_sum(
    ...
    if (count_result != ncclSuccess) {
        throw std::runtime_error("AreaMeanNudging could not query the NCCL communicator size: " +
...
#if defined(ENABLE_NCCL)
    const ncclResult_t result = ncclAllGather(local,
        gather_buffer_.data(),
    ...
    if (result != ncclSuccess) {
        throw std::runtime_error(
            "AreaMeanNudging NCCL all-gather failed: " + std::string(ncclGetErrorString(result)));
    }
    const cudaError_t sync_result = cudaStreamSynchronize(state.get_cuda_stream());
    if (sync_result != cudaSuccess) {
        throw std::runtime_error("AreaMeanNudging all-gather synchronization failed: " +
                                 std::string(cudaGetErrorString(sync_result)));
    }
#else
    ...
    Kokkos::deep_copy(Kokkos::View<VVM::Real*, Kokkos::HostSpace>(host_local.data(), count),
        Kokkos::View<const VVM::Real*>(local, count));
    const int mpi_result = MPI_Allgather(host_local.data(),
        count,
        VVM_MPI_REAL,
        host_gather.data(),
        count,
...
void
AreaMeanNudging::initialize(Core::State& state) {
    if (!enable_) {
        return;
    }
    ...
    if (!state.has_field("areamn_xi0")) {
        state.add_field<1>("areamn_xi0",
            {nz},
            Core::FieldMetadata{Core::GridStaggering::StaggeredYZ,
                "s-1",
                "area-mean nudging x-vorticity target"});
    }
    if (!state.has_field("areamn_eta0")) {
        state.add_field<1>("areamn_eta0",
            {nz},
            Core::FieldMetadata{Core::GridStaggering::StaggeredXZ,
                "s-1",
                "area-mean nudging y-vorticity target"});
    }

    if (!state.has_field("areamn_zeta0_top")) {
        state.add_field<0>("areamn_zeta0_top", {});
    }
    if (!state.has_field("areamn_local_sum_xi")) {
        state.add_field<1>("areamn_local_sum_xi", {nz});
    }
    if (!state.has_field("areamn_global_sum_xi")) {
        state.add_field<1>("areamn_global_sum_xi", {nz});
    }
    if (!state.has_field("areamn_local_sum_eta")) {
        state.add_field<1>("areamn_local_sum_eta", {nz});
    }
    if (!state.has_field("areamn_global_sum_eta")) {
        state.add_field<1>("areamn_global_sum_eta", {nz});
    }
    if (!state.has_field("areamn_local_sum_zeta_top")) {
        state.add_field<0>("areamn_local_sum_zeta_top", {});
    }
    if (!state.has_field("areamn_global_sum_zeta_top")) {
        state.add_field<0>("areamn_global_sum_zeta_top", {});
    }
    if (!state.has_field("areamn_utopmn0")) {
        state.add_field<0>("areamn_utopmn0", {});
    }
    if (!state.has_field("areamn_vtopmn0")) {
        state.add_field<0>("areamn_vtopmn0", {});
    }
    if (!state.has_field("areamn_u_target")) {
        state.add_field<1>("areamn_u_target",
            {nz},
    ...
    if (!state.has_field("areamn_v_target")) {
        state.add_field<1>("areamn_v_target",
            {nz},
    ...
    auto& l_sum_xi =
        areamn_local_sum_xi_ref_.get(state, "areamn_local_sum_xi").get_mutable_device_data();
    auto& l_sum_eta =
        areamn_local_sum_eta_ref_.get(state, "areamn_local_sum_eta").get_mutable_device_data();
    auto& g_sum_xi =
        areamn_global_sum_xi_ref_.get(state, "areamn_global_sum_xi").get_mutable_device_data();
    auto& g_sum_eta =
        areamn_global_sum_eta_ref_.get(state, "areamn_global_sum_eta").get_mutable_device_data();
    auto& l_sum_zeta = areamn_local_sum_zeta_top_ref_.get(state, "areamn_local_sum_zeta_top")
                           .get_mutable_device_data();
    auto& g_sum_zeta = areamn_global_sum_zeta_top_ref_.get(state, "areamn_global_sum_zeta_top")
                           .get_mutable_device_data();
    ...
    // State::calculate_horizontal_mean already does.
    {
        ...
        const int kTeamSize =
            std::min(256, std::max(1, Kokkos::DefaultExecutionSpace().concurrency()));

        Kokkos::parallel_for("AREAMN_Init_Local_Sum",
            TeamPol(nz - 2 * h, kTeamSize),
    ...
    auto& zeta0_top =
        areamn_zeta0_top_ref_.get(state, "areamn_zeta0_top").get_mutable_device_data();
...
void
AreaMeanNudging::update_forcing_target(Core::State& state, VVM::Real current_time) {
    if (!enable_ || !use_netcdf_target_) {
        return;
    }
    ...
    VVM::Real weight = (current_time - time_T1_) / (time_T2_ - time_T1_);
    ...
    Kokkos::parallel_for("AREAMN_Update_NetCDF_Target",
    ...
    Kokkos::parallel_for("AREAMN_Update_NetCDF_Top_Target", 1, KOKKOS_LAMBDA(const int) {
...
void
AreaMeanNudging::apply_vorticity(Core::State& state, VVM::Real dt) {
    if (!enable_) {
        return;
    }
    ...
    // Deterministic reductions -- see the note at AREAMN_Init_Local_Sum above.
    {
        ...
        Kokkos::parallel_reduce("AREAMN_Sum_top",
            Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
            KOKKOS_LAMBDA(const int j, const int i, VVM::Real& az) { az += zeta(top_k, j, i); },
            szeta);
        Kokkos::deep_copy(l_sum_zeta, szeta);
    }
    ...
    Kokkos::parallel_for("AREAMN_Apply",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            if (z_coords(k) >= nlim) {
                ...
                VVM::Real sumxn =
                    (uvtau == 0.0) ? xi0(k) : (1.0 - dt / uvtau) * sum_xi + xi0(k) * (dt / uvtau);
                VVM::Real sumyn = (uvtau == 0.0)
                                      ? eta0(k)
                                      : (1.0 - dt / uvtau) * sum_eta + eta0(k) * (dt / uvtau);
...
void
AreaMeanNudging::apply_uvtopmn(Core::State& state, VVM::Real dt) {
    if (!enable_) {
        return;
    }
