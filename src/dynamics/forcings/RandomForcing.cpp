...
RandomForcing::RandomForcing(const Utils::ConfigurationManager& config,
    ...
    amplitude_ =
        config.get_value<VVM::Real>("dynamics.forcings.random_perturbation.amplitude", 1.0);
    seed_ = config.get_value<int>("dynamics.forcings.random_perturbation.random_seed", 12345);
}

void
RandomForcing::initialize(Core::State& state) {
    VVM::Real z_start_m =
        config_.get_value<VVM::Real>("dynamics.forcings.random_perturbation.z_start_m", 0);
    VVM::Real z_end_m =
        config_.get_value<VVM::Real>("dynamics.forcings.random_perturbation.z_end_m", 0);
    ...
    if (grid_.get_mpi_rank() == 0 && enabled_) {
        if (k_start_ > k_end_) {
            std::cout << "[RandomForcing] WARNING: No vertical levels found between " << z_start_m
                      << "m and " << z_end_m << "m." << std::endl;
        }
        else {
            std::cout << "[RandomForcing] Initialized. Range: " << z_start_m << "m to " << z_end_m
                      << "m "
...
void
RandomForcing::apply(Core::State& state) {
    if (!enabled_) {
        return;
    }
    if (state.get_time() >= end_time_) {
        return;
    }
    if (k_start_ > k_end_) {
        return;
    ...
    if (k_end > grid_.get_local_total_points_z()) {
        k_end = grid_.get_local_total_points_z();
    }
    ...
    Kokkos::parallel_for("RandomForcing_Apply",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            const VVM::Real noise =
                RandomForcingDetail::signed_unit_random(seed, step, global_k, global_j, global_i) *
                amp;
            th(k, j, i) += noise;
        });
    return;
