...
TendencyCalculator::TendencyCalculator(std::string var_name,
    ...
    : variable_name_(std::move(var_name)), ab2_tendency_terms_(std::move(ab2_terms)),
...
void
TendencyCalculator::calculate_tendencies(
    ...
    if (!ab2_tendency_terms_.empty()) {
        if (!temp_tendency_field_) {
            temp_tendency_field_ =
                std::make_unique<Core::Field<3>>("temp_ab2_tendency_" + variable_name_,
                    std::array<int, 3>{nz, ny, nx});
        }

        const size_t now_idx = state.get_step() % 2;
        auto& current_tendency_field = (now_idx == 0) ? hist_0_ref_.get(state, hist_0_name_)
...
Core::Field<3>&
TendencyCalculator::calculate_multistage_tendency(Core::State& state,
    const Core::Grid& grid,
    const Core::Parameters& params,
    VVM::Real stage_dt) {
    if (multistage_tendency_terms_.empty()) {
        throw std::runtime_error(
            "No multistage tendency terms are configured for '" + variable_name_ + "'.");
    }
    if (!multistage_tendency_field_) {
        multistage_tendency_field_ =
            std::make_unique<Core::Field<3>>("multistage_tendency_" + variable_name_,
                std::array<int, 3>{grid.get_local_total_points_z(),
