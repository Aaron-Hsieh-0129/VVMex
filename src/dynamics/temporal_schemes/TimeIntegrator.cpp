...
TimeIntegrator::TimeIntegrator(std::string var_name,
    bool has_ab2,
    bool has_fe,
    std::unique_ptr<TemporalScheme> multistage_scheme)
    : variable_name_(std::move(var_name)), has_ab2_terms_(has_ab2), has_fe_terms_(has_fe),
      multistage_scheme_(std::move(multistage_scheme)), prev_state_name_(variable_name_ + "_m"),
      hist_0_name_("d_" + variable_name_ + "_0"), hist_1_name_("d_" + variable_name_ + "_1"),
...
TimeIntegrator::step(Core::State& state,
    ...
    if (multistage_scheme_) {
        throw std::runtime_error("Multistage integration for '" + variable_name_ +
        ...
        if (state.get_step() == 0) {
            if (variable_name_ == "zeta") {
                Kokkos::parallel_for("AB2_Forward_Step",
                    ...
                    KOKKOS_LAMBDA(const int j, const int i) {
                        field_new_view(nz - h - 1, j, i) =
                            field_old_view(nz - h - 1, j, i) + dt * hist_now(nz - h - 1, j, i);
                    });
            }
            else {
                Kokkos::parallel_for("AB2_Forward_Step",
                    Kokkos::MDRangePolicy<Kokkos::Rank<3>>({k_start, h, h},
                        {k_end, ny - h, nx - h}),
                    KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            if (variable_name_ == "zeta") {
                Kokkos::parallel_for("AdamsBashforth2_Step",
                    ...
                    KOKKOS_LAMBDA(const int j, const int i) {
                        field_new_view(nz - h - 1, j, i) =
                            field_old_view(nz - h - 1, j, i) +
                            dt * (real(1.5) * hist_now(nz - h - 1, j, i) -
                                     real(0.5) * hist_prev(nz - h - 1, j, i));
                    });
            }
            else {
                Kokkos::parallel_for("AdamsBashforth2_Step",
                    Kokkos::MDRangePolicy<Kokkos::Rank<3>>({k_start, h, h},
                        {k_end, ny - h, nx - h}),
                    KOKKOS_LAMBDA(const int k, const int j, const int i) {
                        field_new_view(k, j, i) =
                            field_old_view(k, j, i) +
...
TimeIntegrator::step(Core::State& state,
    ...
    if (!evaluate_tendency || !process_stage) {
        throw std::runtime_error(
            "Multistage integration callbacks are missing for '" + variable_name_ + "'.");
    }
    ...
    for (int stage = 0; stage < stage_count; ++stage) {
        const VVM::Real stage_dt = multistage_scheme_->stage_timestep(dt, stage);
        Core::Field<3>& tendency = evaluate_tendency(stage_dt);
        multistage_scheme_->advance_multistage(state, grid, params, tendency, stage_dt, stage);
        process_stage();
    }
}
