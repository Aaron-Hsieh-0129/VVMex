...
require_outside_capture() {
    ...
    if (!Kokkos::is_initialized()) {
        throw std::logic_error("Initialize Kokkos before wind adapter preparation.");
    }
    ...
    if (status != cudaStreamCaptureStatusNone) {
        throw std::logic_error("Wind adapter initialization must occur outside capture.");
    }
#endif
}

void
validate_storage(const Real* first, const Real* second, std::initializer_list<const Real*> inputs) {
    if (first == second) {
        throw std::invalid_argument("Wind adapter outputs must have distinct storage.");
    }

    for (const Real* input : inputs) {
        if (first == input || second == input) {
            throw std::invalid_argument("Wind adapter inputs and outputs must not alias.");
        }
    }
}

void
validate_spacing(const Core::Field<1>& spacing, int required) {
...
HorizontalWindStateAdapter::HorizontalWindStateAdapter(
    ...
    if (layout_.halo < 1 || layout_.local_physical_nx < 1 || layout_.local_physical_ny < 1) {
        throw std::invalid_argument(
            "Wind adapter requires physical cells and at least one halo cell.");
    }
...
HorizontalWindStateAdapter::prepare_execution() {
    ...
    Kokkos::parallel_for("PrepareHorizontalWindStateAdapter",
...
HorizontalWindStateAdapter::initialize_spacing(
    Real dz, const Core::Field<1>& flex_up, Core::Field<1>& spacing) {
    require_outside_capture();

    if (!std::isfinite(dz) || dz <= real(0.0)) {
        throw std::invalid_argument("Reference dz must be positive and finite.");
    }
    ...
    if (input.data() == output.data()) {
        throw std::invalid_argument("Spacing must not alias flex_up.");
    }
    if (input.extent(0) < output.extent(0)) {
        throw std::invalid_argument("Insufficient flex_up entries.");
    }
    ...
    for (std::size_t k = 0; k < output.extent(0); ++k) {
        ...
        if (!std::isfinite(coefficient) || coefficient <= real(0.0)) {
            throw std::invalid_argument("flex_up must be positive and finite.");
        }

        values(k) = dz / coefficient;

        if (!std::isfinite(values(k)) || values(k) <= real(0.0)) {
            throw std::invalid_argument("Invalid derived wind-level spacing.");
        }
    }
...
void
HorizontalWindStateAdapter::validate_volume(const Core::Field<3>& field, int nz) const {
...
void
HorizontalWindStateAdapter::validate_plane(const Core::Field<2>& field) const {
    const auto& data = field.get_device_data();
...
void
HorizontalWindStateAdapter::diagnose_vorticity(const Core::Field<3>& u,
    ...
    if (first < 0 || last < first || last >= nz - 1) {
        throw std::invalid_argument("Invalid vorticity interface range.");
    }

    for (const auto* field : {&u, &v, &w}) {
        validate_volume(*field, nz);
    }
    ...
    const auto policy = Kokkos::Experimental::require(
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({first, h, h},
            {last + 1, layout_.local_total_ny() - h, layout_.local_total_nx() - h}),
        Kokkos::Experimental::WorkItemProperty::HintLightWeight);

    Kokkos::parallel_for("DiagnosePhysicalHorizontalVorticity",
        policy,
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            // xi=h1*omega^1 and eta=-h2*omega^2; h1/h2 do not vary with height.
            xd(k, j, i) = (wd(k, j + 1, i) - wd(k, j, i)) / dq2 * ih2(j, i) -
                          (vd(k + 1, j, i) - vd(k, j, i)) / ds(k);
            ed(k, j, i) = (wd(k, j, i + 1) - wd(k, j, i)) / dq1 * ih1(j, i) -
                          (ud(k + 1, j, i) - ud(k, j, i)) / ds(k);
        });
}

void
HorizontalWindStateAdapter::reconstruct_top(const Core::Field<2>& psi,
    ...
    if (top < 0 || top >= nz) {
        throw std::invalid_argument("Invalid top wind level.");
    }
    ...
    const auto policy = Kokkos::Experimental::require(
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h},
            {layout_.local_total_ny() - h, layout_.local_total_nx() - h}),
        Kokkos::Experimental::WorkItemProperty::HintLightWeight);
...
void
HorizontalWindStateAdapter::integrate_from_top(const Core::Field<3>& w,
    ...
    if (bottom < 0 || top < bottom || top >= nz) {
        throw std::invalid_argument("Invalid wind integration range.");
    }

    for (const auto* field : {&w, &xi, &eta}) {
        validate_volume(*field, nz);
    }
    validate_volume(u, nz);
    validate_volume(v, nz);
