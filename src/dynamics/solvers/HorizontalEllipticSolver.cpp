...
HorizontalEllipticSolver::HorizontalEllipticSolver(const Core::Grid& grid,
    Core::HaloExchanger& halo_exchanger)
    : grid_(grid), halo_exchanger_(halo_exchanger),
      laplace_beltrami_(Operators::make_horizontal_laplace_beltrami_device_view(grid.geometry())),
      scratch_at_z_("horizontal_elliptic_scratch_at_z",
          {grid.get_local_total_points_y(), grid.get_local_total_points_x()}),
      scratch_at_t_("horizontal_elliptic_scratch_at_t",
          {grid.get_local_total_points_y(), grid.get_local_total_points_x()}) {
    ...
    if (grid_.get_halo_cells() < 1) {
        throw std::invalid_argument(
            "HorizontalEllipticSolver requires at least one horizontal halo cell.");
    }
...
void
HorizontalEllipticSolver::validate_field_extents(const Core::Field<2>& field,
    ...
    if (static_cast<int>(data.extent(0)) != expected_ny ||
        ...
        throw std::invalid_argument(std::string("HorizontalEllipticSolver ") + role + " field '" +
                                    field.get_name() +
...
void
HorizontalEllipticSolver::validate_solve_arguments(const Core::Field<2>& right_hand_side,
    ...
    if (&right_hand_side == &solution) {
        throw std::invalid_argument(
            "HorizontalEllipticSolver requires distinct right-hand-side and solution fields.");
    }
    ...
    if (!std::isfinite(options.diagonal_shift) || options.diagonal_shift < VVM::real(0.0)) {

        throw std::invalid_argument(
            "HorizontalEllipticSolver requires a finite nonnegative diagonal shift.");
    }
}

void
HorizontalEllipticSolver::make_extrapolated_guess(
...
void
HorizontalEllipticSolver::refresh_solution_halos(Core::Field<2>& field) {
...
void
HorizontalEllipticSolver::refresh_solution_halos(Core::Field<2>& first, Core::Field<2>& second) {
...
void
HorizontalEllipticSolver::solve_at_t(
    ...
    if (current != &solution) {
        Kokkos::deep_copy(Kokkos::DefaultExecutionSpace(),
            solution.get_mutable_device_data(),
            current->get_device_data());
    }
}

void
HorizontalEllipticSolver::solve_at_z(
    const Core::Field<2>& right_hand_side, Core::Field<2>& solution, const Options& options) {
...
void
HorizontalEllipticSolver::solve_at_z_and_t(const Core::Field<2>& right_hand_side_at_z,
    ...
    const VVM::Real cartesian_inverse_diagonal =
        VVM::real(1.0) / (diagonal_shift + VVM::real(2.0) * inverse_dq1_squared +
                             VVM::real(2.0) * inverse_dq2_squared);
    ...
    for (int iteration = 0; iteration < options.iterations; ++iteration) {
        ...
        if (geometry_kind == Core::Geometry::GeometryKind::Cartesian) {
            ...
            Kokkos::parallel_for("RelaxHorizontalEllipticCartesianAtZAndT",
                compact_policy,
                KOKKOS_LAMBDA(const int j, const int i) {
                    current_at_z_data(j, i) =
                        (diagonal_shift * previous_at_z_data(j, i) +
                            inverse_dq1_squared *
                                (previous_at_z_data(j, i - 1) + previous_at_z_data(j, i + 1)) +
                            inverse_dq2_squared *
                                (previous_at_z_data(j - 1, i) + previous_at_z_data(j + 1, i)) -
                    ...
                    current_at_t_data(j, i) =
                        (diagonal_shift * previous_at_t_data(j, i) +
                            inverse_dq1_squared *
                                (previous_at_t_data(j, i - 1) + previous_at_t_data(j, i + 1)) +
                            inverse_dq2_squared *
                                (previous_at_t_data(j - 1, i) + previous_at_t_data(j + 1, i)) -
