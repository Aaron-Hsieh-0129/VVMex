...
void
HorizontalEllipticSolver::solve_regular_lat_lon_channel_at_z_and_t(
    ...
    const int north_wall_j = ny - halo - 1;
    ...
    for (int iteration = 0; iteration < options.iterations; ++iteration) {
        ...
        Kokkos::parallel_for("RelaxHorizontalEllipticRLLChannelAtZAndT",
            policy,
            KOKKOS_LAMBDA(const int j, const int i) {
                if (owns_north_wall && j == north_wall_j) {

                    current_at_z_data(j, i) = VVM::real(0.0);
                }
                else {
