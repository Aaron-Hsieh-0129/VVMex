...
WindSolver::~WindSolver() {
    ...
    if (cudaGetDevice(&device) == cudaSuccess) {
        ...
        for (const auto& graph : rll_graphs_) {
...
WindSolver::WindSolver(const Core::Grid& grid,
    ...
    : grid_(grid), config_(config), params_(params), state_(state),
      ...
      halo_exchanger_(halo_exchanger), horizontal_elliptic_solver_(grid_, halo_exchanger_) {
    ...
    std::string solver_method_str =
        config.get_value<std::string>("dynamics.solver.w_solver_method");
    ...
    if (!state_.has_field("utop_mean_tmp")) {
        state_.add_field<0>("utop_mean_tmp", {});
    }
    if (!state_.has_field("vtop_mean_tmp")) {
        state_.add_field<0>("vtop_mean_tmp", {});
    }
    if (!state_.has_field("W3DNM1")) {
        state_.add_field<3>("W3DNM1",
            {nz, ny, nx},
            Core::FieldMetadata{Core::GridStaggering::StaggeredZ,
                "m s-1",
                "previous-step vertical wind"});
    }
    if (!state_.has_field("u_topo")) {
        state_.add_field<3>("u_topo",
            {nz, ny, nx},
            Core::FieldMetadata{Core::GridStaggering::StaggeredX,
                "m s-1",
                "topography-adjusted x wind"});
    }
    if (!state_.has_field("v_topo")) {
        state_.add_field<3>("v_topo",
            {nz, ny, nx},
            Core::FieldMetadata{Core::GridStaggering::StaggeredY,
                "m s-1",
                "topography-adjusted y wind"});
    }
    if (!state_.has_field("w_topo")) {
        state_.add_field<3>("w_topo",
            {nz, ny, nx},
            Core::FieldMetadata{Core::GridStaggering::StaggeredZ,
                "m s-1",
                "topography-adjusted vertical wind"});
    }
    if (!state_.has_field("xi_topo")) {
        state_.add_field<3>("xi_topo",
            {nz, ny, nx},
            Core::FieldMetadata{Core::GridStaggering::StaggeredYZ,
                "s-1",
                "topography-adjusted x vorticity"});
    }
    if (!state_.has_field("eta_topo")) {
        state_.add_field<3>("eta_topo",
            {nz, ny, nx},
            Core::FieldMetadata{Core::GridStaggering::StaggeredXZ,
                "s-1",
                "topography-adjusted y vorticity"});
    }
}
...
WindSolver::solve_w() {
        ...
        for (int iter = 0; iter < iter_num; iter++) {
            {
                ...
                Kokkos::parallel_for("jacobi_w_solver",
                    Kokkos::MDRangePolicy<Kokkos::Rank<3>>({h, jlo, ilo}, {nz - h - 1, jhi, ihi}),
                    KOKKOS_LAMBDA(int k, int j, int i) {
                        const VVM::Real horizontal_terms =
                            (P(k, j, i + 1) + P(k, j, i - 1)) * rdx2() +
                            (P(k, j + 1, i) + P(k, j - 1, i)) * rdy2();

                        const VVM::Real vertical_terms =
                            -AGAU(k) * P(k - 1, j, i) * rhobar_up(k - 1) -
                        ...
                        if (diagonal_term != real(0.0)) {
                            C(k, j, i) =
                                (YTEM(k, j, i) + horizontal_terms + vertical_terms) / diagonal_term;
                        }
                    });
            }
    ...
    // Back into the State field, then the same depth-1 refresh the original did.
    {
        ...
        Kokkos::parallel_for("scatter_w_priv",
...
void
WindSolver::integrate_uv_from_top() {
    if (grid_.geometry().kind() != Core::Geometry::GeometryKind::Cartesian) {
        throw std::logic_error("WindSolver::integrate_uv_from_top currently implements Cartesian "
                               "State wind integration only.");
    }
    ...
    Kokkos::parallel_for("u_downward_integration",
        ...
        KOKKOS_LAMBDA(const int j, const int i) {
            ...
            for (int k = nz - h - 2; k >= h - 1; --k) {
                u(k, j, i) =
                    u(k + 1, j, i) - ((w(k, j, i + 1) - w(k, j, i)) * rdx() - eta(k, j, i)) * dz() /
                                         flex_height_coef_up(k);
                v(k, j, i) =
                    v(k + 1, j, i) - ((w(k, j + 1, i) - w(k, j, i)) * rdy() - xi(k, j, i)) * dz() /
                                         flex_height_coef_up(k);
            }
            ...
            u(nz - h, j, i) =
                u(nz - h - 1, j, i) +
                ((w(nz - h - 1, j, i + 1) - w(nz - h - 1, j, i)) * rdx() - eta(nz - h - 1, j, i)) *
...
WindSolver::relax_2d_batched() {
    ...
    if (geometry_kind == Core::Geometry::GeometryKind::Cartesian) {
        ...
        const auto cartesian_geometry =
            grid_.geometry().device_view(Core::Geometry::HorizontalLocation::T);
            ...
            Kokkos::parallel_for("AOUT",
                ...
                KOKKOS_LAMBDA(const int j, const int i) {
                    ...
                    Cp(j, i) = (WRXMU() * Pp(j, i) + rdx2() * (Pp(j, i - 1) + Pp(j, i + 1)) +
                                   rdy2() * (Pp(j - 1, i) + Pp(j + 1, i)) -
                                   cartesian_jacobian * rhs_psi(j, i)) *
                               inv_C0;

                    Cc(j, i) = (WRXMU() * Pc(j, i) + rdx2() * (Pc(j, i - 1) + Pc(j, i + 1)) +
                                   rdy2() * (Pc(j - 1, i) + Pc(j + 1, i)) -
                                   cartesian_jacobian * rhs_chi(j, i)) *
                               inv_C0;
                });
...
void
WindSolver::fill_bounded_q2_potential_halos(Core::Field<2>& first, Core::Field<2>& second) const {
    if (!bounded_q2_stencils_) {
        return;
    }
...
void
WindSolver::exchange_2d_solver_halos(
    Core::Field<2>& first, Core::Field<2>& second, const int depth) {
...
void
WindSolver::prepare_cartesian_wind_recovery_inputs(
    ...
    bc_manager.apply_horizontal_bcs(xi_topo_ref_.get(state_, "xi_topo"));
