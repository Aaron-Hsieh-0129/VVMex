...
outside_capture() {
    ...
    if (status != cudaStreamCaptureStatusNone) {
        throw std::logic_error("Vertical solver initialization must be outside capture.");
    }
#endif
}
...
positive(Real value) {
    if (!std::isfinite(value) || value <= real(0.0)) {
        throw std::invalid_argument(
            "Vertical solver requires finite positive density, stretching and pivots.");
    }
}
...
VerticalEllipticSolver::VerticalEllipticSolver(const Core::Grid& grid,
    ...
    if (last < h) {
        throw std::invalid_argument("Vertical solver requires an interior w level.");
    }
    for (const auto* field : {&rhobar, &rhobar_up, &flex_mid, &flex_up}) {
        if (int(field->get_device_data().extent(0)) != nz) {
            throw std::invalid_argument("Vertical solver profile extent mismatch.");
        }
    }
    ...
    if (grid.geometry().kind() == Core::Geometry::GeometryKind::RegularLatLon) {
        ...
        const auto a =
            Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), t.sqrt_g.one_dimensional);
        const auto b = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(),
            u.sqrt_g_g_contra.a11.one_dimensional);
        const auto c = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(),
            v.sqrt_g_g_contra.a22.one_dimensional);
        for (int j = 0; j < ny; ++j) {
            jt[j] = a(j);
            b1[j] = b(j);
            b2[j] = c(j);
    ...
    auto a = Kokkos::create_mirror(lower_), b = Kokkos::create_mirror(pivot_),
         c = Kokkos::create_mirror(upper_normalized_);
    ...
    for (int j = h; j < ny - h; ++j) {
        for (int k = h; k <= last; ++k) {
            const auto row =
                host_operation
                    .calculate_row_at_t(rho, rho_up, mid, up, inverse_dz, shift_, k, j, h);
            a(j, k) = row.lower;
            b(j, k) = row.diagonal - (k == h ? real(0.0) : row.lower * c(j, k - 1));
            positive(b(j, k));
            c(j, k) = row.upper / b(j, k);
            if (!std::isfinite(a(j, k)) || !std::isfinite(c(j, k))) {
                throw std::invalid_argument("Nonfinite vertical solver coefficient.");
            }
        }
    }
...
VerticalEllipticSolver::validate(const Core::Field<3>& field) const {
    ...
    if (int(data.extent(0)) != grid_.get_local_total_points_z() ||
        int(data.extent(1)) != grid_.get_local_total_points_y() ||
        int(data.extent(2)) != grid_.get_local_total_points_x()) {
...
VerticalEllipticSolver::solve(const Core::Field<3>& xi,
    ...
    for (int a = 0; a < 4; ++a) {
        for (int b = a + 1; b < 4; ++b) {
            if (pointers[a] == pointers[b]) {
                throw std::invalid_argument("Vertical solver fields must not alias.");
            }
        }
    }
    const int nz = grid_.get_local_total_points_z(), ny = grid_.get_local_total_points_y(),
    ...
    for (int iteration = 0; iteration < iterations; ++iteration) {
        ...
        Kokkos::parallel_for("VerticalFixedLineSweep", policy, KOKKOS_LAMBDA(int column) {
            ...
            const Real east = operation.coefficient(operation.weighted_inverse_metric_u, j) /
                              (operation.dq1 * operation.dq1);
            const Real north = operation.coefficient(operation.weighted_inverse_metric_v, j) /
                               (operation.dq2 * operation.dq2);
            const Real south = operation.coefficient(operation.weighted_inverse_metric_v, j - 1) /
                               (operation.dq2 * operation.dq2);
            for (int k = h; k <= last; ++k) {
                const Real r = shift * p(k, j, i) + rhs(k, j, i) +
                               east * (p(k, j, i + 1) + p(k, j, i - 1)) + north * p(k, j + 1, i) +
                               south * p(k, j - 1, i);
                temporary(column, k) =
                    (r - (k == h ? real(0.0) : lower(j, k) * temporary(column, k - 1))) /
                    pivot(j, k);
            }
