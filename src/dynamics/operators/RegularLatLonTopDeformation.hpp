...
struct RegularLatLonTopDeformationDeviceView {
    ...
    calculate_at_z(const Fields& fields, int k_top, int j, int i) const noexcept {
        ...
        const VVM::Real left_w = fields.w(lower, j, i) + fields.w(lower, j + 1, i);
        const VVM::Real right_w = fields.w(lower, j, i + 1) + fields.w(lower, j + 1, i + 1);
        ...
        const VVM::Real omega1_sum =
            fields.xi(lower, j, i) / h1_at_v(j, i) + fields.xi(lower, j, i + 1) / h1_at_v(j, i + 1);
        const VVM::Real omega2_sum = -fields.eta(lower, j, i) / h2_at_u(j, i) -
                                     fields.eta(lower, j + 1, i) / h2_at_u(j + 1, i);

        const VVM::Real difference_q1 = fields.w(lower, j + 1, i + 1) - fields.w(lower, j + 1, i) +
                                        fields.w(lower, j, i + 1) - fields.w(lower, j, i);
        const VVM::Real difference_q2 = fields.w(lower, j + 1, i + 1) - fields.w(lower, j, i + 1) +
        ...
        result.twisting = VVM::real(0.125) * lower_factor *
                          (omega1_sum * difference_q1 / dq1 + omega2_sum * difference_q2 / dq2);
        result.planetary = -VVM::real(0.125) * inverse_mid * planetary_stretching;
        return result;
    }
};
