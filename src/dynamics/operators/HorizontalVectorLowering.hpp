...
KOKKOS_INLINE_FUNCTION ContravariantVectorStencilAtU
load_contravariant_vector_stencil_at_u(const ContravariantQ1View& contravariant_q1_at_u,
    const ContravariantQ2View& contravariant_q2_at_v,
    const int j,
...
template <typename ContravariantQ1View, typename ContravariantQ2View>
KOKKOS_INLINE_FUNCTION ContravariantVectorStencilAtV
load_contravariant_vector_stencil_at_v(const ContravariantQ1View& contravariant_q1_at_u,
    const ContravariantQ2View& contravariant_q2_at_v,
    const int j,
    const int i) noexcept {
...
struct HorizontalVectorLoweringDeviceView {
    ...
    calculate_covariant_q1_at_u(
        const int j, const int i, const ContravariantVectorStencilAtU& vector) const noexcept {

        const VVM::Real direct_component = u.g_cov.a11(j, i) * vector.q1_at_u_j_i;

        const VVM::Real cross_component =
            VVM::real(0.25) *
            (v.g_cov.a12(j, i) * vector.q2_at_v_j_i + v.g_cov.a12(j, i + 1) * vector.q2_at_v_j_ip1 +
                v.g_cov.a12(j - 1, i) * vector.q2_at_v_jm1_i +
                v.g_cov.a12(j - 1, i + 1) * vector.q2_at_v_jm1_ip1);
    ...
    calculate_covariant_q2_at_v(
        const int j, const int i, const ContravariantVectorStencilAtV& vector) const noexcept {
        ...
        const VVM::Real cross_component =
            ...
            (u.g_cov.a12(j, i) * vector.q1_at_u_j_i + u.g_cov.a12(j + 1, i) * vector.q1_at_u_jp1_i +
                u.g_cov.a12(j, i - 1) * vector.q1_at_u_j_im1 +
                u.g_cov.a12(j + 1, i - 1) * vector.q1_at_u_jp1_im1);
    ...
    calculate_covariant_q1_at_u(const ContravariantQ1View& contravariant_q1_at_u,
        const ContravariantQ2View& contravariant_q2_at_v,
        const int j,
        const int i) const noexcept {

        return calculate_covariant_q1_at_u(j,
            i,
            load_contravariant_vector_stencil_at_u(contravariant_q1_at_u,
    ...
    template <typename ContravariantQ1View, typename ContravariantQ2View>
    KOKKOS_INLINE_FUNCTION VVM::Real
    calculate_covariant_q2_at_v(const ContravariantQ1View& contravariant_q1_at_u,
        const ContravariantQ2View& contravariant_q2_at_v,
        const int j,
        const int i) const noexcept {

        return calculate_covariant_q2_at_v(j,
            i,
            load_contravariant_vector_stencil_at_v(contravariant_q1_at_u,
                contravariant_q2_at_v,
                j,
                i));
    }
...
inline HorizontalVectorLoweringDeviceView
make_horizontal_vector_lowering_device_view(const Core::Geometry::HorizontalGeometry& geometry) {
