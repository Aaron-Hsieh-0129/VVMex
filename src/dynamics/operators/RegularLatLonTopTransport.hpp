...
struct RegularLatLonTopTransportDeviceView {
    ...
    scalar(const Fields& f, int k, int j, int i, bool planetary) const {
        return planetary ? f.f_at_z(j, i) / f.rho(k) : f.zeta(k, j, i);
    }

    template <typename Fields>
    KOKKOS_INLINE_FUNCTION Real
    mass_flux(const Fields& f, int k, int j, int i, int direction) const {
        Real sum = real(0.0);
        ...
        const Real jacobian =
            direction == 0 ? horizontal.jacobian_v(j, i + 1) : horizontal.jacobian_u(j + 1, i);
        return real(0.25) * f.rho(k) * sum * jacobian;
    }

    template <typename Fields>
    KOKKOS_INLINE_FUNCTION Real
    face(const Fields& f, int k, int j, int i, int direction, bool planetary) const {
        const int di = direction == 0 ? 1 : 0;
        const int dj = direction == 1 ? 1 : 0;
        return horizontal.face_flux.calculate(mass_flux(f, k, j - dj, i - di, direction),
            mass_flux(f, k, j, i, direction),
            mass_flux(f, k, j + dj, i + di, direction),
            scalar(f, k, j - dj, i - di, planetary),
            scalar(f, k, j, i, planetary),
            scalar(f, k, j + dj, i + di, planetary),
            scalar(f, k, j + 2 * dj, i + 2 * di, planetary));
    }

    template <typename Fields>
    KOKKOS_INLINE_FUNCTION Real
    vertical_mass_flux(const Fields& f, int k, int j, int i) const {
    ...
    calculate_at_z(const Fields& f, int top, int j, int i, bool planetary = false) const {
        ...
        result.vertical =
            horizontal.face_flux.calculate_at_upper_boundary(vertical_mass_flux(f, top - 2, j, i),
                vertical_mass_flux(f, top - 1, j, i),
                scalar(f, top - 2, j, i, planetary),
                scalar(f, top - 1, j, i, planetary),
                scalar(f, top, j, i, planetary)) *
            f.inverse_spacing_mid(top);
        return result;
    }
};
