...
struct ProductProfile {
    ...
    int offset = 0;
    KOKKOS_INLINE_FUNCTION Real operator()(int k) const { return first(k) * second(k + offset); }
};
struct InverseSpacing {
    Profile flex;
    Kokkos::View<Real> inverse_dz;
    KOKKOS_INLINE_FUNCTION Real operator()(int k) const { return flex(k) * inverse_dz(); }
};
struct Fields {
...
struct TendencyFunctor {
    ...
    KOKKOS_INLINE_FUNCTION void operator()(int k, int j, int i) const {
        if (!evaluate) return;
        using Term = RegularLatLonVorticityTendency::Term;
        Real value = real(0.0);
        if (term == Term::Transport || (term == Term::Planetary && variable == 2)) {
            const auto t = variable == 2 ? transport().calculate_at_z(fields, k, j, i, term == Term::Planetary)
                : variable == 0 ? transport().horizontal.calculate_xi_at_v(fields, fields.w, k, j, i, begin, end)
                                : transport().horizontal.calculate_eta_at_u(fields, fields.w, k, j, i, begin, end);
            value = t.q1 + t.q2 + t.vertical;
        }
        if (term != Term::Transport) {
            if (variable == 2) {
                const auto t = deformation().calculate_at_z(fields, k, j, i);
                value += term == Term::Stretching ? t.stretching : term == Term::Twisting ? t.twisting : t.planetary;
            } else {
                const auto t = variable == 0 ? transport().horizontal.components.calculate_xi_at_v(fields, k, j, i)
...
RegularLatLonVorticityTendency::RegularLatLonVorticityTendency(const Core::Geometry::HorizontalGeometry& geometry)
...
void RegularLatLonVorticityTendency::prepare_execution() {
    ...
    Kokkos::parallel_for("PrepareRLLVorticityTendency", Kokkos::Experimental::require(
        Policy({0, 0, 0}, {1, 1, 1}), Kokkos::Experimental::WorkItemProperty::HintLightWeight), preparation);
    Kokkos::fence();
}

void RegularLatLonVorticityTendency::add(const Core::State& state, const Core::Grid& grid,
    const Core::Parameters& params, Core::Field<3>& output, const std::string& variable, Term term) const {
    const int component = variable == "xi" ? 0 : variable == "eta" ? 1 : variable == "zeta" ? 2 : -1;
    ...
    Kokkos::parallel_for("RLLVorticityTendency", Kokkos::Experimental::require(
        Policy({begin, h, h}, {end, grid.get_local_total_points_y() - h, grid.get_local_total_points_x() - h}),
        Kokkos::Experimental::WorkItemProperty::HintLightWeight),
        TendencyFunctor{transport_, deformation_, f, output.get_mutable_device_data(), component, h, top, term});
}
} // namespace VVM::Dynamics::Operators
