...
struct RegularLatLonTerrainDeviceView {
    ...
    KOKKOS_INLINE_FUNCTION Real xi(const Volume& v, const Volume& w, Real inverse_dz,
    ...
    template<class Volume>
    KOKKOS_INLINE_FUNCTION Real eta(const Volume& u, const Volume& w, Real inverse_dz,
                                    int k, int j, int i) const {
...
inline RegularLatLonTerrainDeviceView make_regular_lat_lon_terrain_device_view(
    const Core::Geometry::HorizontalGeometry& geometry) {
    using namespace Core::Geometry;
    if (geometry.kind() != GeometryKind::RegularLatLon)
        throw std::invalid_argument("RLL terrain curl requires regular latitude-longitude geometry.");
    return {geometry.device_view(HorizontalLocation::U).physical_to_contravariant.a11,
            ...
            real(1.)/geometry.dq1(), real(1.)/geometry.dq2()};
}
} // namespace VVM::Dynamics::Operators
#endif
