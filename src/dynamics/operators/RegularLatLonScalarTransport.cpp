...
require_cuda_success(const cudaError_t status, const char* operation) {
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("RegularLatLonScalarTransport: ") + operation + ": " +
...
RegularLatLonScalarTransport::RegularLatLonScalarTransport(
...
RegularLatLonScalarTransport::prepare_execution() {
    ...
    if (!Kokkos::is_initialized()) {
        throw std::logic_error("RegularLatLonScalarTransport::prepare_execution "
    ...
    Kokkos::parallel_for("PrepareRegularLatLonScalarTransport",
        preparation_policy,
        KOKKOS_LAMBDA(const int){});

    require_cuda_success(cudaGetLastError(), "launch backend preparation");

    execution.fence("Complete RegularLatLonScalarTransport "
                    "backend preparation");
...
void
RegularLatLonScalarTransport::validate_volume(
    ...
    if (static_cast<int>(data.extent(0)) != nz ||
        ...
        throw std::invalid_argument(std::string("RegularLatLonScalarTransport: "
...
RegularLatLonScalarTransport::add_flux_convergence(const Core::Field<3>& scalar_q,
    ...
    Kokkos::parallel_for("RegularLatLonTakacsScalarFluxConvergence", policy, functor);
}
