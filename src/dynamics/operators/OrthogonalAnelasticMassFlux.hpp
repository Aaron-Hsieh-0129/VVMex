...
struct OrthogonalContravariantMassFluxQ1DeviceView {
    ...
    VVM::Real operator()(
        ...
        return physical_to_contravariant_11(j, i) *
            physical_mass_flux(k, j, i);
    }
};
...
struct OrthogonalContravariantMassFluxQ2DeviceView {
    ...
    VVM::Real operator()(
        ...
        return physical_to_contravariant_22(j, i) *
            physical_mass_flux(k, j, i);
    }
};

inline bool is_constant_zero(
    const Core::Geometry::GeometryField2D& field) noexcept {

    return
        field.layout ==
            Core::Geometry::GeometryFieldLayout::Constant
        && field.constant == VVM::real(0.0);
}

inline void require_orthogonal_mass_flux_geometry(
    const Core::Geometry::HorizontalGeometry& geometry) {

    const auto u = geometry.device_view(
        Core::Geometry::HorizontalLocation::U);
    const auto v = geometry.device_view(
        Core::Geometry::HorizontalLocation::V);

    if (!is_constant_zero(
            u.physical_to_contravariant.a12)
        || !is_constant_zero(
            u.physical_to_contravariant.a21)
        || !is_constant_zero(
            v.physical_to_contravariant.a12)
        || !is_constant_zero(
            v.physical_to_contravariant.a21)) {

        throw std::invalid_argument(
            "Physical mass-flux adaptation currently requires an "
...
make_orthogonal_contravariant_mass_flux_q1_device_view(
    ...
    OrthogonalContravariantMassFluxQ1DeviceView<
        PhysicalMassFluxView> result;

    result.physical_mass_flux = physical_mass_flux;
    result.physical_to_contravariant_11 =
        u.physical_to_contravariant.a11;
...
make_orthogonal_contravariant_mass_flux_q2_device_view(
    ...
    result.physical_mass_flux = physical_mass_flux;
    result.physical_to_contravariant_22 =
        v.physical_to_contravariant.a22;
