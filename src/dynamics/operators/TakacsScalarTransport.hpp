...
struct TakacsFaceFluxDeviceView {
    ...
    static VVM::Real positive_part(const VVM::Real value) noexcept {
        return VVM::real(0.5) * (value + Kokkos::abs(value));
    }

    KOKKOS_INLINE_FUNCTION
    static VVM::Real negative_part(const VVM::Real value) noexcept {
        return VVM::real(0.5) * (value - Kokkos::abs(value));
    }

    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate(
        const VVM::Real mass_flux_previous, const VVM::Real mass_flux, const VVM::Real mass_flux_next,
        const VVM::Real scalar_previous, const VVM::Real scalar_left, const VVM::Real scalar_right,
        ...
        const VVM::Real correction =
            positive * (scalar_right - scalar_left)
            - Kokkos::sqrt(positive) *
                Kokkos::sqrt(positive_previous) *
    ...
    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate_at_lower_boundary(
        const VVM::Real mass_flux, const VVM::Real mass_flux_next,
        const VVM::Real scalar_left, const VVM::Real scalar_right, const VVM::Real scalar_next) const noexcept {
    ...
    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate_at_upper_boundary(
        const VVM::Real mass_flux_previous, const VVM::Real mass_flux,
        const VVM::Real scalar_previous, const VVM::Real scalar_left, const VVM::Real scalar_right) const noexcept {
        ...
        const VVM::Real positive_previous =
            positive_part(mass_flux_previous);
        const VVM::Real positive =
            positive_part(mass_flux);

        const VVM::Real correction = positive * (scalar_right - scalar_left)
            - Kokkos::sqrt(positive) * Kokkos::sqrt(positive_previous) * (scalar_left - scalar_previous);
...
struct TakacsScalarTransportDeviceView {
    ...
    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate_horizontal_flux_convergence_at_t(
        const ScalarView& scalar_q, const MassFluxQ1View& mass_flux_q1, const MassFluxQ2View& mass_flux_q2,
        const int k, const int j, const int i) const noexcept {

        const VVM::Real flux_q1_plus = face_flux.calculate(
        ...
        const VVM::Real flux_q1_minus = face_flux.calculate(
            mass_flux_q1(k, j, i - 2),
            mass_flux_q1(k, j, i - 1),
            mass_flux_q1(k, j, i),
            scalar_q(k, j, i - 2),
            scalar_q(k, j, i - 1),
        ...
        const VVM::Real flux_q2_plus = face_flux.calculate(
        ...
        const VVM::Real flux_q2_minus = face_flux.calculate(
            mass_flux_q2(k, j - 2, i),
            mass_flux_q2(k, j - 1, i),
            mass_flux_q2(k, j, i),
            scalar_q(k, j - 2, i),
            scalar_q(k, j - 1, i),
        ...
        return -horizontal_divergence.at_t(
            j,
            i,
            flux_q1_plus,
            flux_q1_minus,
    ...
    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate_vertical_face_flux(
        const ScalarView& scalar_q,
    ...
    template<typename ScalarView, typename VerticalMassFluxView>
    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate_vertical_flux_convergence_at_t(
        const ScalarView& scalar_q,
        const VerticalMassFluxView& mass_flux_vertical,
        const int k,
        const int j,
    ...
    KOKKOS_INLINE_FUNCTION
    VVM::Real calculate_tendency_at_t(
        const ScalarView& scalar_q,
