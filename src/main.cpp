...
#if defined(ENABLE_NCCL)
void
init_nccl(ncclComm_t* comm, int rank, int size, MPI_Comm mpi_comm) {
    ncclUniqueId id;
    if (rank == 0) {
        ncclGetUniqueId(&id);
    }
...
int
get_io_tasks(int argc, char* argv[]) {
...
bool
is_launcher_rank_zero() {
    const char* rank_vars[] = {"OMPI_COMM_WORLD_RANK",
        "PMIX_RANK",
        "PMI_RANK",
        "MV2_COMM_WORLD_RANK",
        "SLURM_PROCID"};
...
print_usage(const char* argv0) {
    ...
    std::cout
        << "VVMex -- Vector Vorticity cloud-resolving Model (Kokkos + MPI)\n"
           ...
           "There is no built-in default configuration, which is why a bare `"
        << exe
        << "`\n"
           "has nothing to run.\n"
           "\n"
        ...
        << "\n"
           ...
           "  4. Run it:\n"
           "       ./submit.py --local --preset <preset> -c "
        << example
        << " --compute 1\n"
        ...
        << "\n"
           ...
           "\n"
           "WHERE TO READ MORE (under "
        << (root.empty() ? std::string("the repository root") : root)
        << ")\n"
...
int
run_vvm(int argc, char* argv[], int world_rank, int world_size) {
    const std::string config_file_path = parse_command_line(argc, argv).config_file_path;

    if (!std::ifstream(config_file_path).good()) {
        if (world_rank == 0) {
            std::cerr << "[Main] ERROR: cannot open configuration file: " << config_file_path
                      << "\n"
                      << "  Run `" << argv[0] << " --help` for how to pick one." << std::endl;
        }
        return 1;
    }
            ...
            if (world_rank == 0) {
                try {
                    const std::string output_dir =
                        config.get_value<std::string>("output.output_dir", std::string());
                    const std::string prefix =
                        config.get_value<std::string>("output.output_filename_prefix",
                            std::string());

                    const VVM::Utils::SstCleanupResult cleanup =
                        VVM::Utils::remove_stale_sst_path(output_dir, prefix);

                    switch (cleanup.outcome) {
                    case VVM::Utils::SstCleanupOutcome::Removed:
                        std::cout << "[Main] Removing stale SST path: " << cleanup.path.string()
                    ...
                    case VVM::Utils::SstCleanupOutcome::Failed:
                        std::cerr << "[Main] ERROR: stale SST cleanup: " << cleanup.message << "\n"
    ...
    if (num_sim_tasks <= 0) {
        if (world_rank == 0) {
            std::cerr << "Error: Not enough ranks for simulation!" << std::endl;
        }
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
        ...
        if (cuda_err != cudaSuccess || num_gpus <= 0) {
            std::cerr << "[Rank " << world_rank
                      ...
                      << (std::getenv("CUDA_VISIBLE_DEVICES") ? std::getenv("CUDA_VISIBLE_DEVICES")
                                                              : "<unset>")
                      << " cudaGetDeviceCount error=" << cudaGetErrorString(cuda_err) << std::endl;
    ...
    std::cout << "[KokkosInit]"
              << " world_rank=" << world_rank << " role=" << (is_compute_rank ? "compute" : "io")
...

    {
        ...
        timing.configure(config.get_value<bool>("performance.timing.enable", true),
        ...
        const std::string output_engine = config.get_value<std::string>("output.engine", "HDF5");
        if (output_engine == "BP5") {
            history_writer = std::make_unique<VVM::IO::BP5::Bp5HistoryWriter>(config,
                grid,
                parameters,
                state,
                split_comm);
            ...
            history_writer = std::make_unique<VVM::IO::LegacyHistoryWriter>(config,
                grid,
                parameters,
                state,
                split_comm);
        ...
        const bool output_initial_step = config.get_value<bool>("output.output_initial_step", true);
        ...
        if (restart_enabled) {
            next_output_time =
                (std::floor(state.get_time() / output_interval) + 1.0) * output_interval;
        }
