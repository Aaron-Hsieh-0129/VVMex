...
inline const char*
to_string(RestartTimeSource source) {
    switch (source) {
    case RestartTimeSource::FileTimeAndStep:
        return "file model_time_s + model_step";
    case RestartTimeSource::FileTimeDerivedStep:
        return "file model_time_s, step derived from dt";
    case RestartTimeSource::FileStepDerivedTime:
        return "file model_step, time derived from dt";
    case RestartTimeSource::LegacyConfig:
        return "restart.legacy_time_s";
    case RestartTimeSource::Filename:
        return "file name digits (legacy fallback)";
    }
    return "unknown";
}
...
inline double
restart_time_tolerance_s(double a, double b) {
    const double scale = std::max(std::fabs(a), std::fabs(b));
    return std::max(1e-6, 1e-6 * scale);
}

inline std::string
format_restart_seconds(double value) {
...
inline bool
parse_trailing_file_index(const std::string& source_file, long long& index) {
    const std::size_t dot_pos = source_file.find_last_of('.');
    const std::size_t search_end = (dot_pos == std::string::npos) ? source_file.size() : dot_pos;
    ...
    while (
        digit_start > 0 && std::isdigit(static_cast<unsigned char>(source_file[digit_start - 1]))) {
        --digit_start;
    }
...
inline RestartTimeResolution
resolve_restart_time(const RestartFileMetadata& meta,
    ...
    auto derive_step = [&](double time_s) {
        ...
        if (std::fabs(expected - time_s) > tolerance) {
            ...
            oss << "[WARNING] Restart time " << format_restart_seconds(time_s)
                ...
                << format_restart_seconds(policy.dt_s) << " s. Using the nearest step " << step
    ...
    if (meta.has_time) {
        ...
        if (meta.has_step && !policy.ignore_stored_step) {
            if (meta.step < 0) {
                std::ostringstream oss;
                oss << "restart step stored in '" << source_file << "' is negative (" << meta.step
                    << ").";
                return fail(oss.str());
            }
            ...
            if (std::fabs(expected - meta.time_s) > tolerance) {
                ...
                oss << "Inconsistent restart metadata in '" << source_file << "':\n"
                    << "    stored time          = " << format_restart_seconds(meta.time_s)
                    << " s\n"
                    << "    stored step          = " << meta.step << "\n"
                    << "    configured dt        = " << format_restart_seconds(policy.dt_s)
                    << " s\n"
                    << "    expected time (step * dt) = " << format_restart_seconds(expected)
                    << " s\n"
                    << "    difference           = "
                    ...
                    << "  Neither value is silently preferred. If simulation.dt_s was changed on "
                       "purpose, set \"restart\": {\"ignore_stored_step\": true} to keep the "
                       "stored "
    ...
    if (policy.has_legacy_time) {
        const std::string time_error = validate_time(policy.legacy_time_s, "restart.legacy_time_s");
        if (!time_error.empty()) {
            return fail(time_error);
        }
    ...
    if (policy.allow_filename_fallback) {
        ...
        if (index < 0) {
            std::ostringstream oss;
            oss << "restart file name index parsed from '" << source_file << "' is negative ("
                << index << ").";
            return fail(oss.str());
        }
        ...
        oss << "[WARNING] Restart file contains no model-time metadata and "
            ...
            << index << " * " << format_restart_seconds(policy.filename_interval_s)
            << " s = " << format_restart_seconds(time_s) << " s.\n"
