...
strip_trailing_separator(std::filesystem::path p) {
...
inline SstPathResult
resolve_sst_path(const std::string& output_dir, const std::string& prefix) {
    ...
    if (output_dir.empty()) {
        return reject(SstPathError::EmptyOutputDir, "output.output_dir is empty");
    }
    if (prefix.empty()) {
        return reject(SstPathError::EmptyPrefix, "output.output_filename_prefix is empty");
    }
    ...
    if (prefix == "." || prefix == ".." || prefix_path.has_root_path() ||
    ...
    if (ec) {
        return reject(SstPathError::UnresolvableOutputDir,
            "cannot resolve output.output_dir '" + output_dir + "': " + ec.message());
    }
    ...
    if (filename == "." || filename == "..") {
        return reject(SstPathError::NotAnSstName, "refusing to remove '" + target.string() + "'");
    }
    ...
    if (strip_trailing_separator(target.lexically_normal()) != target ||
        ...
        return reject(SstPathError::EscapesOutputDir,
            "refusing to remove '" + target.string() +
                "': not a direct child of output.output_dir '" + directory.string() + "'");
    }
...
struct SstCleanupResult {
    ...
    bool
    ok() const {
        return outcome == SstCleanupOutcome::NotPresent || outcome == SstCleanupOutcome::Removed;
    }
};

inline SstCleanupResult
remove_stale_sst_path(const std::string& output_dir, const std::string& prefix) {
    namespace fs = std::filesystem;
    ...
    if (status_ec) {
        ...
        result.message = "cannot inspect '" + resolved.path.string() + "': " + status_ec.message();
        return result;
    }
    ...
    if (remove_ec) {
        result.outcome = SstCleanupOutcome::Failed;
        result.message =
            "failed to remove '" + resolved.path.string() + "': " + remove_ec.message();
        return result;
    }
    ...
    result.outcome = (removed == 0) ? SstCleanupOutcome::NotPresent : SstCleanupOutcome::Removed;
