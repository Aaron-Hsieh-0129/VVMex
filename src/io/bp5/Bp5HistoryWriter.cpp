...
Bp5HistoryWriter::Bp5HistoryWriter(const Utils::ConfigurationManager& config,
    const Core::Grid& grid,
    const Core::Parameters& parameters,
    Core::State& state,
    MPI_Comm comm)
    : grid_(grid), parameters_(parameters), state_(state), comm_(comm),
      config_(Bp5OutputConfig::from_config(config)),
      schema_(Bp5FieldSchema::parse_bounds(config,
      ...
      adios_(comm), field_source_(state_, buffers_) {
    MPI_Comm_rank(comm_, &rank_);
    MPI_Comm_size(comm_, &size_);

    element_type_ = config_.element_type();
    effective_buffer_mode_ = config_.effective_buffer_mode();
    narrowed_restart_source_ = element_type_ == OutputElementType::Float32 &&
    ...
    try {
        ...
        prepare_coordinates();
        validate_coverage();
        const adios2::Mode open_mode = config_.existing_dataset == ExistingDatasetPolicy::Append
                                           ? adios2::Mode::Append
                                           : adios2::Mode::Write;
        ...
        if (config_.existing_dataset == ExistingDatasetPolicy::Append) {
            ...
            if (rank_ == 0) {
                std::cout << "  [BP5] Appending after " << existing_steps_
                          << " existing step(s), restored model_time_s=" << state_.get_time()
                          << ", model_step=" << state_.get_step() << std::endl;
            }
        }
...
void
Bp5HistoryWriter::validate_collective_configuration(
    ...
    resolved << config_.num_subfiles << ':' << config_.stats_level << ':' << config_.async_write
             << ':' << static_cast<int>(config_.buffer_mode) << ':'
             << static_cast<int>(config_.precision) << ':' << static_cast<int>(element_type_) << ':'
             << static_cast<int>(effective_buffer_mode_) << ':'
             << static_cast<int>(config_.existing_dataset) << ':';
    const auto& bounds = schema_.bounds();
    resolved << bounds.x_start << ':' << bounds.x_end << ':' << bounds.y_start << ':'
             << bounds.y_end << ':' << bounds.z_start << ':' << bounds.z_end << ':';
...
Bp5HistoryWriter::~Bp5HistoryWriter() {
    if (!closed_) {
        try {
            ...
            std::cerr << "[BP5 rank " << rank_ << "] destructor close failed for '"
...
void
Bp5HistoryWriter::prepare_dataset_path(const Utils::ConfigurationManager& config) {
    const std::string output_dir = config.get_value<std::string>("output.output_dir");
    const std::string prefix = config.get_value<std::string>("output.output_filename_prefix");
    ...
    if (rank_ == 0) {
        try {
            ...
            const bool append = config_.existing_dataset == ExistingDatasetPolicy::Append;
            ...
            if (restart_enabled && has_restart_source) {
                ...
                const auto source = std::filesystem::weakly_canonical(
                    std::filesystem::path(config.get_value<std::string>("restart.source_file")),
                    source_ec);
                const auto target = std::filesystem::weakly_canonical(dataset_path_, target_ec);
                if (source_ec || target_ec) {
                    throw std::runtime_error("cannot resolve BP5 restart source and output target");
                }
            ...
            dataset_path_ = prepare_bp5_dataset_path(output_dir, prefix, config_.existing_dataset);
        }
        catch (const std::exception& e) {
            std::cerr << "[BP5 rank 0] path preparation failed for '" << dataset_path_.string()
                      << "': " << e.what() << std::endl;
            path_status = 1;
        }
    }
...
void
Bp5HistoryWriter::define_metadata(const std::string& field_name,
    ...
    if (metadata.grid_staggering != Core::GridStaggering::Unspecified) {
        io_.DefineAttribute<std::string>("grid_staggering",
...
void
Bp5HistoryWriter::skip_field(const std::string& field_name, const char* reason) const {
    if (rank_ == 0) {
...
Bp5HistoryWriter::define_field(const std::string& field_name) {
    ...
    std::visit([&](const auto& field) {
        ...
        if constexpr (std::is_same_v<FieldType, std::monostate>) {
            ...
            const std::size_t components = FieldType::DimValue == 4 ? view.extent(0) : 1;
            FieldSelection selection = schema_.selection(FieldType::DimValue, components);
            for (std::size_t d = 0; d < selection.memory_count.size(); ++d) {
                if (view.extent(d) != selection.memory_count[d]) {
                    throw std::invalid_argument("Configured BP5 field '" + field_name +
            ...
            if (effective_buffer_mode_ == CpuBufferMode::Direct) {
                ...
                if constexpr (!std::is_same_v<Layout, Kokkos::LayoutRight>) {
                    throw std::invalid_argument("BP5 direct mode requires LayoutRight for field '" +
                                                field_name +
            ...
            if (element_type_ == OutputElementType::Float32) {
                auto typed = io_.DefineVariable<float>(field_name,
                    selection.shape,
                    selection.start,
                    selection.count,
                    adios2::ConstantDims);
                if (direct && !selection.empty()) {
                    typed.SetMemorySelection({selection.memory_start, selection.memory_count});
                }
                variable = typed;
            }
            else {
                auto typed = io_.DefineVariable<double>(field_name,
                    selection.shape,
                    selection.start,
                    selection.count,
                    adios2::ConstantDims);
...
void
Bp5HistoryWriter::define_schema() {
    ...
    x_variable_ = io_.DefineVariable<VVM::Real>("coordinates/x",
        {region.global_nx},
        {0},
        {rank_ == 0 ? region.global_nx : 0},
        adios2::ConstantDims);
    y_variable_ = io_.DefineVariable<VVM::Real>("coordinates/y",
        {region.global_ny},
        {0},
        {rank_ == 0 ? region.global_ny : 0},
        adios2::ConstantDims);
    z_variable_ = io_.DefineVariable<VVM::Real>("coordinates/z_mid",
        {region.global_nz},
        {0},
        {rank_ == 0 ? region.global_nz : 0},
        adios2::ConstantDims);
    ...
    io_.DefineAttribute<std::string>("vvm_real_precision",
        sizeof(VVM::Real) == 8 ? "float64" : "float32");
    io_.DefineAttribute<std::string>("vvm_field_precision",
        output_element_type_name(element_type_));
    io_.DefineAttribute<std::string>("vvm_coordinate_order", "z,y,x");
    const std::array<std::uint64_t, 3> grid_shape = {region.global_nz,
        region.global_ny,
        region.global_nx};
    io_.DefineAttribute<std::uint64_t>("vvm_global_grid_shape",
        grid_shape.data(),
        grid_shape.size());
    const auto& bounds = schema_.bounds();
    const std::array<std::uint64_t, 6> output_bounds =
        {bounds.z_start, bounds.z_end, bounds.y_start, bounds.y_end, bounds.x_start, bounds.x_end};
    io_.DefineAttribute<std::uint64_t>("vvm_output_bounds_zyx",
        output_bounds.data(),
        output_bounds.size());
...
std::string
Bp5HistoryWriter::describe_field(const std::string& field_name,
    const Core::FieldMetadata& metadata) {
    std::string description = metadata.long_name.empty() ? field_name : metadata.long_name;
    if (!metadata.units.empty()) {
        description += " (" + metadata.units + ")";
    }
    return description;
}

void
Bp5HistoryWriter::write_grads_ctl_file(const Utils::ConfigurationManager& config) {
    const bool use_taiwanvvm_coordinates =
        (grid_.vertical_specification().type == Core::VerticalCoordinateType::TaiwanVVM);
    // Collective on every rank; only rank 0 writes the descriptor.
    const auto axes = grads_horizontal_axes(grid_, state_, use_taiwanvvm_coordinates, comm_);
    if (rank_ != 0) {
        return;
    }
    ...
    ctl.time_count =
        existing_steps_ + static_cast<std::size_t>(std::max(0.0,
                              intervals_before(total_time) - intervals_before(start_time)));
    if (config.get_value<bool>("output.output_initial_step", true)) {
        ++ctl.time_count;
    }
...
void
Bp5HistoryWriter::prepare_coordinates() {
    if (rank_ != 0) {
        return;
    }
    const auto& region = schema_.grid();
    ...
    if (grid_.geometry().kind() == Core::Geometry::GeometryKind::RegularLatLon) {
        ...
        for (std::size_t i = 0; i < region.global_nx; ++i) {
            x_coordinates_[i] = (geometry.regular_lat_lon.longitude_west_edge +
                                    (static_cast<VVM::Real>(i) + real(.5)) * geometry.dq1) *
                                degrees;
        }
        for (std::size_t j = 0; j < region.global_ny; ++j) {
            y_coordinates_[j] = (geometry.regular_lat_lon.latitude_south_edge +
                                    (static_cast<VVM::Real>(j) + real(.5)) * geometry.dq2) *
                                degrees;
        }
    }
...
void
Bp5HistoryWriter::validate_coverage() {
    const auto check = [&](std::size_t dimensions, std::size_t expected) {
        const FieldSelection selection = schema_.selection(dimensions, 1);
        const unsigned long long local = static_cast<unsigned long long>(selection.elements());
        unsigned long long global = 0;
        MPI_Allreduce(&local, &global, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM, comm_);
        if (global != expected) {
            throw std::runtime_error(
                "BP5 rank selections do not cover the requested " + std::to_string(dimensions) +
                "-D output domain exactly once (covered " + std::to_string(global) + ", expected " +
...
void
Bp5HistoryWriter::print_configuration() const {
    if (rank_ != 0) {
        return;
    }
    std::cout << "  [BP5] Direct compute-rank history output\n"
              ...
              << "', model VVM::Real is " << (sizeof(VVM::Real) == 8 ? "float64" : "float32")
              << ")\n"
              << "  [BP5] Field buffer mode: " << cpu_buffer_mode_name(effective_buffer_mode_);
    ...
    if (narrowed_restart_source_) {
        std::cout << "  [BP5] WARNING: narrowed output is a restart source; "
                     "restarts from this dataset lose precision."
                  << std::endl;
    }
}

void
Bp5HistoryWriter::write(std::size_t step, VVM::Real time) {
    if (closed_) {
        throw std::logic_error("Cannot write to a closed BP5 history dataset.");
    }
    ...
    try {
...

        {
            ...
            for (std::size_t i = 0; i < fields_.size(); ++i) {
                if (fields_[i].selection.empty()) {
                    continue;
                }
                if (inputs[i].data == nullptr) {
                    throw std::logic_error(
                        "BP5 field source returned a null pointer for '" + fields_[i].name + "'.");
                }
                std::visit([&](auto& variable) {
                    ...
                    if constexpr (std::is_same_v<VariableType, adios2::Variable<float>>) {
                        writer_.Put(variable, static_cast<const float*>(inputs[i].data), put_mode);
                    }
                    else {
    ...
    MPI_Reduce(local.data(),
        ...
        comm_);
    if (rank_ == 0) {
        std::cout << "  [BP5] Step " << steps_written_ - 1 << " model_step=" << step
                  << " prepare_max=" << maximum[0] << " s"
                  << " put_max=" << maximum[1] << " s"
                  << " end_step_max=" << maximum[2] << " s" << std::endl;
    }
}

void
Bp5HistoryWriter::close() {
    if (closed_) {
        return;
    }
    const auto start = std::chrono::steady_clock::now();
    ...
    if (rank_ == 0) {
        std::cout << "  [BP5] Closed " << dataset_path_.string() << " after " << steps_written_
                  << " step(s); close_max=" << maximum << " s" << std::endl;
    }
}

[[noreturn]] void
Bp5HistoryWriter::throw_operation(const char* operation, const std::exception& error) const {
    throw std::runtime_error("BP5 rank " + std::to_string(rank_) + " failed to " + operation +
