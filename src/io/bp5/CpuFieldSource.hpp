...
public:
    ...
    static void
    pack_view(const View& view, const FieldSelection& selection, std::vector<Out>& output) {
        ...
        if constexpr (dimensions == 1) {
            for (std::size_t k = 0; k < selection.count[0]; ++k) {
                output[out++] = static_cast<Out>(view(selection.memory_start[0] + k));
            }
        }
            ...
            for (std::size_t j = 0; j < selection.count[0]; ++j) {
                for (std::size_t i = 0; i < selection.count[1]; ++i) {
                    output[out++] = static_cast<Out>(
                        view(selection.memory_start[0] + j, selection.memory_start[1] + i));
                }
            }
        }
                ...
                for (std::size_t k = 0; k < selection.count[1]; ++k) {
                    for (std::size_t j = 0; j < selection.count[2]; ++j) {
                        for (std::size_t i = 0; i < selection.count[3]; ++i) {
                            output[out++] = static_cast<Out>(view(selection.memory_start[0] + c,
