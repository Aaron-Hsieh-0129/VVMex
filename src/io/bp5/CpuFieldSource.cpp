...
FieldInput
CpuFieldSource::prepare(const std::string& field_name,
        ...
        if constexpr (std::is_same_v<FieldType, std::monostate>) {
            ...
            for (std::size_t d = 0; d < selection.memory_count.size(); ++d) {
                if (view.extent(d) != selection.memory_count[d]) {
                    throw std::invalid_argument("BP5 field '" + field_name + "' extent " +
                                                std::to_string(d) +
            ...
            if (mode == CpuBufferMode::Direct) {
                ...
                throw std::logic_error(
                    "BP5 direct mode cannot expose CUDA device memory for field '" + field_name +
                    "'; the writer must resolve CUDA output to host packing.");
#else
                ...
                if (element_type == OutputElementType::Float32) {
                    auto& buffer = buffers_.require<float>(field_name, selection.elements());
                    pack_view(source, selection, buffer);
                    result.data = buffer.data();
                }
                else {
                    auto& buffer = buffers_.require<double>(field_name, selection.elements());
                    pack_view(source, selection, buffer);
                    result.data = buffer.data();
                }
                result.location = FieldMemoryLocation::Host;
