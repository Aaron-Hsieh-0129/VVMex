...
std::string
describe(const adios2::Dims& dims) {
    std::string text = "{";
...
adios2::Dims
expected_shape(std::size_t dimensions, const Core::Grid& grid) {
...
public:
    OpenDataset(const std::string& path, MPI_Comm comm, adios2::ADIOS& adios, const char* io_name)
        ...
        if (steps_ == 0) {
            throw std::runtime_error(
                std::string("[") + kTag + "] Restart dataset '" + path + "' contains no steps.");
        }
    }
...
Bp5RestartReader::Bp5RestartReader(const std::string& dataset_path,
    ...
    : dataset_path_(dataset_path), grid_(grid), params_(params), config_(config),
      halo_exchanger_(halo_exchanger), comm_(grid.get_cart_comm()) {
    MPI_Comm_rank(comm_, &rank_);
}

std::size_t
Bp5RestartReader::resolve_step(std::size_t available_steps) const {
    const std::int64_t requested = config_.get_value<std::int64_t>("restart.step_index", -1);
    if (requested == -1) {
        return available_steps - 1;
    }
    if (requested < 0 || static_cast<std::size_t>(requested) >= available_steps) {
        throw std::runtime_error(std::string("[") + kTag + "] restart.step_index " +
...
void
Bp5RestartReader::read_typed_field(
    ...
    if (!variable) {
        throw std::runtime_error(std::string("[") + kTag + "] Variable '" + var_name +
    ...
    if (shape != wanted) {
        throw std::runtime_error(std::string("[") + kTag + "] Variable '" + var_name +
                                 "' has shape " + describe(shape) +
                                 ", which does not match this grid " + describe(wanted) + ".");
    }
    ...
    if constexpr (Dim == 1) {
        ...
        for (std::size_t j = 0; j < ny; ++j) {
            for (std::size_t i = 0; i < nx; ++i) {
                host_view(j + halo, i + halo) = static_cast<VVM::Real>(buffer[j * nx + i]);
            }
        }
    }
    else {
...
Bp5RestartReader::read_field(
    ...
    if (type == "float") {
        ...
        throw std::runtime_error(std::string("[") + kTag + "] Variable '" + var_name +
                                 "' has type '" + type +
                                 "', which is not a restartable field type.");
    }
}

void
Bp5RestartReader::read_and_initialize(Core::State& state) {
    const RestartVariables variables = select_restart_variables(config_, state, rank_, kTag);
    if (variables.empty()) {
        throw std::runtime_error(
    ...
    for (const auto& name : variables.vars_1d) {
        if (state.has_field(name)) {
            halo_exchanger_.exchange_halos(state.get_field<1>(name));
        }
    }
    for (const auto& name : variables.vars_2d) {
        if (state.has_field(name)) {
            halo_exchanger_.exchange_halos(state.get_field<2>(name));
        }
    }
    for (const auto& name : variables.vars_3d) {
        if (state.has_field(name)) {
            halo_exchanger_.exchange_halos(state.get_field<3>(name));
        }
    }
}

VVM::Utils::RestartFileMetadata
Bp5RestartReader::read_restart_metadata() {
    ...
    if (rank_ == 0) {
        if (!metadata.has_time && !metadata.has_step) {
            ...
            std::cout << "  [" << kTag << "] Restart clock read from " << dataset_path_ << " step "
                      << dataset.step() << ":";
            if (metadata.has_time) {
