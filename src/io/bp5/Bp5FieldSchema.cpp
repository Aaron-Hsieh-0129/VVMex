...
std::size_t
checked_size(int value, const char* name) {
    if (value < 0) {
...
normalize_axis(std::int64_t start, std::int64_t end, std::size_t global_size, const char* axis) {
    if (global_size == 0) {
        throw std::invalid_argument(std::string("Global ") + axis + " size must be positive.");
    }
    if (end == -1) {
        end = static_cast<std::int64_t>(global_size) - 1;
    }
    if (start < 0 || end < 0 || start > end || static_cast<std::uint64_t>(end) >= global_size) {
        throw std::invalid_argument(std::string("Invalid output.output_grid ") + axis +
                                    " bounds [" + std::to_string(start) + ", " +
                                    std::to_string(end) + "] for global size " +
...
AxisIntersection
intersect_axis(std::size_t rank_start,
...
bool
FieldSelection::empty() const noexcept {
    return count.empty() || std::any_of(count.begin(), count.end(), [](std::size_t n) {
        return n == 0;
    });
}

std::size_t
FieldSelection::elements() const noexcept {
    if (empty()) {
        return 0;
    }
    return std::accumulate(count.begin(),
        count.end(),
        std::size_t{1},
...
Bp5FieldSchema::Bp5FieldSchema(InclusiveBounds bounds, GridRegion grid, int rank)
    ...
    if (rank_ < 0) {
        throw std::invalid_argument("MPI rank cannot be negative.");
    }
...
Bp5FieldSchema::parse_bounds(
    ...
    const auto x = normalize_axis(config.get_value<std::int64_t>("output.output_grid.x_start", 0),
        config.get_value<std::int64_t>("output.output_grid.x_end", -1),
        nx,
        "x");
    const auto y = normalize_axis(config.get_value<std::int64_t>("output.output_grid.y_start", 0),
        config.get_value<std::int64_t>("output.output_grid.y_end", -1),
        ny,
        "y");
    const auto z = normalize_axis(config.get_value<std::int64_t>("output.output_grid.z_start", 0),
        config.get_value<std::int64_t>("output.output_grid.z_end", -1),
        nz,
        "z");
...
GridRegion
Bp5FieldSchema::from_grid(const Core::Grid& grid) {
    GridRegion result;
...
FieldSelection
Bp5FieldSchema::selection(std::size_t dimensions, std::size_t components) const {
    ...
    const auto x = intersect_axis(grid_.local_x_start,
        grid_.local_nx,
        bounds_.x_start,
        bounds_.x_end,
        grid_.halo);
    const auto y = intersect_axis(grid_.local_y_start,
        grid_.local_ny,
        bounds_.y_start,
        bounds_.y_end,
        grid_.halo);
    const auto z = intersect_axis(grid_.local_z_start,
        grid_.local_nz,
        bounds_.z_start,
        bounds_.z_end,
        grid_.halo);
    ...
    if (dimensions == 1) {
        ...
        result.memory_count = {grid_.local_ny + 2 * grid_.halo, grid_.local_nx + 2 * grid_.halo};
    }
        ...
        result.memory_count = {components,
