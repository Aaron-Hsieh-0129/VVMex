...
std::string
OutputManager::format_to_six_digits(int number) {
    std::stringstream ss;
    ss << std::setfill('0') << std::setw(6) << number;
    return ss.str();
}

OutputManager::OutputManager(const Utils::ConfigurationManager& config,
    ...
    if (config.has_key("dynamics.tracers")) {
        ...
        static const std::unordered_set<std::string> optional_builtin_output_fields = {"qc",
            "qr",
            "qi",
            "qm",
            "nc",
        ...
        for (const auto& field_name : fields_to_output_) {
            if (tracer_config.contains(field_name) && !state_.is_tracer(field_name)) {
                throw std::runtime_error(
                    "Output field '" + field_name + "' names a disabled or unregistered tracer.");
            }
    ...
    if (engine_type_ == "HDF5") {
        const bool use_collective_mpio =
            config.get_value<bool>("output.hdf5_collective_mpio", false);
        const std::string use_collective = use_collective_mpio ? "true" : "false";
        if (rank_ == 0) {
            std::cout << "  [OutputManager] Engine: HDF5. Collective: " << use_collective
                      << std::endl;
        }
        ...
        if (rank_ == 0) {
            ...
            std::cout << "  [OutputManager] SST DataTransport: "
                      << ((data_transport.empty() || data_transport == "AUTO") ? "AUTO"
                                                                               : data_transport)
                      << std::endl;
            std::cout << "  [OutputManager] SST ControlTransport: " << control_transport
                      << std::endl;
        ...
        if (rank_ == 0) {
            std::cout << "  [OutputManager] SST Streaming: " << stream_name << std::endl;
        }
...
void
OutputManager::define_adios_field_metadata(const std::string& field_name,
...
void
OutputManager::define_variables() {
        ...
        if (it != state_.end()) {
            std::visit([&](const auto& field) {
                ...
                if constexpr (!std::is_same_v<T, std::monostate>) {
                    ...
                    size_t local_nx = (actual_out_x_end >= actual_out_x_start)
                                          ? (actual_out_x_end - actual_out_x_start + 1)
                                          : 0;
                    ...
                    size_t local_ny = (actual_out_y_end >= actual_out_y_start)
                                          ? (actual_out_y_end - actual_out_y_start + 1)
                                          : 0;
                    ...
                    size_t local_nz = (actual_out_z_end >= actual_out_z_start)
                                          ? (actual_out_z_end - actual_out_z_start + 1)
                                          : 0;
                    ...
                    if constexpr (T::DimValue == 1) {
                        ...
                        define_field_variable(field_name,
                            {gny, gnx},
                        ...
                        define_field_variable(field_name,
                            {dim4, gnz, gny, gnx},
                            {0, actual_out_z_start, actual_out_y_start, actual_out_x_start},
                            {dim4, local_nz, local_ny, local_nx});
                    }
...
void
OutputManager::define_field_variable(const std::string& field_name,
...
void
OutputManager::put_field(const std::string& field_name, const VVM::Real* data, size_t elements) {
    if (!converts_precision_) {
...
void
OutputManager::write(size_t step, VVM::Real time) {
    ...
    std::string filename;
    if (engine_type_ == "HDF5") {
        filename = output_dir_ + "/" + filename_prefix_ + "_" +
                   format_to_six_digits((int)(time / output_interval_s_)) + ".h5";
        if (rank_ == 0) {
            std::cout << "  [OutputManager] HDF5 Writing: " << filename << std::endl;
        }
        ...
        if (!writer_) {
            ...
            if (rank_ == 0) {
                std::cout << "  [OutputManager] SST Streaming: " << filename << std::endl;
            }
            writer_ = io_.Open(filename, adios2::Mode::Write, comm_);
        }
    }
                    ...
                    if constexpr (!std::is_same_v<T, std::monostate>) {
                        ...
                        if constexpr (T::DimValue == 1) {
                            ...
                            auto subview = Kokkos::subview(full_data_view,
                                std::make_pair(k_start, k_start + count));

                            if (host_buffers_1d_.find(field_name) == host_buffers_1d_.end()) {
                                host_buffers_1d_[field_name] =
                                    Kokkos::View<VVM::Real*, Kokkos::HostSpace>(field_name +
                                                                                    "_host",
                                        count);
                            }
                            ...
                            if (dev_buffers_2d_.find(field_name) == dev_buffers_2d_.end()) {
                                dev_buffers_2d_[field_name] =
                                    Kokkos::View<VVM::Real**, Kokkos::LayoutRight>(field_name +
                                                                                       "_dev",
                                        ny,
                                        nx);
                            }
                            ...
                            if (host_buffers_2d_.find(field_name) == host_buffers_2d_.end()) {
                                host_buffers_2d_[field_name] = Kokkos::View<VVM::Real**,
                                    Kokkos::LayoutRight,
                                    Kokkos::HostSpace>(field_name + "_host", ny, nx);
                            }
                            ...
                            if (host_buffers_3d_.find(field_name) == host_buffers_3d_.end()) {
                                host_buffers_3d_[field_name] = Kokkos::View<VVM::Real***,
                                    Kokkos::LayoutRight,
                                    Kokkos::HostSpace>(field_name + "_host", nz, ny, nx);
                            }
                            ...
                            if (dev_buffers_4d_.find(field_name) == dev_buffers_4d_.end()) {
                                dev_buffers_4d_[field_name] = Kokkos::View<VVM::Real****,
                                    Kokkos::LayoutRight>(field_name + "_dev", d4, nz, ny, nx);
                            }
                            ...
                            if (host_buffers_4d_.find(field_name) == host_buffers_4d_.end()) {
                                host_buffers_4d_[field_name] = Kokkos::View<VVM::Real****,
                                    Kokkos::LayoutRight,
                                    Kokkos::HostSpace>(field_name + "_host", d4, nz, ny, nx);
                            }
...
void
OutputManager::write_static_data() {
    const size_t gnx = grid_.get_global_points_x();
    const size_t gny = grid_.get_global_points_y();
    const size_t gnz = grid_.get_global_points_z();
    const size_t h = grid_.get_halo_cells();
    ...
    if (rank_ == 0) {
        ...
        if (grid_.geometry().kind() == Core::Geometry::GeometryKind::RegularLatLon) {
            ...
            for (size_t i = 0; i < gnx; ++i) {
                x_coords[i] = (geometry.regular_lat_lon.longitude_west_edge +
                                  (real(i) + real(0.5)) * geometry.dq1) *
                              real(180.0) / std::acos(real(-1.0));
            }
        }
    }
    ...
    if (rank_ == 0) {
        y_coords.resize(gny);
        for (size_t i = 0; i < gny; ++i) {
            y_coords[i] = i * grid_.get_dy();
        }
        if (grid_.geometry().kind() == Core::Geometry::GeometryKind::RegularLatLon) {
            const auto& geometry = grid_.horizontal_specification().geometry;
            for (size_t j = 0; j < gny; ++j) {
                y_coords[j] = (geometry.regular_lat_lon.latitude_south_edge +
                                  (real(j) + real(0.5)) * geometry.dq2) *
                              real(180.0) / std::acos(real(-1.0));
            }
        }
    }
...
void
OutputManager::attach_hdf5_field_metadata(const std::string& filename) {
    if (rank_ != 0) {
        return;
    }
    ...
    if (file < 0) {
        std::cerr << "[OutputManager] Failed to reopen HDF5 file '" << filename
                  << "' for metadata output.\n";
        return;
    }

    const auto write_attribute =
        [](hid_t dataset, const std::string& name, const std::string& value) {
        if (value.empty()) {
            return;
        }
    ...
    for (const auto& entry : labelled_datasets) {
        ...
        write_attribute(dataset,
            "units",
            rll && name == "coordinates/x"   ? "degrees_east"
            : rll && name == "coordinates/y" ? "degrees_north"
                                             : entry[1]);
...
OutputManager::grads_ctl_file() {
    ...
    for (int k = h; k < h + nz_phy; ++k) {
        ctl.z_levels.push_back(z_mid_host(k));
    }
...
std::vector<GradsVariable>
OutputManager::grads_variables(const std::string& dataset_prefix, std::size_t levels) const {
...
void
OutputManager::write_static_topo_file() {
    if (rank_ == 0) {
    ...
    auto var_topo = topo_io.DefineVariable<VVM::Real>("topo",
        {gny, gnx},
    ...
    try {
        ...
        Kokkos::View<VVM::Real**, Kokkos::LayoutRight> topo_phys_subview("topo_phys_subview",
            rank_lny,
            rank_lnx);
        ...
        if (rank_ == 0) {
            std::cerr << "Warning: Could not write 'topo': " << e.what() << std::endl;
        }
    }
