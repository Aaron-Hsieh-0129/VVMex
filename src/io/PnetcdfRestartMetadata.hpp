...
inline bool
is_elapsed_seconds_units(std::string units) {
    units.erase(units.begin(), std::find_if(units.begin(), units.end(), [](unsigned char c) {
    ...
    return units == "s" || units == "sec" || units == "secs" || units == "second" ||
           units == "seconds";
}

inline bool
read_pnetcdf_text_attribute(int ncid, int varid, const std::string& name, std::string& out) {
    MPI_Offset length = 0;
    if (ncmpi_inq_attlen(ncid, varid, name.c_str(), &length) != NC_NOERR) {
        return false;
    }
    if (length < 0) {
        return false;
    ...
    if (ncmpi_get_att_text(ncid, varid, name.c_str(), buffer.data()) != NC_NOERR) {
        return false;
    }
...
inline bool
read_pnetcdf_scalar_double(int ncid, const std::string& name, double& out) {
    int varid = -1;
    if (ncmpi_inq_varid(ncid, name.c_str(), &varid) != NC_NOERR) {
        return false;
    }

    int ndims = 0;
    if (ncmpi_inq_varndims(ncid, varid, &ndims) != NC_NOERR) {
        return false;
    }
...
inline bool
read_pnetcdf_scalar_int64(int ncid, const std::string& name, long long& out) {
    int varid = -1;
    if (ncmpi_inq_varid(ncid, name.c_str(), &varid) != NC_NOERR) {
        return false;
    }
...
inline VVM::Utils::RestartFileMetadata
read_pnetcdf_restart_metadata(int ncid) {
    ...
    if (read_pnetcdf_scalar_double(ncid, "model_time_s", time_s)) {
        ...
        if (ncmpi_inq_varid(ncid, "time", &varid) == NC_NOERR &&
            ...
            is_elapsed_seconds_units(units) && read_pnetcdf_scalar_double(ncid, "time", time_s)) {
            metadata.has_time = true;
            metadata.time_s = time_s;
        }
    }
