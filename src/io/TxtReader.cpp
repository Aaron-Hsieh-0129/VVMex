...
TxtReader::TxtReader(const std::string& filepath,
...
void
TxtReader::read_and_initialize(VVM::Core::State& state) {
    int rank = grid_.get_mpi_rank();

    if (raw_data_.empty()) {
        if (rank == 0) {
            std::cerr << "[TxtReader] Warning: No data loaded. Skipping initialization."
                      << std::endl;
        }
        return;
    }
    ...
    if (rank == 0) {
        std::cout << "[TxtReader] Starting Profile Initialization..." << std::endl;
    }
    ...
    if (rank == 0) {
        std::cout << "[TxtReader] Initialization Complete." << std::endl;
    }
}
...
TxtReader::read_file() {
    ...
    if (!infile.is_open()) {
        throw std::runtime_error("[TxtReader] Failed to open profile file '" + source_file_ + "'.");
    }
    ...
    if (line.empty()) {
        throw std::runtime_error("[TxtReader] Profile file '" + source_file_ + "' has no header.");
    }
    ...
    while (std::getline(infile, line)) {
        ...
        for (const auto& h : headers) {
            if (!(ss_data >> val)) {
                throw std::runtime_error("[TxtReader] Profile file '" + source_file_ + "', line " +
    ...
    if (raw_data_.find("pbar") == raw_data_.end()) {
        throw std::runtime_error(
            "[TxtReader] Profile file '" + source_file_ + "' is missing required column 'pbar'.");
    }

    if (raw_data_.find("qvbar") == raw_data_.end()) {
        throw std::runtime_error(
            "[TxtReader] Profile file '" + source_file_ + "' is missing required column 'qvbar'.");
    }
    ...
    if (raw_data_.at("pbar").size() < 2) {
        throw std::runtime_error(
            "[TxtReader] Profile file '" + source_file_ + "' must contain at least two data rows.");
    }

    bool enable_constant_wind =
        config_.get_value<bool>("initial_conditions.constant_upper_wind.enable", false);
    if (enable_constant_wind) {
        VVM::Real p_threshold = config_.get_value<VVM::Real>(
        ...
        if (has_u && has_v) {
            ...
            int rank = grid_.get_mpi_rank();
            if (rank == 0 && ik1 != -1) {
                std::cout << "[TxtReader] Applied constant upper wind (U, V) above " << p_threshold
                          << " Pa (Data Index: " << ik1 << ")" << std::endl;
            }
        }
    }
}
...
void
TxtReader::calculate_input_heights() {
...
void
TxtReader::initialize_thermodynamics(VVM::Core::State& state) {
    ...
    const std::string v_coord_type =
        VVM::Core::vertical_coordinate_type_to_string(grid_.vertical_specification().type);
    ...
    // and off by default.
    bool rcemip_ref_fix =
        config_.get_value<bool>("initial_conditions.rcemip_consistent_reference_state", false);
    ...
    if (v_coord_type == "rcemip") {
        for (int k = h - 1; k < nz; ++k) {
            ...
            pibar(k) =
                rcemip_ref_fix ? std::pow(pbar(k) / P0, RbCp) : std::pow(pbar(k) / P0, 2. / 7.);
            ...
            rhobar(k) =
                rcemip_ref_fix ? pbar(k) / (Rd * Tvbar(k)) : pbar(k) / (Rd * thbar(k) * pibar(k));
        }

        for (int k = h - 1; k < nz - 1; ++k) {
            rhobar_up(k) = rhobar(k) + (rhobar(k + 1) - rhobar(k)) / (z_mid(k + 1) - z_mid(k)) *
                                           (z_up(k) - z_mid(k));
            pibar_up(k) = real(0.5) * (pibar(k) + pibar(k + 1));
        }

        rhobar_up(nz - 1) = rhobar_up(nz - 2) + (rhobar_up(nz - 2) - rhobar_up(nz - 3)) /
                                                    (z_up(nz - 2) - z_up(nz - 3)) *
                                                    (z_up(nz - 1) - z_up(nz - 2));
        ...
        pilog[h] =
            pilog[h - 1] - GDZBCP / (Tvbar(h - 1) + Tvbar(h)) * (z_mid(h) - z_up(h - 1)) / dz;
        for (int k = h + 1; k < nz; ++k) {
            pilog[k] =
                pilog[k - 1] - GDZBCP / (Tvbar(k - 1) + Tvbar(k)) / flex_height_coef_up(k - 1);
        }
...
void
TxtReader::initialize_forcing(VVM::Core::State& state) {
    const int nz = grid_.get_local_total_points_z();
    const int h = grid_.get_halo_cells();
    ...
    if (has_q1) {
        if (!state.has_field("Q1")) {
            state.add_field<1>("Q1", {nz});
        }
        Q1LS = state.get_field<1>("Q1").get_mutable_device_data();
        Q1LS_h = Kokkos::create_mirror_view(Q1LS);
    }
    if (has_q2) {
        if (!state.has_field("Q2")) {
            state.add_field<1>("Q2", {nz});
        }
    ...
    for (int k = h - 1; k < nz; ++k) {
        if (v_coord_type == "rcemip") {
            ...
            if (idx < n_in) {
                if (has_u) {
                    u_val = raw_data_.at("U")[idx];
                }
                if (has_v) {
                    v_val = raw_data_.at("V")[idx];
                }
                if (has_q1) {
                    q1_val = raw_data_.at("Q1")[idx];
                }
                if (has_q2) {
                    q2_val = raw_data_.at("Q2")[idx];
                }
            }
            else {
                if (n_in >= 2) {
                    ...
                    if (has_u) {
                        u_val = raw_data_.at("U")[last] +
                                (raw_data_.at("U")[last] - raw_data_.at("U")[prev]) * diff;
                    }
                    if (has_v) {
                        v_val = raw_data_.at("V")[last] +
                                (raw_data_.at("V")[last] - raw_data_.at("V")[prev]) * diff;
                    }
                    if (has_q1) {
                        q1_val = raw_data_.at("Q1")[last] +
                                 (raw_data_.at("Q1")[last] - raw_data_.at("Q1")[prev]) * diff;
                    }
                    if (has_q2) {
                        q2_val = raw_data_.at("Q2")[last] +
                                 (raw_data_.at("Q2")[last] - raw_data_.at("Q2")[prev]) * diff;
                    }
                }
                else {
                    if (has_u) {
                        u_val = raw_data_.at("U")[0];
                    }
                    if (has_v) {
                        v_val = raw_data_.at("V")[0];
                    }
                    if (has_q1) {
                        q1_val = raw_data_.at("Q1")[0];
                    }
                    if (has_q2) {
                        q2_val = raw_data_.at("Q2")[0];
                    }
                }
            }
            ...
            if (has_u) {
                U_h(k) = interpolate(p_target, P_in, raw_data_.at("U"), true);
            }
            if (has_v) {
                V_h(k) = interpolate(p_target, P_in, raw_data_.at("V"), true);
            }
    ...
    if (v_coord_type != "rcemip") {
        ...
        if (has_q1) {
            Q1LS_h(h - 1) = Q1LS_h(h);
            Q1LS_h(nz - 1) = Q1LS_h(nz - 2);
        }
        if (has_q2) {
            Q2LS_h(h - 1) = Q2LS_h(h);
            Q2LS_h(nz - 1) = Q2LS_h(nz - 2);
        }
    }
...
VVM::Real
TxtReader::interpolate(VVM::Real target_x,
    const std::vector<VVM::Real>& x_vec,
    const std::vector<VVM::Real>& y_vec,
    bool is_pressure_coord) const {
    size_t n = x_vec.size();
    if (n == 0) {
        return 0.0;
    }
