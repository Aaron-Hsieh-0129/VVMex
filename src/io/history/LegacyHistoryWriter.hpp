...
public:
    LegacyHistoryWriter(const Utils::ConfigurationManager& config,
        const Core::Grid& grid,
        const Core::Parameters& parameters,
        Core::State& state,
        MPI_Comm comm)
        : manager_(std::make_unique<OutputManager>(config, grid, parameters, state, comm)) {}

    void
    write(std::size_t step, VVM::Real time) override {
        if (!manager_) {
