...
inline hid_t
open_restart_scalar_dataset(hid_t file_id, const std::string& name) {
    hid_t dataset = -1;
    H5E_BEGIN_TRY {
...
inline bool
read_restart_scalar(hid_t file_id, const std::string& name, hid_t mem_type, void* out) {
...
inline bool
read_restart_scalar_double(hid_t file_id, const std::string& name, double& out) {
    return read_restart_scalar(file_id, name, H5T_NATIVE_DOUBLE, &out);
}

inline bool
read_restart_scalar_int64(hid_t file_id, const std::string& name, long long& out) {
...
hdf5_restart_time_variable_names() {
    static const std::vector<std::string> names = {"model_time_s", "time"};
    return names;
}

inline const std::vector<std::string>&
hdf5_restart_step_variable_names() {
...
read_hdf5_restart_metadata(hid_t file_id) {
...
inline bool
read_hdf5_restart_metadata(const std::string& path, VVM::Utils::RestartFileMetadata& metadata) {
    hid_t file_id = -1;
    H5E_BEGIN_TRY {
