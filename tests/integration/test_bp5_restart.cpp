...
hid_t
open_hdf5_dataset(hid_t file, const std::string& name) {
    const std::string path = "/Step0/" + name;
    hid_t dataset = -1;
    H5E_BEGIN_TRY {
        dataset = H5Dopen2(file, path.c_str(), H5P_DEFAULT);
    }
    H5E_END_TRY;
    if (dataset < 0) {
...
template <typename T>
std::vector<T>
read_hdf5(hid_t file, const std::string& name, adios2::Dims& shape) {
...
void
compare_any_field(adios2::IO& io,
    adios2::Engine& reader,
    std::size_t step,
    hid_t h5_file,
    const std::string& name) {
    ...
    if (type == "float") {
        compare_field<float>(io, reader, step, h5_file, name);
    }
    else if (type == "double") {
        compare_field<double>(io, reader, step, h5_file, name);
    }
    else {
        check(false, name + " has a comparable float type in the BP5 dataset (got '" + type + "')");
    }
}
} // namespace
...
main(int argc, char** argv) {
    if (argc < 4) {
        std::fprintf(stderr, "usage: test_bp5_restart BP5_DATASET STEP RESUMED_HDF5 [field ...]\n");
        return 2;
    }
    ...
    try {
        ...
        if (model_time && model_step) {
            ...
            check(bp_time == h5_time,
                "recovered model_time_s matches the source step (" + std::to_string(h5_time) +
                    " vs " + std::to_string(bp_time) + ")");
            check(bp_step == h5_step,
                "recovered model_step matches the source step (" + std::to_string(h5_step) +
                    " vs " + std::to_string(bp_step) + ")");
        }
    ...
    if (failures == 0) {
        std::puts("test_bp5_restart: PASS");
    }
