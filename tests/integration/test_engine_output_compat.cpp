...
std::size_t
elements(const adios2::Dims& shape) {
...
template <typename T>
T
read_hdf5_scalar(hid_t file, const std::string& name) {
    const std::string path = "/Step0/" + name;
    const hid_t dataset = H5Dopen2(file, path.c_str(), H5P_DEFAULT);
    if (dataset < 0) {
        throw std::runtime_error("Cannot open HDF5 dataset " + path);
    }
    T value{};
    const herr_t result = H5Dread(dataset, hdf5_type<T>(), H5S_ALL, H5S_ALL, H5P_DEFAULT, &value);
    H5Dclose(dataset);
    if (result < 0) {
...
template <typename T>
std::pair<adios2::Dims, std::vector<T>>
read_hdf5_array(hid_t file, const std::string& name) {
    const std::string path = "/Step0/" + name;
    const hid_t dataset = H5Dopen2(file, path.c_str(), H5P_DEFAULT);
    if (dataset < 0) {
        throw std::runtime_error("Cannot open HDF5 dataset " + path);
...
std::string
read_hdf5_attribute(hid_t file, const std::string& name) {
    ...
    H5Aclose(attribute);
    if (result < 0) {
        throw std::runtime_error("Cannot read HDF5 attribute " + name);
    }
    return value.data();
}
...
void
compare_scalar(
    adios2::IO& bp_io, adios2::Engine& bp_reader, hid_t h5_file, const std::string& name) {
...
void
compare_attribute(
    adios2::IO& bp_io, hid_t h5_file, const std::string& variable, const std::string& name) {
    ...
    if (bp_attr && !h5_attr.empty()) {
        check(bp_attr.Data().at(0) == h5_attr, variable + "/" + name + " metadata is identical");
    }
}
std::filesystem::path
step_file(const std::string& directory, int step) {
    char filename[64];
...
hid_t
open_step(const std::string& directory, int step) {
...
herr_t
collect_attribute(hid_t, const char* name, const H5A_info_t*, void* data) {
...
void
compare_bp5_against_hdf5(const std::string& bp5_dataset,
    ...
    while (bp_reader.BeginStep() == adios2::StepStatus::OK) {
        ...
        for (const std::string name :
            {"coordinates/x", "coordinates/y", "coordinates/z_mid", "thbar", "topo", "u"}) {
...
void
compare_hdf5_metadata(const std::string& a, const std::string& b, int steps) {
    for (int step = 0; step < steps; ++step) {
        ...
        check(attributes_a == attributes_b,
            "step " + std::to_string(step) + ": HDF5 and SST files carry identical attributes");
...
main(int argc, char** argv) {
    ...
    try {
        ...
        if (argc == 4) {
            ...
            check(sst_steps == hdf5_steps, "the SST relay wrote the same number of history times");
    ...
    if (failures == 0) {
        std::puts("test_engine_output_compat: PASS");
    }
