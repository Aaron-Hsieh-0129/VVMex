...
inline double
expected_3d(int k, int j, int i) {
    return 300.0 + 0.0001 * (10000 * k + 100 * j + i);
}

inline void
fill_fields(VVM::Core::State& state, const VVM::Core::Grid& grid) {
    ...
    Kokkos::parallel_for("fill_1d", Kokkos::RangePolicy<>(h, nz - h), KOKKOS_LAMBDA(const int k) {
        thbar(k) = VVM::real(k - h);
    });
    ...
    Kokkos::parallel_for("fill_3d",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            u(k, j, i) =
                VVM::real(300.0) +
                VVM::real(0.0001) * VVM::real(10000 * (k - h) + 100 * (j0 + j - h) + (i0 + i - h));
        });
...
fill_coordinates(VVM::Core::Parameters& parameters, const VVM::Core::Grid& grid) {
    ...
    Kokkos::parallel_for("fill_z", Kokkos::RangePolicy<>(h, nz - h), KOKKOS_LAMBDA(const int k) {
        z(k) = VVM::real(50 + 25 * (k - h));
    });
...
make_config(const std::filesystem::path& base_config,
    ...
    config["output"]["fields_to_output"] = nlohmann::json::array({"thbar", "topo", "u"});
...
inline std::size_t
dataset_element_size(hid_t file, const std::string& path) {
...
read_dataset(hid_t file, const std::string& path) {
...
inline bool
narrowed_on_disk(const std::string& precision) {
...
inline void
inspect(const std::filesystem::path& file_path, const std::string& precision) {
    ...
    for (int k = 0; k < kNz && values_match; ++k) {
        for (int j = 0; j < kNy && values_match; ++j) {
            for (int i = 0; i < kNx; ++i) {
                const std::size_t index = (static_cast<std::size_t>(k) * kNy + j) * kNx + i;
                if (std::fabs(u[index] - expected_3d(k, j, i)) > tolerance) {
                    std::fprintf(stderr,
                        "  u(%d,%d,%d): got %.9g want %.9g\n",
                        k,
                        j,
                        i,
                        u[index],
