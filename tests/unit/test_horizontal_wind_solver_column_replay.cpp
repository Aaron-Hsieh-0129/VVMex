...
void
require_mpi(int status, const char* operation) {
...
void
append_values(std::vector<Real>& values, const Field<Dim>& field) {
...
bool
all_finite(const std::vector<Real>& values) {
...
struct Inputs {
    ...
    Inputs(int ny, int nx)
        : rhs_psi("chain_rhs_psi", {ny, nx}), rhs_chi("chain_rhs_chi", {ny, nx}),
          current_psi("chain_current_psi", {ny, nx}), current_chi("chain_current_chi", {ny, nx}),
          previous_psi("chain_previous_psi", {ny, nx}),
          previous_chi("chain_previous_chi", {ny, nx}), w("chain_w", {nz, ny, nx}),
          omega1("chain_omega1", {nz, ny, nx}), omega2("chain_omega2", {nz, ny, nx}),
    ...
    initialize(const Grid& grid, int replay) {
        ...
        for (int j = 0; j < ny; ++j) {
            ...
            const Real angle_y =
                two_pi * (y + real(0.5)) / static_cast<Real>(grid.get_global_points_y());

            for (int i = 0; i < nx; ++i) {
                const Real x = static_cast<Real>(grid.get_local_physical_start_x() + i - h);
                const Real angle_z =
                    two_pi * (x + real(1.0)) / static_cast<Real>(grid.get_global_points_x());
                const Real angle_t =
                    two_pi * (x + real(0.5)) / static_cast<Real>(grid.get_global_points_x());

                host[0](j, i) = factor * real(1e-10) * std::sin(angle_z);
                host[1](j, i) = factor * real(-6e-11) * std::cos(real(2.0) * angle_t);

                host[2](j, i) =
                    factor * real(1000.0) * (std::sin(angle_z) * std::cos(angle_y) + real(0.3));
                host[3](j, i) =
                    factor * real(700.0) * (std::cos(angle_t) * std::sin(angle_y) - real(0.2));

                host[4](j, i) =
                    factor * real(800.0) * (std::cos(angle_z) * std::cos(angle_y) - real(0.1));
                host[5](j, i) =
                    factor * real(400.0) * (std::sin(angle_t) * std::sin(angle_y) + real(0.15));

                for (int k = 0; k < nz; ++k) {
                    const Real level = static_cast<Real>(k);

                    w_host(k, j, i) = factor * real(0.1) *
                                      ((real(1.0) + real(0.125) * level) * std::sin(angle_t) +
                                          real(0.3) * std::cos(angle_y));
                    omega1_host(k, j, i) =
                        factor * real(2e-4) / radius *
                        (real(1.0) + real(0.125) * level + real(0.2) * std::cos(angle_t));
                    omega2_host(k, j, i) =
                        factor * real(-3e-4) / radius *
                        (real(1.0) + real(0.0625) * level + real(0.1) * std::sin(angle_y));
                }
            }
        }
        ...
        for (int k = 0; k < nz - 1; ++k) {
            spacing_host(k) = replay == 0 ? real(100.0)
                                          : real(40.0) * static_cast<Real>(k + 1) +
                                                real(10.0) * static_cast<Real>(replay);
        }
...
struct Outputs {
    ...
    Outputs(const std::string& prefix, int ny, int nx)
        : psi(prefix + "_psi", {ny, nx}), chi(prefix + "_chi", {ny, nx}),
          wind1(prefix + "_wind1", {nz, ny, nx}), wind2(prefix + "_wind2", {nz, ny, nx}) {}
    ...
    bool
    check_regions(const Grid& grid, int bottom, bool require_zero) const {
        ...
        for (int k = 0; k < nz; ++k) {
            for (int j = 0; j < ny; ++j) {
                for (int i = 0; i < nx; ++i) {
                    const bool written =
                        k >= bottom && k <= top && j >= h && j < end_j && i >= h && i < end_i;
                    ...
                    if (!written) {
                        ...
                        valid =
                            valid && first(k, j, i) == real(0.0) && second(k, j, i) == real(0.0);
                    }
                }
            }
        }
...
struct TestGraph {
    ...
    finish() {
        ...
        require_cuda(cudaGraphGetNodes(graph, nullptr, &node_count),
            "Read solver-column graph nodes");
        ...
        require_cuda(cudaGraphInstantiate(&executable, graph, nullptr, nullptr, 0),
            "Instantiate solver-column graph");
    }
...
run_case(const Grid& grid, HaloExchanger& halo, int iterations, int bottom) {
    ...
    static_assert(std::is_same_v<Kokkos::DefaultExecutionSpace, Kokkos::Cuda>,
        "This NCCL replay test requires Kokkos::Cuda.");
    ...
    try {
        ...
        for (int replay = 0; replay < 4; ++replay) {
            ...
            const bool regions = direct.check_regions(grid, bottom, replay == 3) &&
                                 replayed.check_regions(grid, bottom, replay == 3);
            const bool updated = replay == 0 || !same_bits(previous_result, actual);
            const bool finite = all_finite(expected) && all_finite(actual);

            std::array<int, 5> local_flags = {exact ? 1 : 0,
                inputs ? 1 : 0,
                regions ? 1 : 0,
                updated ? 1 : 0,
                finite ? 1 : 0};
            ...
            require_mpi(MPI_Allreduce(local_flags.data(),
                            global_flags.data(),
                            static_cast<int>(global_flags.size()),
                            MPI_INT,
                            MPI_MIN,
                            comm),
                "Reduce replay checks");

            const bool passed =
                std::all_of(global_flags.begin(), global_flags.end(), [](int value) {
                return value == 1;
            });

            if (grid.get_mpi_rank() == 0) {
                const char* geometry_name = grid.geometry().kind() == GeometryKind::Cartesian
                                                ? "cartesian"
                                                : "regular_latlon";

                std::printf("%s ranks=%d execution=%s iterations=%d bottom=%d replay=%d exact=%d "
                            "inputs=%d regions=%d updated=%d finite=%d %s\n",
                    geometry_name,
                    grid.get_mpi_size(),
                    execution_name,
                    iterations,
...
int
run_tests(const Grid& grid, HaloExchanger& halo) {
    int failures = 0;
    ...
    try {
        ...
        if (argc != 2) {
            throw std::runtime_error(
                "Usage: test_horizontal_wind_solver_column_replay <configuration.json>");
        }
...

        {
            ...
            require_mpi(MPI_Bcast(&id, static_cast<int>(sizeof(id)), MPI_BYTE, 0, MPI_COMM_WORLD),
                "Broadcast NCCL unique ID");

            ncclComm_t nccl_comm = nullptr;
            require_nccl(ncclCommInitRank(&nccl_comm, ranks, id, rank),
        ...
        require_mpi(MPI_Allreduce(&failures, &global_failures, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD),
