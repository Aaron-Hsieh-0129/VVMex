...
test_explicit_metric_weighting() {
    ...
    Kokkos::parallel_for("EvaluateMetricWeightedDivergence",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
            result() = divergence.at_t(0,
                0,
                VVM::real(7.0),
                VVM::real(1.0),
                VVM::real(6.0),
    ...
    const VVM::Real expected =
        VVM::real(0.25) * (VVM::real(3.0) * (VVM::real(7.0) - VVM::real(1.0)) / VVM::real(2.0) +
                              VVM::real(5.0) * (VVM::real(6.0) - VVM::real(2.0)) / VVM::real(4.0));

    check(close(host_result, expected),
...
test_cartesian_equivalence() {
    ...
    Kokkos::parallel_for("InitializeCartesianFluxes",
    ...
    Kokkos::parallel_for("CompareCartesianFluxDivergence",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            actual(j, i) = divergence.at_t(j,
                i,
                flux_q1(j, i),
                flux_q1(j, i - 1),
                flux_q2(j, i),
            ...
            legacy(j, i) =
                (flux_q1(j, i) - flux_q1(j, i - 1)) / dx + (flux_q2(j, i) - flux_q2(j - 1, i)) / dy;
        });
    ...
    for (int j = h; j < ny - h; ++j) {
        for (int i = h; i < nx - h; ++i) {
            maximum_difference =
                std::max(maximum_difference, std::abs(actual_host(j, i) - legacy_host(j, i)));
        }
    }

    check(maximum_difference <= VVM::real(1.0e-6),
...
test_constant_flux() {
    ...
    Kokkos::parallel_for("EvaluateConstantFluxDivergence",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
            result() = divergence.at_t(layout.halo,
                layout.halo,
                VVM::real(4.0),
                VVM::real(4.0),
                VVM::real(-3.0),
    ...
    check(close(host_result, VVM::real(0.0)),
...
test_linear_flux() {
    ...
    Kokkos::parallel_for("InitializeLinearFlux",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
    ...
    Kokkos::parallel_for("EvaluateLinearFluxDivergence",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            result(j, i) = divergence.at_t(j,
                i,
                flux_q1(j, i),
                flux_q1(j, i - 1),
                flux_q2(j, i),
...
periodic_sine_error(const int n) {
    ...
    Kokkos::parallel_for("InitializePeriodicSineFlux",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
    ...
    Kokkos::parallel_for("EvaluatePeriodicSineDivergence",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            result(j, i) = divergence.at_t(j,
                i,
...
test_periodic_global_flux_integral() {
    ...
    Kokkos::parallel_for("InitializePeriodicIntegralFlux",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            flux_q1(j, i) = Kokkos::sin(VVM::real(2.0) * u.q1(j, i)) +
                            VVM::real(0.37) * Kokkos::cos(VVM::real(3.0) * u.q2(j, i));

            flux_q2(j, i) = VVM::real(0.41) * Kokkos::sin(VVM::real(4.0) * v.q1(j, i)) -
                            VVM::real(0.65) * Kokkos::cos(VVM::real(2.0) * v.q2(j, i));
        });

    Kokkos::parallel_for("EvaluatePeriodicIntegralDivergence",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            divergence_at_t(j, i) = divergence.at_t(j,
                i,
                flux_q1(j, i),
                flux_q1(j, i - 1),
                flux_q2(j, i),
    ...
    Kokkos::parallel_reduce("IntegratePeriodicDivergence",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i, VVM::Real& sum) {
            const VVM::Real cell_area = t.sqrt_g(j, i) * t.dq1 * t.dq2;
    ...
    const VVM::Real integral_tolerance =
        sizeof(VVM::Real) == sizeof(double) ? VVM::real(1.0e-10) : VVM::real(1.0e-4);

    check(std::abs(area_integral_host) <= integral_tolerance,
...
test_takacs_face_reconstruction() {
    ...
    Kokkos::View<VVM::Real*> result("takacs_face_flux_result", 2);

    Kokkos::parallel_for("EvaluateTakacsFaceFlux",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
            result(0) = face_flux.calculate(VVM::real(4.0),
                VVM::real(9.0),
                VVM::real(-16.0),
                VVM::real(1.0),
                VVM::real(2.0),
            ...
            result(1) = face_flux.calculate(VVM::real(16.0),
                VVM::real(-9.0),
                VVM::real(-4.0),
                VVM::real(1.0),
                VVM::real(2.0),
    ...
    check(close(host(0), VVM::real(28.875)),
        "Positive Takacs face reconstruction must match the CVVM formula");

    check(close(host(1), VVM::real(-30.375)),
...
test_cartesian_takacs_linear_scalar() {
    ...
    const CartesianGeometry geometry(layout, dx, dy);
    const auto transport = make_takacs_scalar_transport_device_view(geometry);

    const int nx = layout.local_total_nx();
    const int ny = layout.local_total_ny();
    const int h = layout.halo;

    Kokkos::View<VVM::Real***> scalar_q("cartesian_takacs_scalar", 1, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_q1("cartesian_takacs_mass_flux_q1", 1, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_q2("cartesian_takacs_mass_flux_q2", 1, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_vertical("cartesian_takacs_mass_flux_vertical", 1, ny, nx);
    Kokkos::View<VVM::Real***> tendency("cartesian_takacs_tendency", 1, ny, nx);

    Kokkos::parallel_for("InitializeCartesianTakacsLinearScalar",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            const VVM::Real x = static_cast<VVM::Real>(i - h) * dx;
            const VVM::Real y = static_cast<VVM::Real>(j - h) * dy;

            scalar_q(0, j, i) = VVM::real(3.0) + slope_q1 * x + slope_q2 * y;

            mass_flux_q1(0, j, i) = density * velocity_q1;
            mass_flux_q2(0, j, i) = density * velocity_q2;
            mass_flux_vertical(0, j, i) = VVM::real(0.0);
        });

    Kokkos::parallel_for("EvaluateCartesianTakacsLinearScalar",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            tendency(0, j, i) = transport.calculate_tendency_at_t(scalar_q,
                mass_flux_q1,
                mass_flux_q2,
                mass_flux_vertical,
                0,
    ...
    const auto tendency_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), tendency);

    const VVM::Real expected = -(velocity_q1 * slope_q1 + velocity_q2 * slope_q2);

    VVM::Real maximum_error = VVM::real(0.0);

    for (int j = h; j < ny - h; ++j) {
        for (int i = h; i < nx - h; ++i) {
            maximum_error = std::max(maximum_error, std::abs(tendency_host(0, j, i) - expected));
        }
    }

    check(maximum_error <= VVM::real(2.0e-5),
...
test_regular_latlon_constant_tracer() {
    ...
    const VVM::Real dlatitude = (pi / VVM::real(3.0)) / static_cast<VVM::Real>(physical_ny);
    const VVM::Real radius = VVM::real(6371220.0);
    const VVM::Real density = VVM::real(1.2);
    const VVM::Real eastward_wind = VVM::real(18.0);
    ...
    Kokkos::View<VVM::Real***> scalar_q("rll_constant_scalar", 1, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_q1("rll_constant_mass_flux_q1", 1, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_q2("rll_constant_mass_flux_q2", 1, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_vertical("rll_constant_mass_flux_vertical", 1, ny, nx);
    Kokkos::View<VVM::Real***> tendency("rll_constant_tendency", 1, ny, nx);

    Kokkos::parallel_for("InitializeRllConstantTracer",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            scalar_q(0, j, i) = VVM::real(7.0);

            mass_flux_q1(0, j, i) =
                density * u_geometry.physical_to_contravariant.a11(j, i) * eastward_wind;

            mass_flux_q2(0, j, i) = VVM::real(0.0);
            mass_flux_vertical(0, j, i) = VVM::real(0.0);
        });

    Kokkos::parallel_for("EvaluateRllConstantTracer",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            tendency(0, j, i) = transport.calculate_tendency_at_t(scalar_q,
                mass_flux_q1,
    ...
    for (int j = h; j < ny - h; ++j) {
        for (int i = h; i < nx - h; ++i) {
            maximum_tendency = std::max(maximum_tendency, std::abs(tendency_host(0, j, i)));
        }
    }

    check(maximum_tendency <= VVM::real(1.0e-7),
...
test_regular_latlon_discrete_mass_budget() {
    ...
    const HorizontalDomainLayout layout = make_layout(physical_nx, physical_ny, halo);
    ...
    Kokkos::View<VVM::Real***> scalar_q("rll_budget_scalar", nz, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_q1("rll_budget_mass_flux_q1", nz, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_q2("rll_budget_mass_flux_q2", nz, ny, nx);
    Kokkos::View<VVM::Real***> mass_flux_vertical("rll_budget_mass_flux_vertical", nz, ny, nx);
    Kokkos::View<VVM::Real***> tendency("rll_budget_tendency", nz, ny, nx);
    Kokkos::View<VVM::Real*> density("rll_budget_density", nz);
    Kokkos::View<VVM::Real*> inverse_vertical_spacing("rll_budget_inverse_vertical_spacing", nz);

    Kokkos::parallel_for("InitializeRllBudgetProfiles",
        Kokkos::RangePolicy<>(0, nz),
        KOKKOS_LAMBDA(const int k) {
            density(k) = VVM::real(1.1) + VVM::real(0.04) * static_cast<VVM::Real>(k - k_begin);

            inverse_vertical_spacing(k) =
                VVM::real(0.20) + VVM::real(0.015) * static_cast<VVM::Real>(k - k_begin);
        });

    Kokkos::parallel_for("InitializeRllBudgetFields",
        ...
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            ...
            const VVM::Real longitude_phase = VVM::real(2.0) * pi *
                                              static_cast<VVM::Real>(global_i) /
                                              static_cast<VVM::Real>(physical_nx);

            const VVM::Real latitude_index = static_cast<VVM::Real>(j - halo);
            const VVM::Real vertical_index = static_cast<VVM::Real>(k - k_begin);

            scalar_q(k, j, i) = VVM::real(2.0) + VVM::real(0.25) * Kokkos::sin(longitude_phase) +
                                VVM::real(0.03) * latitude_index + VVM::real(0.07) * vertical_index;

            mass_flux_q1(k, j, i) = VVM::real(0.8) + VVM::real(0.2) * Kokkos::cos(longitude_phase) +
                                    VVM::real(0.015) * latitude_index +
                                    VVM::real(0.025) * vertical_index;

            const int meridional_face = j - (halo - 1);

            if (meridional_face <= 0 || meridional_face >= physical_ny) {

                mass_flux_q2(k, j, i) = VVM::real(0.0);
            }
            else {
                mass_flux_q2(k, j, i) =
                    VVM::real(0.35) *
                    Kokkos::sin(pi * static_cast<VVM::Real>(meridional_face) /
                                static_cast<VVM::Real>(physical_ny)) *
                    (VVM::real(1.0) + VVM::real(0.2) * Kokkos::sin(longitude_phase));
            }

            const int vertical_face = k - (k_begin - 1);

            if (vertical_face <= 0 || vertical_face >= physical_nz) {

                mass_flux_vertical(k, j, i) = VVM::real(0.0);
            }
            else {
                mass_flux_vertical(k, j, i) =
                    VVM::real(0.18) *
                    Kokkos::sin(pi * static_cast<VVM::Real>(vertical_face) /
                                static_cast<VVM::Real>(physical_nz)) *
                    (VVM::real(1.0) + VVM::real(0.1) * Kokkos::cos(longitude_phase));
            }
        });

    Kokkos::parallel_for("EvaluateRllBudgetTransport",
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({k_begin, halo, halo},
            {k_end, ny - halo, nx - halo}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            tendency(k, j, i) = transport.calculate_tendency_at_t(scalar_q,
                mass_flux_q1,
                mass_flux_q2,
                mass_flux_vertical,
                k,
    ...
    const auto inverse_vertical_spacing_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), inverse_vertical_spacing);
    const auto jacobian_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), t_geometry.sqrt_g.one_dimensional);
        ...
        for (int j = halo; j < ny - halo; ++j) {
            for (int i = halo; i < nx - halo; ++i) {
                const long double cell_volume =
                    static_cast<long double>(jacobian_host(j)) *
                    static_cast<long double>(t_geometry.dq1) *
                    static_cast<long double>(t_geometry.dq2) /
                    static_cast<long double>(inverse_vertical_spacing_host(k));

                const long double contribution = static_cast<long double>(density_host(k)) *
                                                 static_cast<long double>(tendency_host(k, j, i)) *
                                                 cell_volume;

                net_tendency += contribution;
                absolute_tendency += std::abs(contribution);
            }
        }
    }

    const long double relative_budget_error =
        std::abs(net_tendency) / std::max(absolute_tendency, 1.0L);

    const long double tolerance = sizeof(VVM::Real) == sizeof(double) ? 5.0e-11L : 5.0e-5L;

    check(relative_budget_error <= tolerance,
...
test_orthogonal_anelastic_mass_flux_adapter() {
    ...
    const auto transport = make_takacs_scalar_transport_device_view(geometry);
    const auto u_geometry = geometry.device_view(HorizontalLocation::U);
    const auto v_geometry = geometry.device_view(HorizontalLocation::V);

    const int nx = layout.local_total_nx();
    const int ny = layout.local_total_ny();

    Kokkos::View<VVM::Real***> scalar_q("adapter_scalar_q", 1, ny, nx);
    Kokkos::View<VVM::Real***> physical_mass_flux_q1("adapter_physical_mass_flux_q1", 1, ny, nx);
    Kokkos::View<VVM::Real***> physical_mass_flux_q2("adapter_physical_mass_flux_q2", 1, ny, nx);
    Kokkos::View<VVM::Real***> explicit_mass_flux_q1("adapter_explicit_mass_flux_q1", 1, ny, nx);
    Kokkos::View<VVM::Real***> explicit_mass_flux_q2("adapter_explicit_mass_flux_q2", 1, ny, nx);
    Kokkos::View<VVM::Real***> adapted_tendency("adapter_adapted_tendency", 1, ny, nx);
    Kokkos::View<VVM::Real***> explicit_tendency("adapter_explicit_tendency", 1, ny, nx);

    Kokkos::parallel_for("InitializeOrthogonalMassFluxAdapter",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            const VVM::Real i_value = static_cast<VVM::Real>(i - halo);
            const VVM::Real j_value = static_cast<VVM::Real>(j - halo);

            scalar_q(0, j, i) = VVM::real(2.0) + VVM::real(0.03) * i_value -
                                VVM::real(0.02) * j_value + VVM::real(0.001) * i_value * j_value;

            physical_mass_flux_q1(0, j, i) =
                VVM::real(1.4) + VVM::real(0.06) * i_value + VVM::real(0.025) * j_value;

            physical_mass_flux_q2(0, j, i) =
                VVM::real(-0.7) + VVM::real(0.015) * i_value - VVM::real(0.04) * j_value;

            explicit_mass_flux_q1(0, j, i) =
                u_geometry.physical_to_contravariant.a11(j, i) * physical_mass_flux_q1(0, j, i);

            explicit_mass_flux_q2(0, j, i) =
                v_geometry.physical_to_contravariant.a22(j, i) * physical_mass_flux_q2(0, j, i);
        });

    const auto adapted_mass_flux_q1 =
        make_orthogonal_contravariant_mass_flux_q1_device_view(geometry, physical_mass_flux_q1);
    const auto adapted_mass_flux_q2 =
        make_orthogonal_contravariant_mass_flux_q2_device_view(geometry, physical_mass_flux_q2);

    Kokkos::parallel_for("CompareOrthogonalMassFluxAdapter",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({halo, halo}, {ny - halo, nx - halo}),
        KOKKOS_LAMBDA(const int j, const int i) {
            adapted_tendency(0, j, i) =
                transport.calculate_horizontal_flux_convergence_at_t(scalar_q,
    ...
    const auto adapted_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), adapted_tendency);
    const auto explicit_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), explicit_tendency);
    ...
    for (int j = halo; j < ny - halo; ++j) {
        for (int i = halo; i < nx - halo; ++i) {
            maximum_difference = std::max(maximum_difference,
                std::abs(adapted_host(0, j, i) - explicit_host(0, j, i)));
        }
    }

    check(maximum_difference <= VVM::real(1.0e-7),
