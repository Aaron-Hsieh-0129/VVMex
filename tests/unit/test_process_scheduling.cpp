...
void
check_throws(void (*fn)(), const std::string& what) {
    try {
        fn();
    }
...
test_interval_one_runs_every_step() {
    for (std::size_t step = 0; step < 10; ++step) {
        check(is_process_step(step, 1), "interval 1 computes at step " + std::to_string(step));
    }
...
test_legacy_phase_is_reproduced() {
    ...
    for (int n : {3, 5, 15}) {
        check(is_legacy_surface_compute_step(0, n),
            "v1.0.0 computes at step 0 for interval " + std::to_string(n) + " (SIZE_MAX % N == 0)");
    }
    ...
    for (int n : {3, 5, 15, 24, 120}) {
        ...
        for (std::size_t k = 0; k < 4; ++k) {
            check(is_legacy_surface_compute_step(1 + k * un, n),
                "v1.0.0 computes at step " + std::to_string(1 + k * un) + " for interval " +
                    std::to_string(n));
        }
...
test_pending_fix_changes_only_the_phase() {
    for (int n : {3, 5, 15, 24, 120}) {
        ...
        check(disagreements > 0, "the conventions differ for interval " + std::to_string(n));
    }
    ...
    for (int n : {3, 5, 15, 24, 120}) {
        ...
        for (std::size_t step = 0; step < horizon; ++step) {
            if (is_process_step(step, n)) {
                ++fixed;
            }
            if (is_legacy_surface_compute_step(step, n)) {
                ++legacy;
            }
        }
        const int accidental = is_legacy_surface_compute_step(0, n) ? 1 : 0;
        check(legacy == fixed + accidental,
            "over " + std::to_string(horizon) + " steps at interval " + std::to_string(n) +
                ", v1.0.0 does " + std::to_string(accidental) + " extra update(s): expected " +
                std::to_string(fixed + accidental) + ", got " + std::to_string(legacy));
...
test_frequency_conversion() {
    ...
    for (const auto& c : ok) {
        ...
        check(steps == c.steps,
            "frequency " + std::to_string(c.freq_s) + " s at dt " + std::to_string(c.dt_s) +
                " s is " + std::to_string(c.steps) + " steps, got " + std::to_string(steps));
