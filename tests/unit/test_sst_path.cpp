...
void
check_target(const std::string& output_dir,
...
void
check_rejected(const std::string& output_dir,
    const std::string& prefix,
    SstPathError expected,
    const std::string& what) {
    const SstPathResult r = resolve_sst_path(output_dir, prefix);
    if (r.ok) {
    ...
    check(r.error == expected, what + ": rejected for the wrong reason (" + r.message + ")");
...
void
test_normal_path() {
    check_target("/data/run",
        "vvm_output",
        "/data/run/vvm_output.sst",
    ...
    check_target("/data/run/./sub",
        "vvm_output",
        "/data/run/sub/vvm_output.sst",
...
test_spaces_are_ordinary_characters() {
    check_target("/data/my run",
        "vvm_output",
        "/data/my run/vvm_output.sst",
        "a space in output_dir");
    check_target("/data/run", "vvm output", "/data/run/vvm output.sst", "a space in the prefix");
    check_target("/data/my run dir",
        "my output file",
...
test_empty_configuration_is_refused() {
    check_rejected("", "vvm_output", SstPathError::EmptyOutputDir, "an empty output_dir");
    check_rejected("/data/run", "", SstPathError::EmptyPrefix, "an empty prefix");
    check_rejected("", "", SstPathError::EmptyOutputDir, "an empty output_dir and prefix");
}
...
test_traversal_in_the_prefix_is_refused() {
    ...
    for (const auto& prefix : bad_prefixes) {
        check_rejected("/data/run",
            prefix,
            SstPathError::PrefixNotAFileName,
...
make_scratch_dir() {
    const fs::path dir =
        fs::temp_directory_path() / ("vvm_sst_path_test_" + std::to_string(getpid()));
    std::error_code ec;
...
test_missing_path_is_not_an_error() {
    ...
    check(result.outcome == SstCleanupOutcome::NotPresent, "a missing .sst path is 'not present'");
...
test_rejected_configuration_removes_nothing() {
    ...
    const auto traversal = remove_stale_sst_path((scratch / "sub").string(), "../victim");
