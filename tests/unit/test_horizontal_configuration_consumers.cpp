...
void
expect_invalid_argument(Function function, const std::string& label) {
    bool rejected = false;
...
public:
    TemporaryDirectory() {
        const std::string pattern =
            (std::filesystem::temp_directory_path() / "vvm_horizontal_config_XXXXXX").string();
...
Json
make_configuration(bool fixed, int mode) {
    Json config;
    ...
    if (mode == 0) {
        ...
        config["grid"]["boundary_condition"] = {{"x", "periodic"}, {"y", "periodic"}};
    }
    else {
        config["grid"]["horizontal"] = {{"nx", 4},
            {"ny", 4},
            {"n_halo_cells", 2},
            {"geometry",
                {{"kind", "cartesian"}, {"dx", 100.0}, {"dy", 100.0}, {"fix_lonlat", fixed}}},
        ...
        if (mode == 2) {
            ...
            config["grid"]["fix_lonlat"] = !fixed;
            config["grid"]["boundary_condition"] = {{"x", "zero_gradient"}, {"y", "zero_gradient"}};
        }
    }

    config["constants"] = {{"gravity", 9.81}, {"Rd", 287.0}, {"P0", 100000.0}, {"Cp", 1004.0}};

    config["simulation"] = {{"dt_s", 1.0}, {"idealized_test", "2dbubble"}};

    config["dynamics"]["solver"] = {{"WRXMU", 0.25}, {"iteration", 4}};

    config["dynamics"]["tracers"]["config_probe"] = {{"enable", true}};

    return config;
}

Real
longitude_value(int j, int i) {
    return real(100.0) + real(0.25) * i + real(0.125) * j;
}

Real
latitude_value(int j, int i) {
    return real(10.0) + real(0.125) * i + real(0.25) * j;
}

Real
tracer_value(int k, int j, int i) {
    return static_cast<Real>(10000 * k + 100 * j + i);
}

void
run_case(
    const std::filesystem::path& path, bool expected_fixed, const Communication& communication) {
    ...
    check(coordinates_correct,
        "Fixed coordinates are applied, or existing coordinates are preserved.");
    ...
    check(tracer_correct,
        "Tracer initialization preserves horizontal policy and fills vertical halos.");
    Kokkos::fence();
}

void
check_bounded_guard(const std::filesystem::path& path) {
    const ConfigurationManager config(path.string());
    ...
    try {
        ...
        rejected =
            std::string(error.what()).find("Bounded horizontal topology") != std::string::npos;
    }
