...
test_cartesian_identity() {
    ...
    Kokkos::parallel_for("EvaluateCartesianVectorLowering",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
            result(0) = lowering.calculate_covariant_q1_at_u(layout.halo, layout.halo, vector_at_u);

            result(1) = lowering.calculate_covariant_q2_at_v(layout.halo, layout.halo, vector_at_v);
        });

    const auto result_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), result);

    check(close(result_host(0), VVM::real(4.5)),
        "Cartesian lowering must preserve the q1 component");
    check(close(result_host(1), VVM::real(-2.25)),
        "Cartesian lowering must preserve the q2 component");
}
...
test_nonorthogonal_metric() {
    ...
    Kokkos::parallel_for("EvaluateNonorthogonalVectorLowering",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
    ...
    const VVM::Real expected_q1 = VVM::real(2.0) * VVM::real(4.0) + VVM::real(0.5) * VVM::real(5.0);

    const VVM::Real expected_q2 =
        VVM::real(3.0) * VVM::real(-2.0) + VVM::real(0.5) * VVM::real(4.0);

    check(close(result_host(0), expected_q1),
        "Covariant q1 must contain the interpolated g12 q2 contribution");
    check(close(result_host(1), expected_q2),
        "Covariant q2 must contain the interpolated g12 q1 contribution");
}
...
test_metric_weighting_before_interpolation() {
    ...
    Kokkos::parallel_for("InitializeWeightedLoweringFields",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            q1_at_u(j, i) = VVM::real(1.0) + VVM::real(0.5) * static_cast<VVM::Real>(i) +
                            VVM::real(0.25) * static_cast<VVM::Real>(j);

            q2_at_v(j, i) = VVM::real(2.0) + VVM::real(0.75) * static_cast<VVM::Real>(i) -
                            VVM::real(0.125) * static_cast<VVM::Real>(j);

            g12_at_u(j, i) = VVM::real(0.1) * static_cast<VVM::Real>(1 + 2 * j + i);

            g12_at_v(j, i) = VVM::real(0.05) * static_cast<VVM::Real>(2 + j + 3 * i);
        });
    ...
    Kokkos::parallel_for("EvaluateWeightedVectorLowering",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
            constexpr int j = 1;
            constexpr int i = 1;
            ...
            result(1) = VVM::real(2.0) * q1_at_u(j, i) +
                        VVM::real(0.25) * (g12_at_v(j, i) * q2_at_v(j, i) +
            ...
            result(3) = VVM::real(3.0) * q2_at_v(j, i) +
                        VVM::real(0.25) * (g12_at_u(j, i) * q1_at_u(j, i) +
    ...
    check(close(result_host(0), result_host(1)),
        "The q1 cross term must weight values before interpolation");
    check(close(result_host(2), result_host(3)),
        "The q2 cross term must weight values before interpolation");
}
...
test_three_dimensional_stencil_loaders() {
    ...
    Kokkos::parallel_for("InitializeThreeDimensionalLoweringFields",
        Kokkos::MDRangePolicy<Kokkos::Rank<3>>({0, 0, 0}, {2, 3, 3}),
        KOKKOS_LAMBDA(const int k, const int j, const int i) {
            q1_at_u(k, j, i) = VVM::real(100.0) * static_cast<VVM::Real>(k) +
                               VVM::real(10.0) * static_cast<VVM::Real>(j) +
                               static_cast<VVM::Real>(i);

            q2_at_v(k, j, i) = VVM::real(1000.0) * static_cast<VVM::Real>(k) +
                               VVM::real(100.0) * static_cast<VVM::Real>(j) +
                               VVM::real(10.0) * static_cast<VVM::Real>(i);
        });

    Kokkos::parallel_for("LoadThreeDimensionalLoweringStencils",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
...
VVM::Real
smooth_lowering_error(const int n) {
    ...
    Kokkos::parallel_for("InitializeSmoothLoweringFields",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            const VVM::Real x_at_u = (static_cast<VVM::Real>(i - halo) + VVM::real(0.5)) * spacing;
            const VVM::Real y_at_u = static_cast<VVM::Real>(j - halo) * spacing;

            const VVM::Real x_at_v = static_cast<VVM::Real>(i - halo) * spacing;
            const VVM::Real y_at_v = (static_cast<VVM::Real>(j - halo) + VVM::real(0.5)) * spacing;

            q1_at_u(j, i) = Kokkos::sin(x_at_u) + VVM::real(0.5) * Kokkos::cos(y_at_u);

            q2_at_v(j, i) = Kokkos::cos(x_at_v) - VVM::real(0.25) * Kokkos::sin(y_at_v);

            g12_at_u(j, i) = VVM::real(0.2) + VVM::real(0.05) * Kokkos::sin(x_at_u + y_at_u);

            g12_at_v(j, i) = VVM::real(0.2) + VVM::real(0.05) * Kokkos::sin(x_at_v + y_at_v);
        });
    ...
    Kokkos::parallel_for("EvaluateSmoothVectorLowering",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({halo, halo}, {ny - halo, nx - halo}),
        KOKKOS_LAMBDA(const int j, const int i) {
            covariant_q1_at_u(j, i) = lowering.calculate_covariant_q1_at_u(q1_at_u, q2_at_v, j, i);

            covariant_q2_at_v(j, i) = lowering.calculate_covariant_q2_at_v(q1_at_u, q2_at_v, j, i);
        });
    ...
    for (int j = halo; j < ny - halo; ++j) {
        for (int i = halo; i < nx - halo; ++i) {
            ...
            const VVM::Real q1_exact = std::sin(x_at_u) + VVM::real(0.5) * std::cos(y_at_u);

            const VVM::Real q2_exact_at_u = std::cos(x_at_u) - VVM::real(0.25) * std::sin(y_at_u);

            const VVM::Real g12_exact_at_u =
                VVM::real(0.2) + VVM::real(0.05) * std::sin(x_at_u + y_at_u);

            const VVM::Real q1_exact_at_v = std::sin(x_at_v) + VVM::real(0.5) * std::cos(y_at_v);

            const VVM::Real q2_exact = std::cos(x_at_v) - VVM::real(0.25) * std::sin(y_at_v);

            const VVM::Real g12_exact_at_v =
                VVM::real(0.2) + VVM::real(0.05) * std::sin(x_at_v + y_at_v);

            const VVM::Real expected_q1 =
                VVM::real(1.5) * q1_exact + g12_exact_at_u * q2_exact_at_u;

            const VVM::Real expected_q2 =
                g12_exact_at_v * q1_exact_at_v + VVM::real(1.25) * q2_exact;

            const VVM::Real q1_error = covariant_q1_host(j, i) - expected_q1;

            const VVM::Real q2_error = covariant_q2_host(j, i) - expected_q2;
...
test_second_order_cross_component_interpolation() {
    ...
    check(coarse_error > fine_error,
        "Refining the grid must reduce the vector-lowering interpolation error");
    check(error_ratio > VVM::real(3.5),
        "The U/V cross-component interpolation must show second-order convergence");
}
