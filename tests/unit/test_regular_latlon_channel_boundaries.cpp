...
bool
close_enough(Real actual, Real expected) {
...
centered_q2_neumann_halos(const Grid& grid, const Field<Dim>& field) {
    ...
    const bool owns_south = grid.get_local_physical_start_y() == 0;
    const bool owns_north = grid.get_local_physical_end_y() == grid.get_global_points_y() - 1;
    ...
    if constexpr (Dim == 2) {
        ...
        if (owns_south) {
            for (int distance = 0; distance < h; ++distance) {
                ...
                const int exterior_j = h - 1 - distance;

                const int interior_j = h + distance;

                for (int i = h; i < nx - h; ++i) {

                    if (data(exterior_j, i) != data(interior_j, i)) {
                        return false;
                    }
                }
            }
        ...
        if (owns_north) {
            ...
            for (int distance = 0; distance < h; ++distance) {
                ...
                const int exterior_j = wall_j + 1 + distance;

                const int interior_j = wall_j - distance;
            ...
            for (int distance = 0; distance < h; ++distance) {
                ...
                for (int k = 0; k < nz; ++k) {
                    ...
                    for (int i = h; i < nx - h; ++i) {
                        ...
                        if (data(k, exterior_j, i) != data(k, interior_j, i)) {
                            return false;
                        }
                    }
                }
        ...
        if (owns_south) {
            ...
            for (int i = h; i < nx - h; ++i) {
                ...
                for (int distance = 1; distance < h; ++distance) {
                    ...
                    if (data(wall_j - distance, i) != -data(wall_j + distance, i)) {
                        return false;
                    }
                }
            }
            ...
            for (int k = 0; k < nz; ++k) {
                ...
                for (int i = h; i < nx - h; ++i) {
                    ...
                    for (int distance = 1; distance < h; ++distance) {
                        ...
                        if (data(k, wall_j - distance, i) != -data(k, wall_j + distance, i)) {
                            return false;
                        }
                    }
                }
...
free_slip_physical_wind_halos(const Grid& grid, const Field<3>& u, const Field<3>& v) {
    ...
    const auto h1_device = grid.geometry()
                               .device_view(HorizontalLocation::U)
                               .contravariant_to_physical.a11.one_dimensional;

    const auto h1 = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), h1_device);
        ...
        for (int k = 0; k < nz; ++k) {
            ...
            for (int i = h; i < nx - h; ++i) {
                ...
                for (int distance = 0; distance < h; ++distance) {
                    ...
                    if (!close_enough(h1(exterior_j) * u_data(k, exterior_j, i),
                            h1(interior_j) * u_data(k, interior_j, i))) {
                        return false;
                    }
