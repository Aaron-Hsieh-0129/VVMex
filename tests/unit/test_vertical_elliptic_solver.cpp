...
struct Reference {
    ...
    Reference(const Grid& grid, bool stretched)
        : nx(grid.get_global_points_x()), ny(grid.get_global_points_y()),
          nz(grid.get_local_total_points_z()), h(grid.get_halo_cells()),
          bounded(grid.horizontal_specification().topology.q2 ==
                  VVM::Core::HorizontalEdgeTopology::Bounded),
          spherical(grid.geometry().kind() == VVM::Core::Geometry::GeometryKind::RegularLatLon),
          dx(grid.geometry().dq1()), dy(grid.geometry().dq2()), w(nz * ny * nx, 0.0), history(w),
          rho(nz), rho_up(nz), mid(nz), up(nz) {
        if (spherical) {
            const auto& geometry =
                dynamic_cast<const VVM::Core::Geometry::RegularLatLonGeometry&>(grid.geometry());
    ...
    Real
    source(bool is_eta, int step, int k, int j, int i) const {
        const int n = index(k, j, i);
        i = n % nx;
        j = (n / nx) % ny;
        const double factor = step == 2 ? 0.0 : step + 1.0;
        const double angle = 2.0 * std::acos(-1.0) * i / nx;
        return Real(factor * (is_eta ? .02 * std::cos(angle) - .004 * j
                                     : .001 * k + .01 * std::sin(angle) + .003 * j));
    }
    ...
    dense_solve(std::vector<double> a, std::vector<double> b) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            if (!std::isfinite(a[k * n + k]) || a[k * n + k] == 0.0) {
                fatal("Invalid dense reference pivot.");
            }
            for (int j = k + 1; j < n; ++j) {
    ...
    void
    advance(int step, int iterations, double shift) {
        ...
        for (int iteration = 0; iteration < iterations; ++iteration) {
            ...
            for (int j = 0; j < ny; ++j) {
                for (int i = 0; i < nx; ++i) {
                    ...
                    const double east = 1.0 / (ct * dx * dx), north = cn / (dy * dy),
                                 south_weight = cs / (dy * dy);
                    ...
                    for (int k = h; k <= last; ++k) {
                        ...
                        matrix[level * levels + level] =
                            (shift + 2.0 * east + north + south_weight) / rho_up[k] - lower - upper;
                        if (level > 0) {
                            matrix[level * levels + level - 1] = lower;
                        }
                        if (level + 1 < levels) {
                            matrix[level * levels + level + 1] = upper;
                        }
                        const double forcing =
                            -radius * ((double(source(true, step, k, j, i)) -
                                           double(source(true, step, k, j, i - 1))) /
                                              dx +
                                          (cn * double(source(false, step, k, j, i)) -
                                              cs * double(source(false, step, k, j - 1, i))) /
                                              dy);
                        rhs[level] =
                            shift * current[index(k, j, i)] + forcing +
                            east * (current[index(k, j, i + 1)] + current[index(k, j, i - 1)]) +
                            north * current[index(k, j + 1, i)] +
                            south_weight * current[index(k, j - 1, i)];
                    }
                    const auto solution = dense_solve(matrix, rhs);
                    for (int k = h; k <= last; ++k) {
                        next[index(k, j, i)] = solution[k - h] / rho_up[k];
                    }
...
std::vector<Real>
snapshot(const Field<3>& field) {
    const auto data = field.get_host_data();
    std::vector<Real> values;
    values.reserve(data.size());
    for (int k = 0; k < int(data.extent(0)); ++k) {
        for (int j = 0; j < int(data.extent(1)); ++j) {
            for (int i = 0; i < int(data.extent(2)); ++i) {
...
int
run_case(const Grid& grid, HaloExchanger& halo, int iterations, bool stretched) {
    const int nz = grid.get_local_total_points_z(), ny = grid.get_local_total_points_y(),
              nx = grid.get_local_total_points_x(), h = grid.get_halo_cells();
    ...
    const auto upload = [](Field<1>& field, const std::vector<double>& values) {
        ...
        for (int k = 0; k < int(values.size()); ++k) {
            host(k) = Real(values[k]);
        }
        Kokkos::deep_copy(field.get_mutable_device_data(), host);
    };
    upload(rho, reference.rho);
    upload(density, reference.rho_up);
    upload(mid, reference.mid);
    upload(up, reference.up);
    Field<3> xi("xi", {nz, ny, nx}), eta("eta", {nz, ny, nx});
    Field<3> w("w", {nz, ny, nx}), previous("previous", {nz, ny, nx}),
        replay_w("replay_w", {nz, ny, nx}), replay_previous("replay_previous", {nz, ny, nx});
    ...
    const auto sources = [&](int step) {
        auto x = Kokkos::create_mirror(xi.get_device_data()),
             e = Kokkos::create_mirror(eta.get_device_data());
        for (int k = 0; k < nz; ++k) {
            for (int j = 0; j < ny; ++j) {
                for (int i = 0; i < nx; ++i) {
                    const int gj = grid.get_local_physical_start_y() + j - h,
                              gi = grid.get_local_physical_start_x() + i - h;
    ...
    Kokkos::fence();
    w.set_to_zero();
    previous.set_to_zero();
    replay_w.set_to_zero();
    replay_previous.set_to_zero();
    Kokkos::fence();
#if defined(ENABLE_NCCL)
    Graph graph;
    ...
    for (int step = 0; step < 3; ++step) {
        ...
        for (int k = 0; k < nz; ++k) {
            for (int j = 0; j < ny; ++j) {
                for (int i = 0; i < nx; ++i) {
                    ...
                    maximum_error =
                        std::max(maximum_error, std::abs(double(actual(k, j, i)) - expected));
                    maximum_value = std::max(maximum_value, std::abs(expected));
                    if (k < h || k > nz - h - 2) {
                        surfaces = surfaces && actual(k, j, i) == real(0.0);
                    }
                }
            }
        }
        int flags[4] = {int(snapshot(w) == snapshot(replay_w) &&
                            snapshot(previous) == snapshot(replay_previous)),
            int(snapshot(xi) == before_x && snapshot(eta) == before_e),
            int(snapshot(previous) == before_w),
            int(finite && surfaces)};
        int global[4];
        ...
        const bool pass = global[0] && global[1] && global[2] && global[3] &&
                          error <= (sizeof(Real) == sizeof(double) ? 2e-10 : 5e-4);
        if (grid.get_mpi_rank() == 0) {
            std::printf("ranks=%d iterations=%d stretched=%d step=%d exact=%d sources=%d "
                        "history=%d surfaces_finite=%d reference_error=%.3e %s\n",
                grid.get_mpi_size(),
                iterations,
                int(stretched),
                step,
...
run(const Grid& grid, HaloExchanger& halo) {
    ...
    for (bool stretched : {false, true}) {
        for (int iterations : {1, 4}) {
            failures += run_case(grid, halo, iterations, stretched);
        }
    }
    return failures;
}
...
main(int argc, char** argv) {
    ...
    try {
        ...
        if (argc != 2 || (ranks != 1 && ranks != 2 && ranks != 4)) {
            fatal("Provide a configuration and use 1, 2 or 4 ranks.");
        }
        {
