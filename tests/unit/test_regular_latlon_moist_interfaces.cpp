...
main(int argc, char** argv) {
    ...
    try {
          ...
          "grid":{
            "horizontal":{
              ...
              "geometry":{
                ...
                "longitude_bounds_deg":[-1,1],
                "latitude_bounds_deg":[-1,1]
              },
              "topology":{
                "q1":"periodic",
                "q2":"bounded"
        ...
        // --------------------------------------------------------------------
        // Controlled moist/RLL interface fixture.
        //
        // P3 is now an admitted RLL physics option. This test directly
        // exercises moist scalar transport and moist buoyancy through the
        // NumericalMethodFactory. Full-model P3 execution is covered by the
        // dedicated RLL P3 integration tests.
        // --------------------------------------------------------------------
        ...
        const Json advection = {{"tendency_terms",
            {
                {"advection",
                    {
                        {"enable", true},
        ...
        // qp is a diagnostic condensate quantity and must not be
        // independently transported.
        bool rejected = false;

        try {
            (void)factory.create("qp", advection, false, true, dims);
        }
        ...
        for (const char* name : {"qv", "qc", "qr", "qi", "qm", "nc", "nr", "ni", "bm"}) {
            ...
            if (!state.has_field(tendency)) {
                state.add_field<3>(tendency, dims);
            }
            ...
            if (std::string(name) == "qv") {
                ...
                require(cudaStreamEndCapture(stream, &graph) == cudaSuccess,
                    "end moist transport capture");

                require(cudaGraphInstantiate(&executable, graph, nullptr, nullptr, 0) ==
                            cudaSuccess,
                    "instantiate moist transport graph");
                ...
                for (const Real mixing_ratio : {real(.02), real(0.), real(.01)}) {
                    ...
                    require(cudaGraphLaunch(executable, stream) == cudaSuccess,
                        "replay moist transport graph");

                    require(cudaStreamSynchronize(stream) == cudaSuccess,
                        "complete moist transport graph");

                    const auto replay = state.get_field<3>(tendency).get_host_data();

                    // Independent snapshot: get_host_data may alias host
                    // storage.
                    ...
                    for (std::size_t n = 0; n < snapshot.size(); ++n) {
                        ...
                        require(snapshot[n] == direct.data()[n],
                            "changed-input graph replay parity");
                    }
                }
            ...
            transport.add_flux_convergence(state.get_field<3>(name),
            ...
            for (int k = h; k < nz - h; ++k) {
                ...
                for (int j = h; j < ny - h; ++j) {
                    ...
                    for (int i = h; i < nx - h; ++i) {
                        ...
                        require(std::abs(actual(k, j, i) - reference(k, j, i) / real(2.)) <
                                    real(1e-14),
        ...
        const Json buoyancy = {{"tendency_terms",
            {
                {"buoyancy",
                    {
                        {"enable", true},
