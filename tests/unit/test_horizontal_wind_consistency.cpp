...
enum class TestMode { Constant, Rotational, Divergent, Mixed, Zonal };

const char*
mode_name(const TestMode mode) {
    switch (mode) {
...
int
run_case(const HorizontalGeometry& geometry,
    const TestMode mode,
    const double radius,
    const double south_edge) {
    ...
    const auto physical_scale_q1 =
        geometry.device_view(HorizontalLocation::U).contravariant_to_physical.a11;
    const auto physical_scale_q2 =
        geometry.device_view(HorizontalLocation::V).contravariant_to_physical.a22;

    Kokkos::parallel_for("ConsistencyReconstructWind",
        Policy2D({1, 1}, {ny - 1, nx - 1}),
    ...
    Kokkos::parallel_for("ConsistencyLowerWind",
        Policy2D({2, 2}, {ny - 2, nx - 2}),
        KOKKOS_LAMBDA(const int j, const int i) {
    ...
    Kokkos::parallel_for("ConsistencyEvaluateOperators",
        Policy2D({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            diagnostics(0, j, i) =
                divergence.at_t(j, i, q1(j, i), q1(j, i - 1), q2(j, i), q2(j - 1, i));
    ...
    const auto diagnostics_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), diagnostics);
    const auto physical_q1_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), physical_q1);
    const auto physical_q2_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), physical_q2);
    ...
    const double derivative_scale =
        amplitude * inverse_spacing_squared_sum / (length_scale * length_scale);
    ...
    for (int j = h; j < ny - h; ++j) {
        for (int i = h; i < nx - h; ++i) {
            ...
            const double dpsi_dq2 =
                (static_cast<double>(psi_host(j, i)) - static_cast<double>(psi_host(j - 1, i))) /
                dq2;
            const double dpsi_dq1 =
                (static_cast<double>(psi_host(j, i)) - static_cast<double>(psi_host(j, i - 1))) /
                dq1;
            const double dchi_dq1 =
                (static_cast<double>(chi_host(j, i + 1)) - static_cast<double>(chi_host(j, i))) /
                dq1;
            const double dchi_dq2 =
                (static_cast<double>(chi_host(j + 1, i)) - static_cast<double>(chi_host(j, i))) /
                dq2;
            ...
            if (mode == TestMode::Zonal) {
                ...
                if (spherical) {
                    ...
                    expected_q1 = zonal_speed * std::cos(latitude_u) * std::sin(half_dlatitude) /
                                  half_dlatitude;
                }
                else {
            ...
            const std::array<double, 8> values = {actual_divergence,
                expected_divergence,
                actual_curl,
                expected_curl,
                actual_q1,
            ...
            errors[0] = std::max(errors[0],
                std::abs(actual_divergence - expected_divergence) / derivative_scale);
            errors[1] =
                std::max(errors[1], std::abs(actual_curl - expected_curl) / derivative_scale);
    ...
    const double cell_count =
        static_cast<double>(layout.local_physical_nx) * layout.local_physical_ny;
    mean_q1 /= cell_count;
    ...
    std::printf("%s mode=%s div_error=%.3e curl_error=%.3e physical_q1_error=%.3e "
                "physical_q2_error=%.3e %s\n",
        geometry.name(),
        mode_name(mode),
        errors[0],
        errors[1],
...
run_tests() {
    ...
    const RegularLatLonGeometry regular_latlon(layout,
        real(0.1),
        real(0.075),
        real(0.0),
        south_edge,
    ...
    const std::array<TestMode, 5> modes = {TestMode::Constant,
        TestMode::Rotational,
        TestMode::Divergent,
        TestMode::Mixed,
        TestMode::Zonal};
    ...
    for (const TestMode mode : modes) {
        ...
        failures += run_case(regular_latlon,
            mode,
            static_cast<double>(radius),
            static_cast<double>(south_edge));
    }
