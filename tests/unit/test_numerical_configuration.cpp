...
void
expect_invalid(const char* message,
...
main(int argc, char** argv) {
    ...
    expect_invalid("zero halo width accepted", [](auto& v) {
        v.halo_width = 0;
    });
    expect_invalid("one-cell halo accepted", [](auto& v) {
        v.halo_width = 1;
    });
    expect_invalid("zero grid spacing accepted", [](auto& v) {
        v.dx = 0.0;
    });
    expect_invalid("non-finite grid spacing accepted", [](auto& v) {
        v.dz = std::numeric_limits<double>::quiet_NaN();
    });
    expect_invalid("zero timestep accepted", [](auto& v) {
        v.dt = 0.0;
    });
    expect_invalid("negative timestep accepted", [](auto& v) {
        v.dt = -1.0;
    });
    expect_invalid("zero output interval accepted", [](auto& v) {
        v.output_interval = 0.0;
    });
    expect_invalid("negative total time accepted", [](auto& v) {
        v.total_time = -1.0;
    });
    expect_invalid("zero grid dimension accepted", [](auto& v) {
        v.nx = 0;
    });
