...
void
expect_invalid(const char* value, const char* message) {
    try {
...
void
test_parsing() {
    check(parse_output_precision("native", "k") == OutputPrecision::Native, "native parses");
    check(parse_output_precision("float32", "k") == OutputPrecision::Float32, "float32 parses");
    ...
    check(resolve_output_element_type(OutputPrecision::Float32) == OutputElementType::Float32,
...
std::string
write_config(const std::filesystem::path& path, const std::string& output_body) {
...
void
test_configuration(const std::filesystem::path& root) {
    using VVM::Utils::ConfigurationManager;

    const ConfigurationManager none(write_config(root / "none.json", "    \"output_dir\": \".\""));
    ...
    const ConfigurationManager shared_with_block(write_config(root / "shared_block.json",
        "    \"precision\": \"float32\",\n"
        "    \"bp5\": { \"num_subfiles\": 4 }"));
    check(Bp5OutputConfig::from_config(shared_with_block).precision == OutputPrecision::Float32,
    ...
    const ConfigurationManager overridden(write_config(root / "override.json",
        "    \"precision\": \"float32\",\n"
    ...
    check(legacy.precision_from_bp5_block, "using the deprecated key is recorded");
...
main(int argc, char** argv) {
    ...
    fs::remove_all(root, ec);
    if (failures == 0) {
        std::fprintf(stdout, "test_output_precision: all checks passed\n");
    }
