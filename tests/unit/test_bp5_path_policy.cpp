...
void
check_throws(Function&& function, const char* message) {
    try {
        function();
        check(false, message);
    }
...
main() {
    ...
    const fs::path dataset =
        prepare_bp5_dataset_path(root.string(), "history", ExistingDatasetPolicy::Error);
    ...
    const fs::path appended =
        prepare_bp5_dataset_path(root.string(), "history", ExistingDatasetPolicy::Append);
    ...
    const fs::path replaced =
        prepare_bp5_dataset_path(root.string(), "history.bp", ExistingDatasetPolicy::Replace);
    ...
    check_throws([&] {
        prepare_bp5_dataset_path(root.string(), "history", ExistingDatasetPolicy::Replace);
    ...
    check_throws([&] {
        prepare_bp5_dataset_path(root.string(), "../escape", ExistingDatasetPolicy::Replace);
    }, "parent traversal prefix is refused");
    check_throws([&] {
        prepare_bp5_dataset_path(root.string(), "", ExistingDatasetPolicy::Replace);
    }, "empty prefix is refused");
    check_throws([&] {
        prepare_bp5_dataset_path("", "history", ExistingDatasetPolicy::Replace);
    }, "empty output directory is refused");

    fs::remove_all(root, ec);
    if (failures == 0) {
        std::puts("test_bp5_path_policy: PASS");
    }
