...
void
test_vorticity(const HorizontalGeometry& geometry,
    bool spherical,
    Real radius,
    Real south_edge,
    ...
    for (bool stretched : {false, true}) {
        const std::array<Real, nz> z =
            stretched ? std::array<Real, nz>{real(0), real(40), real(120), real(240), real(400)}
        ...
        for (int mode = 0; mode < 4; ++mode) {
            ...
            Kokkos::parallel_for("TestHorizontalVorticity",
                compact_policy,
                KOKKOS_LAMBDA(const int j, const int i) {
                    for (int k = 0; k < nz - 1; ++k) {
                        omega1(k, j, i) = operation.calculate_contravariant_q1_at_v(w,
                            u2,
                            k,
                            j,
                            i,
                            inverse_spacing(k));
                        omega2(k, j, i) = operation.calculate_contravariant_q2_at_u(w,
                            u1,
                            k,
                            j,
                            i,
            ...
            Kokkos::parallel_for("TestCovariantWindDownwardRecovery",
                compact_policy,
                KOKKOS_LAMBDA(const int j, const int i) {
                    ...
                    for (int k = nz - 2; k >= 0; --k) {
                        const Real du1_dz = operation.calculate_covariant_q1_vertical_shear_at_u(w,
                            omega2,
                            k,
                            j,
                            i);
                        const Real du2_dz = operation.calculate_covariant_q2_vertical_shear_at_v(w,
                            omega1,
                            k,
                            j,
                            i);
            ...
            const auto omega1_host =
                Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), omega1);
            const auto omega2_host =
                Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), omega2);
            const auto recovered1_host =
                Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), recovered1);
            const auto recovered2_host =
                Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), recovered2);
                ...
                for (int i = h; i < nx - h; ++i) {
                    for (int k = 0; k < nz - 1; ++k) {
                        ...
                        if (!std::isfinite(actual_e) || !std::isfinite(actual_n)) {
                            ...
                            vorticity_error = std::max(vorticity_error,
                                std::abs(actual_e - expected_e) / real(1e-3));
                            vorticity_error = std::max(vorticity_error,
                                std::abs(actual_n - expected_n) / real(1e-3));
                        }
                    ...
                    for (int k = 0; k < nz; ++k) {
                        ...
                        if (!std::isfinite(actual_u) || !std::isfinite(actual_v)) {
                            ...
                            recovery_error = std::max(recovery_error,
                                std::abs(actual_u - expected_u) /
                                    std::max(real(1.0), std::abs(expected_u)));
                            recovery_error = std::max(recovery_error,
                                std::abs(actual_v - expected_v) /
                                    std::max(real(1.0), std::abs(expected_v)));
                        }
                    }
                }
            }

            const bool passed = finite && zero_case_correct && vorticity_error <= tolerance &&
            ...
            std::printf("%s %s stretched=%d mode=%d vorticity_error=%.3e recovery_error=%.3e %s\n",
                ...
                mode,
                static_cast<double>(vorticity_error),
                static_cast<double>(recovery_error),
                passed ? "PASS" : "FAIL");
