...
void
launch_kernel(VVM::Dynamics::VerticalWindDiagnosticDeviceView operation,
    Volume xi,
    Volume eta,
    Volume w,
    Volume zeta,
    Kokkos::View<Real**> profiles,
    Kokkos::View<Real****> result,
    int nx,
    int ny,
    int h,
    Real inverse_dz,
    Real shift) {
    const Profile rho{profiles, 0}, rho_up{profiles, 1}, flex_mid{profiles, 2},
        flex_up{profiles, 3}, spacing{profiles, 4};
    const auto policy = Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h});

    Kokkos::parallel_for("VerticalWindDiagnosticBatch",
        Kokkos::Experimental::require(policy,
            Kokkos::Experimental::WorkItemProperty::HintLightWeight),
        KOKKOS_LAMBDA(int j, int i) {
            for (int k = bottom; k <= top; ++k) {
                const Real rhs = operation.calculate_weighted_rhs_at_t(xi, eta, k, j, i);
                const auto row = operation.calculate_row_at_t(rho,
                    rho_up,
                    flex_mid,
                    flex_up,
                    inverse_dz,
                ...
                result(0, k, j, i) = rhs;
                result(1, k, j, i) =
                    operation.calculate_vorticity_divergence_at_z(xi, eta, k, j, i);
...
run_case(const HorizontalGeometry& geometry,
    ...
    Volume xi("xi", nz, ny, nx), eta("eta", nz, ny, nx), w("previous_w", nz, ny, nx),
        zeta("zeta", nz, ny, nx);
    ...
    auto hx = Kokkos::create_mirror(xi), he = Kokkos::create_mirror(eta),
         hw = Kokkos::create_mirror(w), hz = Kokkos::create_mirror(zeta);
        ...
        for (int k = 0; k < nz; ++k) {
            ...
            for (int j = 0; j < ny; ++j) {
                for (int i = 0; i < nx; ++i) {
                    ...
                    he(k, j, i) =
                        factor * (-real(0.015) * k + real(0.004) * i - real(0.0015) * j * j);
                    hw(k, j, i) = factor * (real(0.3) * k + real(0.02) * i * i -
                                               real(0.015) * j * j + real(0.001) * i * j);
    ...
    const auto divergence = [&](int k, int j, int i) {
        return ((hx(k, j, i + 1) - hx(k, j, i)) / dx -
                   (cos_center(j + 1) * he(k, j + 1, i) - cos_center(j) * he(k, j, i)) / dy) /
               (radius * cos_face(j));
    };
    ...
    for (int step = 0; step < 3; ++step) {
        ...
        const auto check = [&](Real value, Real expected, Real scale) {
            if (!std::isfinite(value) || !std::isfinite(expected)) {
                ...
                error = std::max(error,
                    std::abs(value - expected) / std::max(scale, std::abs(expected)));
            }
        };

        for (int k = 0; k < nz; ++k) {
            for (int p = 0; p < 5; ++p) {
                sources = sources && ap(p, k) == hp(p, k);
            }

            for (int j = 0; j < ny; ++j) {
                for (int i = 0; i < nx; ++i) {
                    sources = sources && ax(k, j, i) == hx(k, j, i) && ae(k, j, i) == he(k, j, i) &&
                              aw(k, j, i) == hw(k, j, i);
                    ...
                    for (int p = 0; p < 8; ++p) {
                        ...
                        if (!interior || k < bottom || k > top) {
                            untouched = untouched && actual(p, k, j, i) == sentinel;
                        }
                    }
                    ...
                    const Real lower =
                        -jacobian * hp(3, k) * hp(2, k) * inverse_dz * inverse_dz / hp(0, k);
                    const Real upper = -jacobian * hp(3, k) * hp(2, k + 1) * inverse_dz *
                                       inverse_dz / hp(0, k + 1);
                    const Real diagonal =
                        (shift + real(2.0) * east + north + south_coefficient) / hp(1, k) - lower -
                        upper;

                    const Real rhs = -radius * ((he(k, j, i) - he(k, j, i - 1)) / dx +
                                                   (cv * hx(k, j, i) - cs * hx(k, j - 1, i)) / dy);
                    const Real neighbors = east * (hw(k, j, i + 1) + hw(k, j, i - 1)) +
                                           north * hw(k, j + 1, i) +
                                           south_coefficient * hw(k, j - 1, i);
                    const Real jacobi = (rhs + neighbors - lower * hp(1, k - 1) * hw(k - 1, j, i) -
                                            upper * hp(1, k + 1) * hw(k + 1, j, i)) /
                                        (diagonal * hp(1, k) - shift);

                    const Real expected[8] = {rhs,
                        divergence(k, j, i),
                        lower,
                        diagonal,
                        upper,
                    ...
                    for (int p = 0; p < 8; ++p) {
                        const Real scale =
                            p == 0 ? radius : (p == 1 ? real(0.1) / radius : real(1.0));
        ...
        std::printf(
            "%s R=%.0f %s stretched=%d step=%d exact=%d sources=%d untouched=%d error=%.3e %s\n",
            geometry.name(),
            double(radius),
            layout_name,
            int(stretched),
...
run() {
    ...
    const auto exercise = [&](const HorizontalGeometry& geometry, Real radius, Real south) {
        for (bool stretched : {false, true}) {
            failures +=
                run_case<Kokkos::LayoutLeft>(geometry, radius, south, "LayoutLeft", stretched);
            failures +=
                run_case<Kokkos::LayoutRight>(geometry, radius, south, "LayoutRight", stretched);
        }
    };
...
main(int argc, char** argv) {
    ...
    try {
        if (ranks != 1) {
            throw std::runtime_error("This local stencil test requires one MPI rank.");
        }
        failures = run();
    }
