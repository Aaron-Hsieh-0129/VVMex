...
RestartFileMetadata
time_and_step(double time_s, long long step) {
...
bool
warns(const RestartTimeResolution& r) {
    return !r.warnings.empty();
}

std::string
all_warnings(const RestartTimeResolution& r) {
    std::string joined;
...
void
check_resolved(const RestartTimeResolution& r,
    double time_s,
    long long step,
    RestartTimeSource source,
    ...
    check(r.time_s == time_s,
        what + ": time " + std::to_string(r.time_s) + " != " + std::to_string(time_s));
    check(r.step == step,
        what + ": step " + std::to_string(r.step) + " != " + std::to_string(step));
    check(r.source == source,
        what + ": unexpected source '" + VVM::Utils::to_string(r.source) + "'");
}

void
check_rejected(const RestartTimeResolution& r, const std::string& what) {
    if (r.ok) {
        std::fprintf(stderr,
            "FAIL: %s (accepted: time=%.17g step=%lld)\n",
            what.c_str(),
            r.time_s,
            r.step);
        ++failures;
...
test_time_and_step() {
    ...
    check_resolved(r,
        7200.0,
        3600,
        RestartTimeSource::FileTimeAndStep,
...
test_time_without_step() {
    const auto r =
        resolve_restart_time(time_only(7200.0), policy_with_dt(2.0), "vvm_output_000002.h5");
    check_resolved(r,
        7200.0,
        3600,
        RestartTimeSource::FileTimeDerivedStep,
...
test_step_without_time() {
    const auto r =
        resolve_restart_time(step_only(1800), policy_with_dt(4.0), "vvm_output_000002.h5");
    check_resolved(r,
        7200.0,
        1800,
        RestartTimeSource::FileStepDerivedTime,
...
test_file_metadata_beats_the_filename() {
    ...
    const auto r =
        resolve_restart_time(time_and_step(7200.0, 3600), policy, "vvm_output_000042.h5");
    check_resolved(r,
        7200.0,
        3600,
        RestartTimeSource::FileTimeAndStep,
...
test_odd_and_even_steps() {
    ...
    const auto even_derived = resolve_restart_time(time_only(7200.0), policy_with_dt(2.0), "r.h5");
    check_resolved(even_derived,
        7200.0,
        3600,
        RestartTimeSource::FileTimeDerivedStep,
        "even derived step");
    const auto odd_derived = resolve_restart_time(time_only(7202.0), policy_with_dt(2.0), "r.h5");
    check_resolved(odd_derived,
        7202.0,
        3601,
        RestartTimeSource::FileTimeDerivedStep,
        "odd derived step");
...
test_inconsistent_time_and_step() {
    ...
    const auto r = resolve_restart_time(time_and_step(3600.0, 3600),
        policy_with_dt(2.0),
        "vvm_output_000002.h5");
    ...
    const auto float_noise =
        resolve_restart_time(time_and_step(static_cast<double>(static_cast<float>(7200.0)), 3600),
            policy_with_dt(2.0),
            "r.h5");
...
test_legacy_time_s() {
    ...
    check_resolved(r,
        7200.0,
        3600,
        RestartTimeSource::LegacyConfig,
...
test_missing_metadata_without_fallback() {
    const auto r =
        resolve_restart_time(RestartFileMetadata{}, policy_with_dt(2.0), "vvm_output_000002.h5");
...
test_filename_fallback() {
    ...
    const auto renamed =
        resolve_restart_time(RestartFileMetadata{}, policy, "vvm_output_000003.h5");
    check_resolved(renamed,
        10800.0,
        5400,
        RestartTimeSource::Filename,
