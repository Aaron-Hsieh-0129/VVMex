...
void
check_metadata(const RestartFileMetadata& m,
    bool has_time,
    double time_s,
    bool has_step,
    long long step,
    const std::string& what) {
    check(m.has_time == has_time, what + ": has_time is " + (m.has_time ? "true" : "false"));
    check(m.has_step == has_step, what + ": has_step is " + (m.has_step ? "true" : "false"));
    if (has_time && m.has_time) {
        check(m.time_s == time_s,
            what + ": time " + std::to_string(m.time_s) + " != " + std::to_string(time_s));
    }
    if (has_step && m.has_step) {
        check(m.step == step,
            what + ": step " + std::to_string(m.step) + " != " + std::to_string(step));
    }
}
...
// stops.
void
check_identical_across_ranks(const RestartFileMetadata& m, const std::string& what) {
    if (nranks < 2) {
        return;
    }
...
void
write_hdf5_scalar_double(hid_t where, const char* name, double value) {
    hid_t space = H5Screate(H5S_SCALAR);
    hid_t dataset =
        H5Dcreate2(where, name, H5T_IEEE_F64LE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void
write_hdf5_scalar_float(hid_t where, const char* name, float value) {
    hid_t space = H5Screate(H5S_SCALAR);
    hid_t dataset =
        H5Dcreate2(where, name, H5T_IEEE_F32LE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void
write_hdf5_scalar_int64(hid_t where, const char* name, int64_t value) {
    hid_t space = H5Screate(H5S_SCALAR);
    hid_t dataset =
        H5Dcreate2(where, name, H5T_STD_I64LE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void
write_hdf5_dummy_field(hid_t where) {
    const hsize_t dims[3] = {2, 2, 2};
    double values[8] = {0, 1, 2, 3, 4, 5, 6, 7};
    hid_t space = H5Screate_simple(3, dims, nullptr);
    hid_t dataset =
        H5Dcreate2(where, "th", H5T_IEEE_F64LE, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void
write_hdf5_output_file(const fs::path& path,
    bool with_model_time,
    bool with_model_step,
    bool with_legacy_time,
    double time_s,
    int64_t step) {
    ...
    if (with_legacy_time) {
        write_hdf5_scalar_double(group, "time", time_s);
    }
    if (with_model_time) {
        write_hdf5_scalar_double(group, "model_time_s", time_s);
    }
    if (with_model_step) {
        write_hdf5_scalar_int64(group, "model_step", step);
    }
...
void
write_netcdf_file(MPI_Comm comm,
    const fs::path& path,
    bool with_scalar_vars,
    bool with_global_attrs,
    const char* time_units,
    double time_s,
    long long step) {
    int ncid = -1;
    // CDF-5 (NC_64BIT_DATA): NC_INT64 does not exist in the classic formats.
    if (ncmpi_create(comm,
            path.string().c_str(),
...
RestartFileMetadata
read_netcdf_metadata(MPI_Comm comm, const fs::path& path) {
    int ncid = -1;
...
RestartFileMetadata
read_hdf5_metadata(const fs::path& path) {
...
test_hdf5(const fs::path& dir) {
    ...
    if (rank == 0) {
        write_hdf5_output_file(both, true, true, true, time_s, step);
    }
    ...
    if (rank == 0) {
        write_hdf5_output_file(legacy, false, false, true, time_s, step);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        write_hdf5_output_file(bare, false, false, false, time_s, step);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        hid_t file =
            H5Fcreate(root_level.string().c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        write_hdf5_scalar_double(file, "model_time_s", time_s);
        write_hdf5_scalar_int64(file, "model_step", step);
        H5Fclose(file);
    }
    MPI_Barrier(MPI_COMM_WORLD);
    check_metadata(read_hdf5_metadata(root_level),
        true,
        time_s,
        true,
        step,
    ...
    if (rank == 0) {
        hid_t file =
            H5Fcreate(float_time.string().c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        hid_t group = H5Gcreate2(file, "/Step0", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    check_metadata(read_hdf5_metadata(float_time),
        true,
        time_s,
        false,
        0,
...
test_netcdf(const fs::path& dir) {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    check_metadata(read_netcdf_metadata(MPI_COMM_WORLD, attrs),
        true,
        time_s,
        true,
        step,
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    check_metadata(read_netcdf_metadata(MPI_COMM_WORLD, seconds),
        true,
        time_s,
        false,
        0,
    ...
    write_netcdf_file(MPI_COMM_WORLD,
        calendar,
        false,
        false,
        "hours since 2025-10-07 00:00:00",
        time_s,
        step);
    MPI_Barrier(MPI_COMM_WORLD);

    const RestartFileMetadata calendar_meta = read_netcdf_metadata(MPI_COMM_WORLD, calendar);
    check_metadata(calendar_meta,
        false,
        0.0,
        false,
        0,
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    check_metadata(read_netcdf_metadata(MPI_COMM_WORLD, bare),
        false,
        0.0,
        false,
        0,
...
main(int argc, char** argv) {
    ...
    long long tag = 0;
    if (rank == 0) {
        tag = static_cast<long long>(getpid());
    }
    ...
    if (rank == 0) {
        if (total_failures != 0) {
            std::fprintf(stderr,
                "test_restart_metadata_io: %d check(s) failed over %d rank(s)\n",
                total_failures,
                nranks);
        }
        else {
