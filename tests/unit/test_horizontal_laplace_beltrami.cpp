...
void
set_constant_geometry(HorizontalGeometryDeviceView& geometry,
    const VVM::Real dq1,
    const VVM::Real dq2,
...
HorizontalLaplaceBeltramiDeviceView
make_constant_metric_laplacian(const VVM::Real dq1,
    const VVM::Real dq2,
    const VVM::Real sqrt_g,
    const VVM::Real g11,
    const VVM::Real g12,
...
test_constant_scalar() {
    ...
    Kokkos::parallel_for("EvaluateConstantScalarLaplacian",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(
            const int) { result() = laplacian.calculate_at_t(layout.halo, layout.halo, scalar); });

    VVM::Real result_host = VVM::real(0.0);
    Kokkos::deep_copy(result_host, result);

    check(close(result_host, VVM::real(0.0)),
        "A constant scalar must have zero Laplace-Beltrami value");
}
...
test_cross_metric_mixed_derivative() {
    ...
    const auto laplacian = make_constant_metric_laplacian(dq1,
        dq2,
        VVM::real(3.0),
        VVM::real(2.0),
        g12,
    ...
    Kokkos::parallel_for("EvaluateCrossMetricLaplacian",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) { result() = laplacian.calculate_at_t(0, 0, scalar); });

    VVM::Real result_host = VVM::real(0.0);
    Kokkos::deep_copy(result_host, result);

    check(close(result_host, VVM::real(2.0) * g12),
        "The Laplace-Beltrami operator must include both mixed-derivative contributions");
}
...
test_cartesian_five_point_equivalence() {
    ...
    Kokkos::parallel_for("InitializeCartesianLaplacianScalar",
        ...
        KOKKOS_LAMBDA(const int j, const int i) {
            ...
            scalar(j, i) =
                VVM::real(0.25) * x * x + VVM::real(0.125) * y * y + VVM::real(0.01) * x * y;
        });

    Kokkos::parallel_for("CompareCartesianLaplacian",
    ...
    check(maximum_difference <= VVM::real(1.0e-5),
        "Cartesian Laplace-Beltrami must equal the legacy five-point Laplacian");
}
...
test_quadratic_scalar() {
    ...
    Kokkos::parallel_for("InitializeQuadraticScalar",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            const VVM::Real x = t.q1(j, i);
            const VVM::Real y = t.q2(j, i);
    ...
    Kokkos::parallel_for("EvaluateQuadraticLaplacian",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
    ...
    check(maximum_error <= VVM::real(1.0e-4),
        "The Cartesian Laplacian of x^2 + 3y^2 must equal eight");
}

VVM::Real
periodic_laplacian_error(const int n) {
    ...
    Kokkos::parallel_for("InitializePeriodicLaplacianScalar",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            scalar(j, i) = Kokkos::sin(t.q1(j, i)) + Kokkos::cos(t.q2(j, i));
        });

    Kokkos::parallel_for("EvaluatePeriodicLaplacian",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            result(j, i) = laplacian.calculate_at_t(scalar, j, i);
        });
...
test_regular_latlon_diagonal_matches_center_impulse() {
    ...
    Kokkos::View<VVM::Real*> applied_at_t("regular_latlon_impulse_applied_t", ny);
    Kokkos::View<VVM::Real*> diagonal_at_t("regular_latlon_impulse_diagonal_t", ny);
    Kokkos::View<VVM::Real*> weighted_applied_at_t("regular_latlon_weighted_impulse_applied_t", ny);
    Kokkos::View<VVM::Real*> weighted_diagonal_at_t("regular_latlon_weighted_impulse_diagonal_t",
        ny);

    Kokkos::View<VVM::Real*> applied_at_z("regular_latlon_impulse_applied_z", ny);
    Kokkos::View<VVM::Real*> diagonal_at_z("regular_latlon_impulse_diagonal_z", ny);
    Kokkos::View<VVM::Real*> weighted_applied_at_z("regular_latlon_weighted_impulse_applied_z", ny);
    Kokkos::View<VVM::Real*> weighted_diagonal_at_z("regular_latlon_weighted_impulse_diagonal_z",
        ny);

    Kokkos::parallel_for("CompareRegularLatLonLaplacianDiagonals",
        ...
        KOKKOS_LAMBDA(const int j) {
            ...
            applied_at_t(j) = laplacian.calculate_at_t(j, i, impulse_at_t);
            diagonal_at_t(j) = laplacian.diagonal_at_t(j, i);

            weighted_applied_at_t(j) =
                laplacian.calculate_jacobian_weighted_at_t(j, i, impulse_at_t);
            weighted_diagonal_at_t(j) = laplacian.jacobian_weighted_diagonal_at_t(j, i);

            applied_at_z(j) = laplacian.calculate_at_z(j, i, impulse_at_z);
            diagonal_at_z(j) = laplacian.diagonal_at_z(j, i);

            weighted_applied_at_z(j) =
                laplacian.calculate_jacobian_weighted_at_z(j, i, impulse_at_z);
            weighted_diagonal_at_z(j) = laplacian.jacobian_weighted_diagonal_at_z(j, i);
        });
    ...
    const auto weighted_applied_at_t_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), weighted_applied_at_t);
    const auto weighted_diagonal_at_t_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), weighted_diagonal_at_t);
    ...
    const auto weighted_applied_at_z_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), weighted_applied_at_z);
    const auto weighted_diagonal_at_z_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), weighted_diagonal_at_z);

    const VVM::Real relative_tolerance =
        VVM::real(64.0) * std::numeric_limits<VVM::Real>::epsilon();

    for (int j = h; j < ny - h; ++j) {
        const auto check_pair = [&](const VVM::Real applied_value,
                                    const VVM::Real diagonal_value,
                                    const char* message) {
            const VVM::Real difference = std::abs(applied_value - diagonal_value);
            const VVM::Real scale = std::max(std::abs(applied_value), std::abs(diagonal_value));

            check(scale == VVM::real(0.0) ? difference == VVM::real(0.0)
                                          : difference <= relative_tolerance * scale,
                message);
        };

        check_pair(applied_at_t_host(j),
            diagonal_at_t_host(j),
            "RLL T diagonal must equal the T center-impulse response");

        check_pair(weighted_applied_at_t_host(j),
            weighted_diagonal_at_t_host(j),
            "RLL weighted T diagonal must equal the weighted T center-impulse response");

        check_pair(applied_at_z_host(j),
            diagonal_at_z_host(j),
            "RLL Z diagonal must equal the Z center-impulse response");

        check_pair(weighted_applied_at_z_host(j),
...
test_regular_latlon_shifted_jacobi_equation() {
    ...
    const VVM::Real inverse_dq1_squared = VVM::real(1.0) / (geometry.dq1() * geometry.dq1());
    const VVM::Real inverse_dq2_squared = VVM::real(1.0) / (geometry.dq2() * geometry.dq2());

    const VVM::Real diagonal_shift = VVM::real(2.5e-7);
    const VVM::Real right_hand_side = VVM::real(1.25e-12);

    Kokkos::View<VVM::Real*> solver_at_t("regular_latlon_solver_update_t", ny);
    Kokkos::View<VVM::Real*> reference_at_t("regular_latlon_reference_update_t", ny);
    Kokkos::View<VVM::Real*> solver_at_z("regular_latlon_solver_update_z", ny);
    Kokkos::View<VVM::Real*> reference_at_z("regular_latlon_reference_update_z", ny);

    Kokkos::parallel_for("CompareRegularLatLonShiftedJacobiEquation",
        Kokkos::RangePolicy<>(h, ny - h),
        KOKKOS_LAMBDA(const int j) {
            ...
            const VVM::Real operator_at_t =
                laplacian.calculate_jacobian_weighted_at_t(j, i, scalar_at_t);
            const VVM::Real diagonal_at_t = laplacian.jacobian_weighted_diagonal_at_t(j, i);

            solver_at_t(j) =
                scalar_at_t.center + (operator_at_t - t.sqrt_g(j, i) * right_hand_side) /
            ...
            reference_at_t(j) =
                (diagonal_shift * scalar_at_t.center + t_q1_west * scalar_at_t.west +
                    t_q1_east * scalar_at_t.east + t_q2_south * scalar_at_t.south +
                    t_q2_north * scalar_at_t.north - t.sqrt_g(j, i) * right_hand_side) /
                (diagonal_shift + t_q1_west + t_q1_east + t_q2_south + t_q2_north);

            const VVM::Real operator_at_z =
                laplacian.calculate_jacobian_weighted_at_z(j, i, scalar_at_z);
            const VVM::Real diagonal_at_z = laplacian.jacobian_weighted_diagonal_at_z(j, i);

            solver_at_z(j) =
                scalar_at_z.center + (operator_at_z - z.sqrt_g(j, i) * right_hand_side) /
            ...
            reference_at_z(j) =
                (diagonal_shift * scalar_at_z.center + z_q1_west * scalar_at_z.west +
                    z_q1_east * scalar_at_z.east + z_q2_south * scalar_at_z.south +
                    z_q2_north * scalar_at_z.north - z.sqrt_g(j, i) * right_hand_side) /
                (diagonal_shift + z_q1_west + z_q1_east + z_q2_south + z_q2_north);
        });

    const auto solver_at_t_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), solver_at_t);
    const auto reference_at_t_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), reference_at_t);
    const auto solver_at_z_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), solver_at_z);
    const auto reference_at_z_host =
        Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), reference_at_z);

    const VVM::Real relative_tolerance =
        sizeof(VVM::Real) == sizeof(double) ? VVM::real(2.0e-12) : VVM::real(2.0e-4);

    for (int j = h; j < ny - h; ++j) {
        const VVM::Real t_scale = std::max(VVM::real(1.0), std::abs(reference_at_t_host(j)));
        const VVM::Real z_scale = std::max(VVM::real(1.0), std::abs(reference_at_z_host(j)));

        check(std::abs(solver_at_t_host(j) - reference_at_t_host(j)) <=
                  relative_tolerance * t_scale,
            "RLL T relaxation must match the CVVM shifted-Jacobi equation");

        check(std::abs(solver_at_z_host(j) - reference_at_z_host(j)) <=
