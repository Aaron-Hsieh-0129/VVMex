...
void
append(std::vector<Real>& destination, const std::vector<Real>& source) {
    destination.insert(destination.end(), source.begin(), source.end());
}

int
wrap(int value, int size) {
    value %= size;
    if (value < 0) {
        value += size;
    }
...
int
clamp(int value, int size) {
    return std::max(0, std::min(size - 1, value));
}

template <std::size_t Dim>
bool
centered_q2_neumann_halos(const Grid& grid, const Field<Dim>& field) {
    static_assert(Dim == 2 || Dim == 3);

    const int h = grid.get_halo_cells();
    const bool south = grid.get_local_physical_start_y() == 0;
    const bool north = grid.get_local_physical_end_y() == grid.get_global_points_y() - 1;
    const auto data = field.get_host_data();
    const int ny = static_cast<int>(data.extent(Dim - 2));
    const int nx = static_cast<int>(data.extent(Dim - 1));
    int nz = 1;
    if constexpr (Dim == 3) {
        nz = static_cast<int>(data.extent(0));
    }
    ...
    const auto rows_equal = [&](int first_j, int second_j) {
        for (int k = 0; k < nz; ++k) {
            for (int i = h; i < nx - h; ++i) {
                if constexpr (Dim == 2) {
                    if (data(first_j, i) != data(second_j, i)) {
                        return false;
                    }
                }
                else {
                    if (data(k, first_j, i) != data(k, second_j, i)) {
                        return false;
                    }
                }
            }
    ...
    if (south) {
        for (int distance = 0; distance < h; ++distance) {
            if (!rows_equal(h - 1 - distance, h + distance)) {
                return false;
            }
        }
    }
    ...
    if (north) {
        ...
        for (int distance = 0; distance < h; ++distance) {
            if (!rows_equal(wall_j + 1 + distance, wall_j - distance)) {
                return false;
            }
        }
    }
...
template <std::size_t Dim>
bool
positive_face_q2_dirichlet_halos(const Grid& grid, const Field<Dim>& field) {
    static_assert(Dim == 2 || Dim == 3);
    ...
    const auto rows_are_odd = [&](int exterior_j, int interior_j) {
        for (int k = 0; k < nz; ++k) {
            for (int i = h; i < nx - h; ++i) {
                if (value(k, exterior_j, i) != -value(k, interior_j, i)) {
                    return false;
                }
            }
        }
    ...
    if (south) {
        ...
        if (!wall_is_zero(wall_j)) {
            return false;
        }
        for (int distance = 1; distance < h; ++distance) {
            if (!rows_are_odd(wall_j - distance, wall_j + distance)) {
                return false;
            }
        }
    }
...
bool
free_slip_physical_wind_halos(const Grid& grid, const Field<3>& u, const Field<3>& v) {
    const int h = grid.get_halo_cells();
    const bool south = grid.get_local_physical_start_y() == 0;
    const bool north = grid.get_local_physical_end_y() == grid.get_global_points_y() - 1;
    ...
    const auto h1 = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(),
        grid.geometry()
    ...
    const auto physical_u_equal = [&](int k, int exterior_j, int interior_j, int i) {
        ...
        const Real scale =
            std::max(real(1.0), std::max(Kokkos::abs(actual), Kokkos::abs(expected)));
        ...
        for (int i = h; i < nx - h; ++i) {
            if (south) {
                ...
                for (int distance = 0; distance < h; ++distance) {
                    ...
                    if (!physical_u_equal(k, exterior_j, h + distance, i)) {
                        return false;
                    }
                    if (distance > 0 &&
                        v_data(k, exterior_j, i) != -v_data(k, wall_j + distance, i)) {
                        return false;
                    }
                }
            }
            if (north) {
                ...
                for (int distance = 0; distance < h; ++distance) {
                    if (!physical_u_equal(k, wall_j + 1 + distance, wall_j - distance, i)) {
                        return false;
                    }
                }
                for (int distance = 1; distance <= h; ++distance) {
                    if (v_data(k, wall_j + distance, i) != -v_data(k, wall_j - distance, i)) {
                        return false;
                    }
                }
            }
...
struct Sources {
    ...
    void
    initialize_profiles(bool stretched) {
    ...
    Real
    factor(int step) const {
        return step == 2 ? real(0.0) : Real(step + 1);
    }

    Real
    top_zeta_value(const Grid& grid, int step, int global_j, int global_i) const {
        ...
        const Real angle = real(2.0) * real(std::acos(-1.0)) * Real(global_i) / Real(nx);

        return factor(step) *
               (real(1.0e-5) * Kokkos::sin(angle) + real(2.0e-6) * Real(global_j - ny / 2));
    }

    void
    initialize_step(const Grid& grid, HaloExchanger& halo, int step) {
        initialize_step(grid, step);

        halo.exchange_multiple_halos(std::vector<Field<3>*>{&xi, &eta});
    ...
    bool
    free_slip_boundaries(const Grid& grid) const {
        return positive_face_q2_dirichlet_halos(grid, xi) && centered_q2_neumann_halos(grid, eta);
    }

    void
    initialize_step(const Grid& grid, int step) {
        ...
        for (int k = 0; k < nz; ++k) {
            for (int j = 0; j < ny; ++j) {
                const int global_j = clamp(grid.get_local_physical_start_y() + j - h, global_ny);

                for (int i = 0; i < nx; ++i) {
                    const int global_i = wrap(grid.get_local_physical_start_x() + i - h, global_nx);

                    const Real longitude = two_pi * Real(global_i) / Real(global_nx);

                    x(k, j, i) =
                        scale * (real(2.0e-4) * Kokkos::sin(longitude) +
                                    real(4.0e-6) * Real(global_j) + real(1.0e-6) * Real(k));

                    e(k, j, i) =
                        scale * (real(1.5e-4) * Kokkos::cos(longitude) -
                                    real(3.0e-6) * Real(global_j) + real(2.0e-6) * Real(k));
                }
            }
        }
        ...
        Kokkos::deep_copy(zonal_covariant_increment.get_mutable_device_data(), scale * real(1.0e6));
    }
...
template <std::size_t Dim>
bool
constant_q2_halos(const Grid& grid, const Field<Dim>& field) {
    static_assert(Dim == 2 || Dim == 3);
...
struct DiagnosticState {
    ...
    explicit DiagnosticState(const Grid& grid)
        : psi("combined_psi", {grid.get_local_total_points_y(), grid.get_local_total_points_x()}),
          psi_previous("combined_psi_previous",
              {grid.get_local_total_points_y(), grid.get_local_total_points_x()}),
    ...
    void
    prepare_step(const Grid& grid, const Sources& sources, int step) {
        const int nz = grid.get_local_total_points_z();
        const int ny = grid.get_local_total_points_y();
        const int nx = grid.get_local_total_points_x();
        const int h = grid.get_halo_cells();
        ...
        for (int k = 0; k < nz; ++k) {
            for (int j = 0; j < ny; ++j) {
                ...
                for (int i = 0; i < nx; ++i) {
                    const int global_i = grid.get_local_physical_start_x() + i - h;

                    zeta_host(k, j, i) =
                        k == top ? sources.top_zeta_value(grid, step, global_j, global_i)
                                 : sentinel;
                }
            }
        }

        Kokkos::deep_copy(zeta.get_mutable_device_data(), zeta_host);
    ...
    bool
    free_slip_boundaries(const Grid& grid) const {
        return positive_face_q2_dirichlet_halos(grid, psi) &&
               ...
               positive_face_q2_dirichlet_halos(grid, zeta) && centered_q2_neumann_halos(grid, w) &&
               centered_q2_neumann_halos(grid, w_previous) &&
               free_slip_physical_wind_halos(grid, u, v);
    }

    bool
    physical_checks(const Grid& grid, const Sources& sources, int step) const {
        const int nz = grid.get_local_total_points_z();
        const int ny = grid.get_local_total_points_y();
        const int nx = grid.get_local_total_points_x();
        const int h = grid.get_halo_cells();
        ...
        for (int j = h; j < ny - h; ++j) {
            ...
            for (int i = h; i < nx - h; ++i) {
                ...
                valid = valid && zeta_data(top, j, i) ==
                                     sources.top_zeta_value(grid, step, global_j, global_i);

                valid =
                    valid && w_data(bottom, j, i) == real(0.0) && w_data(top, j, i) == real(0.0);

                for (int k = h; k < top; ++k) {
                    valid = valid && std::isfinite(w_data(k, j, i));
                }

                for (int k = bottom; k <= top; ++k) {
                    valid = valid && std::isfinite(zeta_data(k, j, i)) &&
                            std::isfinite(u_data(k, j, i)) && std::isfinite(v_data(k, j, i)) &&
                            u_data(k, j, i) != sentinel && v_data(k, j, i) != sentinel;
                }

                valid = valid && std::isfinite(zeta_data(top + 1, j, i)) &&
...
int
run_case(const Grid& grid, HaloExchanger& halo, bool stretched) {
    ...
    const Real shift = stretched ? real(0.25) : real(0.0);

    VerticalEllipticSolver direct_vertical(grid,
        halo,
        sources.rhobar,
        sources.rhobar_up,
        sources.flex_mid,
    ...
    VerticalEllipticSolver replayed_vertical(grid,
        halo,
        sources.rhobar,
        sources.rhobar_up,
        sources.flex_mid,
    ...
    HorizontalEllipticSolver direct_horizontal(grid, halo);
    ...
    execute(replayed_vertical, replayed_horizontal, replayed);
    ...
    for (int step = 0; step < 3; ++step) {
        ...
        const auto before_sources = sources.values();
        ...
        const bool exact = direct.values() == replayed.values();

        const bool sources_preserved = sources.values() == before_sources;

        const bool physical = direct.physical_checks(grid, sources, step) &&
                              replayed.physical_checks(grid, sources, step);

        const bool free_slip_walls = sources.free_slip_boundaries(grid) &&
                                     direct.free_slip_boundaries(grid) &&
                                     replayed.free_slip_boundaries(grid);

        int local_flags[4] = {int(exact),
        ...
        MPI_Allreduce(local_flags, global_flags, 4, MPI_INT, MPI_MIN, grid.get_comm());

        const bool pass = global_flags[0] && global_flags[1] && global_flags[2] && global_flags[3];

        if (grid.get_mpi_rank() == 0) {
            std::printf("%s ranks=%d stretched=%d step=%d exact=%d sources=%d physical=%d "
                        "free_slip_walls=%d %s\n",
                execution,
                grid.get_mpi_size(),
                int(stretched),
...
run(const Grid& grid, HaloExchanger& halo) {
    ...
    for (bool stretched : {false, true}) {
        failures += run_case(grid, halo, stretched);
    }
...
main(int argc, char** argv) {
    ...
    try {
        ...
        if (argc != 2 || (ranks != 1 && ranks != 2 && ranks != 4)) {
            ...
            fatal("Provide one RLL configuration and use 1, 2, or 4 ranks.");
        }

        {
            ...
            nccl_check(ncclCommInitRank(&communicator, ranks, id, rank));

            {
                HaloExchanger halo(config, grid, communicator, Kokkos::Cuda().cuda_stream());
