...
Result
run_case(const Grid& grid, HorizontalEllipticSolver& solver, int mode, int iterations) {
    ...
    for (int j = 0; j < ny; ++j) {
        ...
        for (int i = 0; i < nx; ++i) {
            ...
            host[2](j, i) =
                static_cast<Real>(active * 1000.0 * (std::sin(angle_z) * std::cos(angle_y) + 0.3));
            host[3](j, i) =
                static_cast<Real>(active * 700.0 * (std::cos(angle_t) * std::sin(angle_y) - 0.2));
    ...
    result.valid =
        result.valid && unchanged(omega2, omega2_host) && unchanged(spacing, spacing_host);
    ...
    for (int j = 0; j < physical_ny; ++j) {
        ...
        for (int i = 0; i < physical_nx; ++i) {
            ...
            for (int k = bottom; k <= top; ++k) {
                const std::size_t volume_index =
                    static_cast<std::size_t>(k - bottom) * cells + index;
    ...
    for (int k = 0; k < nz; ++k) {
        for (int j = 0; j < ny; ++j) {
            for (int i = 0; i < nx; ++i) {
                ...
                if (!written) {
                    result.valid =
                        result.valid && first(k, j, i) == sentinel && second(k, j, i) == sentinel;
                }
            }
        }
    }
    ...
    for (const auto& values : result.fields) {
        for (double value : values) {
            ...
            if (mode == 0) {
                result.valid = result.valid && value == 0.0;
            }
        }
    }
...
std::vector<Result>
run_cases(const Grid& grid, HaloExchanger& halo_exchanger) {
...
std::vector<Result>
evaluate(const ConfigurationManager& config, MPI_Comm communicator) {
...
int
compare(const Result& reference, const Result& actual, int ranks) {
    ...
    if (reference.mode != actual.mode || reference.iterations != actual.iterations ||
        reference.start_i != 0 || reference.start_j != 0 || actual.start_i < 0 ||
        actual.start_j < 0 || actual.start_i + actual.nx > reference.nx ||
        ...
        for (double value : reference.fields[component]) {
            if (std::isfinite(value)) {
                reference_scales[component] =
                    std::max(reference_scales[component], std::abs(value));
            }
            else {
                local_invalid = 1;
            }
        ...
        for (int k = 0; k < levels; ++k) {
            for (int j = 0; j < actual.ny; ++j) {
                for (int i = 0; i < actual.nx; ++i) {
                    const std::size_t local_index = static_cast<std::size_t>(k) * local_cells +
                                                    static_cast<std::size_t>(j) * actual.nx + i;
                    const std::size_t global_index =
                        static_cast<std::size_t>(k) * reference_cells +
                        static_cast<std::size_t>(actual.start_j + j) * reference.nx +
                        actual.start_i + i;
                    ...
                    if (!std::isfinite(expected) || !std::isfinite(value)) {
                        ...
                        local_errors[component] =
                            std::max(local_errors[component], std::abs(value - expected));
                    }
                }
            }
        }
    ...
    MPI_Allreduce(local_errors.data(),
        global_errors.data(),
        4,
        MPI_DOUBLE,
        MPI_MAX,
        MPI_COMM_WORLD);
    MPI_Allreduce(reference_scales.data(),
        global_scales.data(),
        4,
        MPI_DOUBLE,
        MPI_MAX,
    ...
    if (rank == 0) {
        std::printf("ranks=%d mode=%d iterations=%d psi=%.3e chi=%.3e wind_e=%.3e wind_n=%.3e "
                    "inputs_regions_finite=%d %s\n",
            ranks,
            actual.mode,
            actual.iterations,
            global_errors[0],
    ...
    try {
        if (argc < 2) {
            throw std::invalid_argument(
                "Usage: test_horizontal_wind_solver_column_mpi <configuration.json>");
        }
        ...
        std::fprintf(stderr,
            "Rank %d test_horizontal_wind_solver_column_mpi: %s\n",
            rank,
            error.what());
