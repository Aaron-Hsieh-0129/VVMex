...
std::vector<Real> snapshot(
    ...
    if constexpr (Dim == 1) {
        ...
        static_assert(
            Dim == 1 || Dim == 3,
            "Unsupported snapshot dimension.");
    }
...
public:
    ...
    void finish() {
        ...
        require_cuda(
            status,
            "end scalar-transport capture");

        if (graph_ == nullptr) {
            throw std::runtime_error(
                "Scalar-transport capture returned "
                "a null graph.");
        }
        ...
        if (node_count == 0) {
            throw std::runtime_error(
                "Scalar-transport capture recorded "
                "no nodes.");
        }
    ...
    void launch() {
        require_cuda(
            cudaGraphLaunch(
                executable_,
                stream_),
            "launch scalar-transport graph");

        require_cuda(
            cudaStreamSynchronize(stream_),
            "complete scalar-transport graph");
    }
...
void test_validation(
    ...
    const CartesianGeometry cartesian(
        layout,
        real(1000.0),
        real(1000.0));
    ...
    try {
        const RegularLatLonScalarTransport
            invalid(cartesian);
    ...
    Field<3> scalar(
        "validation_scalar",
        {nz, ny, nx});
    Field<3> mass_flux_q1(
        "validation_mass_flux_q1",
        {nz, ny, nx});
    Field<3> mass_flux_q2(
        "validation_mass_flux_q2",
        {nz, ny, nx});
    Field<3> mass_flux_vertical(
        "validation_mass_flux_vertical",
        {nz, ny, nx});
    Field<1> spacing(
        "validation_spacing",
        {nz});
    ...
    try {
        transport.add_flux_convergence(
            scalar,
            mass_flux_q1,
            mass_flux_q2,
            mass_flux_vertical,
    ...
    check(
        rejected_short_range,
        "RLL scalar transport must reject "
        "a vertical range shorter than three cells");
}

void test_transport_and_replay(
    const RegularLatLonGeometry& geometry) {
    ...
    Field<3> scalar(
        "stage_scalar",
        {nz, ny, nx});
    Field<3> physical_mass_flux_q1(
        "stage_physical_mass_flux_q1",
        {nz, ny, nx});
    Field<3> physical_mass_flux_q2(
        "stage_physical_mass_flux_q2",
        {nz, ny, nx});
    Field<3> vertical_mass_flux(
        "stage_vertical_mass_flux",
        {nz, ny, nx});
    Field<1> vertical_spacing(
        "stage_vertical_spacing",
        {nz});
    ...
    auto scalar_data =
        scalar.get_mutable_device_data();
    auto physical_q1_data =
        physical_mass_flux_q1
            .get_mutable_device_data();
    auto physical_q2_data =
        physical_mass_flux_q2
            .get_mutable_device_data();
    auto vertical_flux_data =
        vertical_mass_flux
            .get_mutable_device_data();
    auto spacing_data =
        vertical_spacing
    ...
    Kokkos::parallel_for(
        "InitializeRllScalarTransportVolumes",
        ...
        KOKKOS_LAMBDA(
            ...
            const Real level =
                static_cast<Real>(k);
            const Real latitude_index =
                static_cast<Real>(j - halo);
            const Real longitude_index =
                static_cast<Real>(i - halo);

            scalar_data(k, j, i) =
                real(2.0)
                + real(0.03) * longitude_index
                - real(0.02) * latitude_index
                + real(0.04) * level
                + real(0.001)
                    * longitude_index
                    * latitude_index;

            physical_q1_data(k, j, i) =
                real(1.1)
                + real(0.025) * longitude_index
                + real(0.015) * latitude_index
                + real(0.01) * level;

            physical_q2_data(k, j, i) =
                real(-0.6)
                + real(0.012) * longitude_index
                - real(0.02) * latitude_index
                + real(0.008) * level;

            vertical_flux_data(k, j, i) =
                real(0.08)
                + real(0.004) * longitude_index
                - real(0.003) * latitude_index
                + real(0.02) * level;
        });

    Kokkos::parallel_for(
        "InitializeRllScalarTransportSpacing",
    ...
    Kokkos::deep_copy(
        expected.get_mutable_device_data(),
        sentinel);
    Kokkos::deep_copy(
        direct.get_mutable_device_data(),
        sentinel);
    Kokkos::deep_copy(
        replay.get_mutable_device_data(),
        sentinel);
    ...
    const auto scalar_reference =
        scalar.get_device_data();
    const auto physical_q1_reference =
        physical_mass_flux_q1.get_device_data();
    const auto physical_q2_reference =
        physical_mass_flux_q2.get_device_data();
    const auto vertical_flux_reference =
        vertical_mass_flux.get_device_data();
    const auto spacing_reference =
        vertical_spacing.get_device_data();
    ...
    const auto adapted_q1 =
        make_orthogonal_contravariant_mass_flux_q1_device_view(
            geometry,
            physical_q1_reference);
    const auto adapted_q2 =
        make_orthogonal_contravariant_mass_flux_q2_device_view(
            geometry,
            physical_q2_reference);

    Kokkos::parallel_for(
        "EvaluateExplicitRllScalarTransportReference",
        Kokkos::MDRangePolicy<
            Kokkos::Rank<3>>(
                {k_begin, halo, halo},
    ...
    const auto expected_host =
        expected.get_host_data();
    const auto direct_host =
        direct.get_host_data();
    const auto replay_host =
        replay.get_host_data();
...
int main(
    ...
    if (failures == 0) {
        std::fprintf(
            stdout,
            "test_regular_latlon_scalar_transport: PASS\n");
    } else {
        std::fprintf(
            stderr,
