...
void
check_value(const Parameters& parameters,
    const Kokkos::View<Real>& field,
    const Real expected,
    const char* name) {
...
void
run_tests(const ConfigurationManager& config, const Grid& grid) {
    if (grid.geometry().kind() != GeometryKind::Cartesian) {
        ...
        try {
            ...
            rejected_for_units =
                std::string(error.what()).find("Cartesian dx/dy") != std::string::npos;
        }

        check(rejected_for_units,
            "Non-Cartesian Parameters must reject angular spacing entering Cartesian dx/dy.");
        return;
    }
    ...
    check(grid.horizontal_specification().geometry.dq1 == expected_dx,
        "Grid must resolve dx to 500 m.");
    check(grid.horizontal_specification().geometry.dq2 == expected_dy,
        "Grid must resolve dy to 750 m.");
    check(grid.vertical_specification().dz == expected_dz,
        "Grid must resolve reference dz to 1000 m.");
    ...
    try {
        if (argc < 2) {
            throw std::invalid_argument(
                "Usage: test_parameters_resolved_grid <configuration.json>");
        }
