...
test_explicit_jacobian_weighting() {
    ...
    curl.z.inv_sqrt_g = GeometryField2D::constant_value(VVM::real(0.25));
    ...
    Kokkos::parallel_for("EvaluateJacobianWeightedCurl",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) { result() = curl.calculate_at_z(0, 0, vector); });

    VVM::Real result_host = VVM::real(0.0);
    Kokkos::deep_copy(result_host, result);

    const VVM::Real expected =
        VVM::real(0.25) * ((VVM::real(9.0) - VVM::real(1.0)) / VVM::real(2.0) -
                              (VVM::real(11.0) - VVM::real(3.0)) / VVM::real(4.0));

    check(close(result_host, expected), "Horizontal curl must apply the Z-point inverse Jacobian");
}
...
test_constant_covariant_vector() {
    ...
    Kokkos::parallel_for("EvaluateConstantVectorCurl",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(
            const int) { result() = curl.calculate_at_z(layout.halo, layout.halo, vector); });

    VVM::Real result_host = VVM::real(0.0);
    Kokkos::deep_copy(result_host, result);

    check(close(result_host, VVM::real(0.0)), "A constant Cartesian vector must have zero curl");
}
...
test_cartesian_equivalence() {
    ...
    Kokkos::View<VVM::Real**> q1_at_u("cartesian_covariant_q1_at_u", ny, nx);
    Kokkos::View<VVM::Real**> q2_at_v("cartesian_covariant_q2_at_v", ny, nx);
    Kokkos::View<VVM::Real**> actual("cartesian_curl", ny, nx);
    Kokkos::View<VVM::Real**> cartesian_reference("cartesian_curl_reference", ny, nx);

    Kokkos::parallel_for("InitializeCartesianCurlVector",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            const VVM::Real i_value = static_cast<VVM::Real>(i - h);
            const VVM::Real j_value = static_cast<VVM::Real>(j - h);

            q1_at_u(j, i) = VVM::real(0.5) * i_value - VVM::real(0.25) * j_value * j_value;

            q2_at_v(j, i) = VVM::real(0.125) * i_value * i_value + VVM::real(0.75) * j_value;
        });

    Kokkos::parallel_for("CompareCartesianCurl",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            actual(j, i) = curl.calculate_at_z(q1_at_u, q2_at_v, j, i);

            cartesian_reference(j, i) =
                (q2_at_v(j, i + 1) - q2_at_v(j, i)) / dx - (q1_at_u(j + 1, i) - q1_at_u(j, i)) / dy;
        });
...
test_solid_body_rotation() {
    ...
    Kokkos::View<VVM::Real**> q1_at_u("solid_body_q1_at_u", ny, nx);
    Kokkos::View<VVM::Real**> q2_at_v("solid_body_q2_at_v", ny, nx);
    Kokkos::View<VVM::Real**> result("solid_body_curl", ny, nx);

    Kokkos::parallel_for("InitializeSolidBodyRotation",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            q1_at_u(j, i) = -angular_velocity * u.q2(j, i);

            q2_at_v(j, i) = angular_velocity * v.q1(j, i);
        });

    Kokkos::parallel_for("EvaluateSolidBodyRotationCurl",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            result(j, i) = curl.calculate_at_z(q1_at_u, q2_at_v, j, i);
        });

    const auto result_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), result);

    const VVM::Real expected = VVM::real(2.0) * angular_velocity;
    ...
    check(maximum_error <= VVM::real(1.0e-6),
...
test_three_dimensional_stencil_loader() {
    ...
    Kokkos::View<VVM::Real*> result("three_dimensional_curl_stencil", 4);

    Kokkos::parallel_for("InitializeThreeDimensionalCurlVector",
    ...
    Kokkos::parallel_for("LoadThreeDimensionalCurlStencil",
        Kokkos::RangePolicy<>(0, 1),
        KOKKOS_LAMBDA(const int) {
            const CovariantVectorStencilAtZ stencil =
                load_covariant_vector_stencil_at_z(q1_at_u, q2_at_v, 1, 1, 1);
    ...
    check(close(result_host(0), VVM::real(111.0)), "The 3-D loader must load q1 at U(j,i)");

    check(close(result_host(1), VVM::real(121.0)), "The 3-D loader must load q1 at U(j+1,i)");

    check(close(result_host(2), VVM::real(1110.0)), "The 3-D loader must load q2 at V(j,i)");

    check(close(result_host(3), VVM::real(1120.0)), "The 3-D loader must load q2 at V(j,i+1)");
}

VVM::Real
periodic_curl_error(const int n) {
    ...
    Kokkos::View<VVM::Real**> q1_at_u("periodic_curl_q1_at_u", ny, nx);
    Kokkos::View<VVM::Real**> q2_at_v("periodic_curl_q2_at_v", ny, nx);
    Kokkos::View<VVM::Real**> result("periodic_curl_result", ny, nx);

    Kokkos::parallel_for("InitializePeriodicCurlVector",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny, nx}),
        KOKKOS_LAMBDA(const int j, const int i) {
            q1_at_u(j, i) = Kokkos::sin(u.q2(j, i));

            q2_at_v(j, i) = Kokkos::cos(v.q1(j, i));
        });

    Kokkos::parallel_for("EvaluatePeriodicCurl",
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({h, h}, {ny - h, nx - h}),
        KOKKOS_LAMBDA(const int j, const int i) {
            result(j, i) = curl.calculate_at_z(q1_at_u, q2_at_v, j, i);
        });
    ...
    for (int j = h; j < ny - h; ++j) {
        for (int i = h; i < nx - h; ++i) {
            const VVM::Real x = (static_cast<VVM::Real>(i - h) + VVM::real(0.5)) * spacing;

            const VVM::Real y = (static_cast<VVM::Real>(j - h) + VVM::real(0.5)) * spacing;

            const VVM::Real expected = -std::sin(x) - std::cos(y);
...
test_second_order_convergence() {
    const VVM::Real coarse_error = periodic_curl_error(32);
    const VVM::Real fine_error = periodic_curl_error(64);
    const VVM::Real error_ratio = coarse_error / fine_error;

    check(coarse_error > fine_error, "Refining the Cartesian grid must reduce the curl error");
...
main(int argc, char** argv) {
    ...
    if (failures == 0) {
        std::fprintf(stdout, "test_horizontal_curl: PASS\n");
    }
    else {
        std::fprintf(stderr, "test_horizontal_curl: %d failure(s)\n", failures);
    }
