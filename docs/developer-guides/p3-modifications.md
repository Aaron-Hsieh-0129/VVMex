# P3 modifications from Fortran P3
...
This computes the tables in memory and writes them to `rundata/p3/`. Use a
single rank because every rank writes the same paths. Set the option back to
`false` afterwards.
...
## Finding the VVMex changes

Start with `p3_main_impl_part2.hpp` to see the process order. Function declarations
and dispatch live in `p3_functions.hpp`; the three implementation files named
above contain the coupled tendency calculations, droplet activation, and
limiting. Search for the tendency symbols such as `qv2qc_conden_tend` and
`qv2qi_depos_tend` when tracing their updates.
