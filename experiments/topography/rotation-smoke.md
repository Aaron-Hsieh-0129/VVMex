...
2026-09-10 update: `initial_conditions.rll_mountain.jet_center_latitude_deg` now independently tunes the jet's peak latitude. The current configuration sets both this and mountain `center_latitude_deg` to10°N. Peak amplitude and width are unchanged (case1:80m/s, support -12.5° to32.5°). Missing jet-center setting preserves the original profile for either Jung case, and the flat Jung reproduction is unchanged. The initializer recomputes background vorticity and channel streamfunction from the shifted wind while keeping metric latitude physical, and rejects a shifted jet extending outside the channel. The user has independently restored duration36000s; that edit is preserved. Build/check status for this update is recorded below; no new history run is authorized under the unresolved storage cap.
